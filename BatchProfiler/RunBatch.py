...
class BPBatch(object):
    ...
    def select_task_count(self, run_type):
        ...
        select count('x') 
         ...
         where r.batch_id = %d and r.run_type='%s'""" % (
...
class BPJobTask(object):
    ...
    def create(cursor, job, batch_array_task):
        ...
        values (%s, %s)
        """
        cursor.execute(cmd, [job.job_record_id,
                             batch_array_task.batch_array_task_id])
...
"""
        cursor.execute(cmd,
                       [run.run_id, JS_RUNNING, JS_DONE, JS_ERROR, run.run_id])
        return cursor.fetchone()[0]
