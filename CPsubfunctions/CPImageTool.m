...
if ~isempty(varargin)
    ...
    if ~isempty(ITh)
        ...
        set(ITh,'position',[pos(1) pos(2) 1.8 3]);

        % Get title of image
        Title = get(get(get(handle,'Parent'),'Title'),'String');

        % Create buttons
        Text = uicontrol(ITh,'style','text','units','normalized','position',[.03 .85 1 .1],'string',Title);
        set(Text,'Backgroundcolor',get(ITh,'Color'))
        NewWindow = uicontrol(ITh,'style','pushbutton','units','normalized','position',[.1 .7 .8 .1],'string','Open in new window');
        Histogram = uicontrol(ITh,'style','pushbutton','units','normalized','position',[.1 .58 .8 .1],'string','Show histogram','Tag','Histogram');
        MatlabWS  = uicontrol(ITh,'style','pushbutton','units','normalized','position',[.1 .46 .8 .1],'string','Save to work space');
        SaveImageAs = uicontrol(ITh,'style','pushbutton','units','normalized','position',[.1 .34 .8 .1],'string','Save to Harddrive');
