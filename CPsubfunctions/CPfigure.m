...
%                 ToggleScale = 'ImageHandles = findobj(gcf,''Type'',''Image'');if ~isempty(ImageHandles),button=findobj(gcf,''tag'',''ToggleScale'');ImageHandles = num2cell(ImageHandles);for i = 1:length(ImageHandles),cax = ancestor(ImageHandles{i},''axes'');if get(button,''value'')==0,set(cax,''CLimMode'',''manual'');set(cax,''CLim'',[0 1]);else,set(cax,''CLimMode'',''auto'');end,end,end;';
                ToggleScale = ...
                    ['ImageHandles = findobj(gcf,''Type'',''Image'');' ...
                    'if ~isempty(ImageHandles),' ...
                        'button=findobj(gcf,''tag'',''ToggleScale'');' ...
                ...
                uicontrol('Style', 'checkbox', ...
                    'Units','normalized',...
                    'Position', [.90 .95 .1 .04], ...
                    'Callback', ToggleScale, 'parent',FigHandle, ...
                    'FontSize',Font,'BackgroundColor',[.7,.7,.9],'min',0,...
                    'max',1,'value',1,'tag','ToggleScale','string','Scale');                
            end
        end
    end
