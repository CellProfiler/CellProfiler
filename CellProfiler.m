    ...
    while 1;
        ...
        elseif strncmp(output,'%filenametextVAR',16) && OptionInCode == SelectedOption
            ...
            handles.BrowseButton{ModuleNums}(lastVariableCheck) = uicontrol(...
                ...
                'Callback','handles=guidata(findobj(''tag'',''figure1''));[filename pathname] = uigetfile(fullfile(handles.Current.DefaultOutputDirectory,''*.*''),''Pick the file you want.'');UserData=get(gco,''UserData'');set(UserData{1},''string'',filename);handles.Settings.VariableValues{UserData{2},UserData{3}}=filename;guidata(handles.figure1,handles);clear UserData handles pathname filename;',...
                'FontName','helvetica',...
                'FontSize',handles.Current.FontSize,...
                'FontWeight','bold',...
...
SkipErrorHelpText = 'If you uncheck this box you will run in diagnostic mode, where all the intermediate images and calculations for the most recent image cycle are saved in the output file, which drastically increases the output file size. Check this box if you would instead like to run in normal mode, producing smaller output files:';
