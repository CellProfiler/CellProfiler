...
def parse_args(args):
    ...
    parser.add_option("-L", "--log-level",
                      ...
                      help=("Set the verbosity for logging messages: " +
                            ("%d or %s for debugging, " % (
                            logging.DEBUG, "DEBUG")) +
                            ("%d or %s for informational, " % (
                            logging.INFO, "INFO")) +
                            ("%d or %s for warning, " % (
...
def print_code_statistics():
    ...
    for filename in filelist:
        if (os.path.exists(filename) and
                any([filename.endswith(x) for x in
                     ".py", ".c", ".pyx", ".java"])):
            if filename.endswith(".c") and os.path.exists(
                            filename[:-1] + "pyx"):
                continue
...
def build_extensions():
    ...
    #
    compile_scripts = [(os.path.join('cellprofiler', 'utilities', 'setup.py'),
