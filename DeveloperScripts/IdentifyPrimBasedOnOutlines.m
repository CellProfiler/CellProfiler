...
% a black and white image where the background is white and the outlines
% are black.  If a problem is encounted where an error states that the
% image might be a color image, run the ColorToGray module before
...
%textVAR04 = Discard objects touching the border of the image?
%defaultVAR04 = Yes
%choiceVAR04 = Yes
%choiceVAR04 = No
ExcludeBorderObjects = char(handles.Settings.VariableValues{CurrentModuleNum,4});
%inputtypeVAR04 = popupmenu

%textVAR05 = Note: this module may fill in holes between objects that are not desired, so follow it with an identify primary objects module.
...
[Objects,NumOfObjects] = bwlabel(ObjectsIdentifiedImage > 0);
