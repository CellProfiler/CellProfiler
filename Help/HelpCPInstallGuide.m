...
% (Applications > Utilities > Terminal) and type the following to
% ensure that display windows behave properly:
% defaults write com.apple.x11 wm_click_through -bool true
%
% 2. Start Matlab and check licenses...
% Start Matlab and type into the main window (command line):
% 		license('test','image_toolbox')
% If the Image Processing Toolbox is installed with a valid license
% you will see: ans = 1, but if ans = 0, the toolbox and/or license is
% not installed. CellProfiler was designed for Matlab Release 14,
% version 7.1, SP3 although it is possible that many functions will
...
% Matlab window to start CellProfiler. To call CellProfiler directly,
% type the following in the main Matlab window (LOCATION is where the
...
% 
% 5. (Optional) Click File > Set preferences. This is just for
% convenience, and can be reset later, if you would like.  This step
% creates a file called CellProfilerPreferences.mat in the root
% directory of Matlab and will make it easier to use CellProfiler.
% 
% 6. (Optional) To run an example image analysis (very quick &
% useful!), download the folder ExampleFlyImages from:
% www.cellprofiler.org/linked_files/ExampleFlyImages.zip
% Then, in CellProfiler, click 'Load' pipeline of modules, choose the
