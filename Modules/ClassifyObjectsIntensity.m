...
MeasurementName = char(handles.Settings.VariableValues{CurrentModuleNum,2});

%textVAR03 = Which column of measurement do you want to use to classify objects? For intensity, integrated = 1, mean = 2. Sorry this isn't very user-friendly now.
%defaultVAR03 = 1
MeasurementColumnToUse = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,3}));

%textVAR04 = Enter the largest value for the low bin
%defaultVAR04 = 50
LowBinMax = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,4}));

%textVAR05 = Enter the smallest value for the high bin
%defaultVAR05 = 999999
HighBinMin = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,5}));
...
% Retrieves measurements from the handles structure.
AllMeasurements = handles.Measurements.(ObjectName).(MeasurementName)(handles.Current.SetBeingAnalyzed);
AllMeasurements = cell2mat(AllMeasurements);
Measurements = AllMeasurements(:,MeasurementColumnToUse);

index1 = find(Measurements < LowBinMax);
index3 = find(Measurements > HighBinMin);
index2 = setdiff(1:length(Measurements),[index1;index3]);
TotalNumberOfObjects = length(index1) + length(index2) + length(index3);
FractionInLowBin = length(index1)/TotalNumberOfObjects;
FractionInMediumBin = length(index2)/TotalNumberOfObjects;
FractionInHighBin = length(index3)/TotalNumberOfObjects;
...
if any(findobj == ThisModuleFigureNumber);
    ...
    uicontrol(ThisModuleFigureNumber,'style','text','units','normalized', 'position', [0.35+0.2*(columns-1) 0.85 0.2 0.03],...
        'HorizontalAlignment','center','BackgroundColor',[1 1 1],'fontname','times',...
        'fontsize',FontSize,'string',[num2str(FractionInLowBin), num2str(FractionInMediumBin), num2str(FractionInHighBin)]);
...
%         0.25 0.25 1;           % light blue RGB color for objects with area < LowBinMax
%         1 0 0;               % red RGB color for objects with LowBinMax < area < HighBinMin
%         1 1 0];              % yellow RGB color for objects with area > HighBinMin
