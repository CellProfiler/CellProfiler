function handles = AlgCorrectIlluminationSubtractAllMin_10(handles)
...
%textVAR3 = To save each corrected image, enter text to append to the image name 
%defaultVAR3 = N
fieldname = ['Vvariable',CurrentAlgorithm,'_03'];
SaveImage = handles.(fieldname);
%textVAR4 =  Otherwise, leave as "N". To save or display other images, press Help button
%textVAR5 = In what file format do you want to save images? Do not include a period
%defaultVAR5 = tif
fieldname = ['Vvariable',CurrentAlgorithm,'_05'];
FileFormat = handles.(fieldname);
%textVAR6 = Block size. This should be set large enough that every square block 
%defaultVAR6 = 60
fieldname = ['Vvariable',CurrentAlgorithm,'_06'];
BlockSize = str2num(handles.(fieldname));
%textVAR7 = of pixels is likely to contain some background. Default = 60 pixels
...
A = isspace(SaveImage);
if any(A) == 1
    error('Image processing was canceled because you have entered one or more spaces in the box of text to append to the object outlines image name in the Correct Illumination All Subtract module.  If you do not want to save the object outlines image to the hard drive, type "N" into the appropriate box.')
    return
end
...
if B == 1
    error('Image processing was canceled because you have not entered text to append to the object outlines image name in the Correct Illumination All Subtract module.  If you do not want to save the object outlines image to the hard drive, type "N" into the appropriate box.')
    return
end
...
if handles.setbeinganalyzed == 1
    ...
    if strcmp(IllumCorrectPathAndFileName, '/') ~= 1
        ...
        try
            ...
            % figure, imshow(MeanMiniIlluminationImage), title('mean mini illum image')
            ...
            % figure, imagesc(MeanIlluminationImage), title('mean illum image, resized')
...
%%%%% "block" of each image, and the values are averaged together for all
%%%%% images.  The block dimensions are entered by the user,
...
%%%%% Therefore, once the average minimums are determined across the images, the
...
%%%%% processed.
%%%%% This image is then subtracted from each original image to
