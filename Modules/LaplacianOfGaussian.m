function handles = LaplacianOfGaussian(handles)
...
%   Steve Lowe
%   Adapted from IdentifyPrimAdaptThresholdA.m,
%   Using core algorithm code supplied by Perlman, Mitchison, et al.
%   TODO: Figure out how this copyright will work.
%
% $Revision$
...
%textVAR03 = Neighborhood size (height, width in pixels)
%defaultVAR03 = 9 9
NbhdSizeStr = char(handles.Settings.VariableValues{CurrentModuleNum,3});

%textVAR04 = Sigma
%defaultVAR04 = 1.8
Sigma = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,4}));

%textVAR05 = Minimum Area
%defaultVAR05 = 3
MinArea = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,5}));

%textVAR06 = Size for Wiener filter (height, width in pixels)
%defaultVAR06 = 5 5
WienerSizeStr = char(handles.Settings.VariableValues{CurrentModuleNum,6});

%textVAR07 = Threshold
%defaultVAR07 = -.001
Threshold = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,7}));
...
if ndims(OrigImageToBeAnalyzed) ~= 2
    error('Image processing was canceled because the Laplacian Of Gaussian module requires an input image that is two-dimensional (i.e. X vs Y), but the image loaded does not fit this requirement.  This may be because the image is a color image.')
end
...
%%% image), which would disrupt several of the image functions.
if ndims(OrigImageToBeAnalyzed) ~= 2
    error('Image processing was canceled bergOutcause the Identify Primary Adaptive Threshold module requires an input image that is two-dimensional (i.e. X vs Y), but the image loaded does not fit this requirement.  This may be because the image is a color image.')
end
