...
class PreferencesView(object):
    ...
    def __init__(self, parent_sizer, panel, progress_panel, status_panel):
        ...
        self.__image_edit_box = self.__make_folder_panel(
            ...
            lambda: cellprofiler.configuration.get_recent_files(cellprofiler.configuration.DEFAULT_IMAGE_DIRECTORY),
            'Default Input Folder',
            cellprofiler.application.help.DEFAULT_IMAGE_FOLDER_HELP,
        ...
        self.__output_edit_box = self.__make_folder_panel(
            ...
            lambda: cellprofiler.configuration.get_recent_files(cellprofiler.configuration.DEFAULT_OUTPUT_DIRECTORY),
            'Default Output Folder',
            cellprofiler.application.help.DEFAULT_OUTPUT_FOLDER_HELP,
    ...
    def close(self):
        cellprofiler.configuration.remove_output_file_name_listener(self.__on_preferences_output_filename_event)
        cellprofiler.configuration.remove_image_directory_listener(self.__on_preferences_image_directory_event)
        cellprofiler.configuration.remove_output_directory_listener(self.__on_preferences_output_directory_event)
    ...
    def __make_odds_and_ends_panel(self):
        ...
        output_filename_edit_box = wx.TextCtrl(
            panel, value=cellprofiler.configuration.get_output_file_name())
        ...
        allow_output_filename_overwrite_check_box.Value = \
            cellprofiler.configuration.get_allow_output_file_overwrite()
        ...
        measurements_mode_idx = [cellprofiler.configuration.WRITE_HDF5, True, False].index(
            cellprofiler.configuration.get_write_MAT_files())
        ...
        cellprofiler.configuration.add_output_file_name_listener(
            self.__on_preferences_output_filename_event)
        cellprofiler.configuration.add_image_directory_listener(
            self.__on_preferences_image_directory_event)
        cellprofiler.configuration.add_output_directory_listener(
            self.__on_preferences_output_directory_event)
        self.__hold_a_reference_to_progress_callback = self.progress_callback
        cellprofiler.configuration.add_progress_callback(
            self.__hold_a_reference_to_progress_callback)

        def on_output_filename_changed(event):
            cellprofiler.configuration.set_output_file_name(output_filename_edit_box.Value)

        def on_allow_checkbox(event):
            cellprofiler.configuration.set_allow_output_file_overwrite(
                allow_output_filename_overwrite_check_box.Value)
        ...
        output_filename_help_button.Bind(
            wx.EVT_BUTTON,
            lambda event: self.__on_help(event, cellprofiler.application.help.USING_THE_OUTPUT_FILE_HELP))
    ...
    def check_preferences(self):
        ...
        if not os.path.isdir(path):
            ...
            os.makedirs(path)
            cellprofiler.configuration.set_default_image_directory(path)
        ...
        if not os.path.isdir(path):
            ...
            os.makedirs(path)
            cellprofiler.configuration.set_default_output_directory(path)
        return True, "OK"
    ...
    def __on_help(self, event, help_text):
        dlg = cellprofiler.application.htmldialog.HTMLDialog(self.__panel, "Help", help_text)
        dlg.Show()
    ...
    def __on_pixel_size_changed(self, event):
        ...
        if text.isdigit():
            cellprofiler.configuration.set_pixel_size(int(text))
            self.pop_error_text(error_text)
        else:
            self.set_error_text(error_text)

    def __on_preferences_output_filename_event(self, event):
        if self.__output_filename_edit_box.Value != cellprofiler.configuration.get_output_file_name():
            self.__output_filename_edit_box.Value = cellprofiler.configuration.get_output_file_name()

    def __on_preferences_output_directory_event(self, event):
        if self.__output_edit_box.Value != cellprofiler.configuration.get_default_output_directory():
            self.__output_edit_box.Value = cellprofiler.configuration.get_default_output_directory()

    def __on_preferences_image_directory_event(self, event):
        if self.__image_edit_box.Value != cellprofiler.configuration.get_default_image_directory():
    ...
    def refresh_input_directory():
        cellprofiler.configuration.fire_image_directory_changed_event()
