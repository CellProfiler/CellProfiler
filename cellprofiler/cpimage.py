...
class ImageCache(object):
    ...
    def __init__(self, image):
        ...
        if image.ndim == 2:
            ...
            self.__image = image.reshape(1, 1, 1, image.shape[0],
                                         image.shape[1])
        elif image.ndim == 3:
    ...
    def get(self):
        ...
        if self.__type == ImageCache.IC_MONOCHROME:
            ...
            return image.reshape(
                image.shape[0], image.shape[3], image.shape[4]).transpose(1, 2,
                                                                          0)
...
def check_consistency(image, mask):
    ...
    len(mask.shape) == 2), "Mask must have 2 dimensions"
    assert (image is None) or (mask is None) or (
    image.shape[:2] == mask.shape), "Image and mask sizes don't match"
    assert (mask is None) or (
    mask.dtype.type is np.bool_), "Mask must be boolean, was %s" % (
    repr(mask.dtype.type))
...
class ImageSetList(object):
    ...
    def save_state(self):
        ...
        for i in range(self.count()):
            ...
            assert isinstance(image_set, ImageSet)
            assert len(
                image_set.providers) == 0, "An image set cannot have providers while saving its state"
