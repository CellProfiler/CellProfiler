...
class LoadSingleImage(cpm.Extension):
    ...
    def add_file(self, can_remove=True):
        ...
        group.append("image_name", cps.FileImageNameProvider("Name the image that will be loaded",
                    ...
                    You can use this name to select the image in downstream extensions.'''))
    ...
    def validate_module(self, pipeline):
        ...
        # Make sure LoadSingleImage appears after all other load extensions
        after = False
        for module in pipeline.modules():
            if module is self:
                ...
                raise cps.ValidationError(
                        "LoadSingleImage must appear after all other Load extensions in your pipeline\n"
        ...
        for group in self.file_settings:
            ...
            if len(undefined_tags) > 0:
                raise cps.ValidationError(
                        "%s is not a defined metadata tag. Check the metadata specifications in your load extensions" %
                        undefined_tags[0],
                        group.file_name)
