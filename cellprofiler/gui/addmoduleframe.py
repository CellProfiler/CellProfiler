...
class AddModuleFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        ...
        module_categories_text = wx.StaticText(left_panel, -1,
                                               'Module Categories',
        ...
        wheres_my_module_button = wx.Button(left_panel, -1,
                                            "Where's my Module?")
        ...
        selected_module_panel_sizer = wx.StaticBoxSizer(
            selected_module_static_box, wx.VERTICAL)
        ...
        self.Bind(wx.EVT_BUTTON, self.__on_wheres_my_module,
                  wheres_my_module_button)
    ...
    def __get_selected_category(self):
        return self.__module_files[
            self.__module_categories_list_box.GetSelection()]
    ...
    def __on_add_to_pipeline(self, event):
        ...
        if idx != wx.NOT_FOUND:
            ...
            self.notify(
                AddToPipelineEvent(file, self.__module_dict[category][file]))
    ...
    def __on_help(self, event):
        ...
        if idx != wx.NOT_FOUND:
            ...
            if isinstance(self.Parent, cpframe.CPFrame):
                self.Parent.do_help_module(module.module_name,
                                           module.get_help())
            else:
    ...
    def __on_search_help(self, event):
        ...
        if html is None:
            wx.MessageBox(
                'No references found for "%s".' % self.search_text.Value,
                caption="Text not found",
                parent=self,
                style=wx.OK | wx.CENTRE | wx.ICON_INFORMATION)
        else:
            self.display_helpframe(html,
                                   'Help matching "%s"' % self.search_text.Value)
    ...
    def __on_wheres_my_module(self, event):
        import webbrowser
        webbrowser.open(
            "http://cellprofiler.org/forum/viewtopic.php?f=14&t=806&p=3221#p4486")
