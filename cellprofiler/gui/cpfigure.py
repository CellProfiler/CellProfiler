...
class CPFigureFrame(wx.Frame):
    ...
    def __init__(self, parent=None, id=-1, title="",
        ...
        super(CPFigureFrame, self).__init__(parent, id, title, pos, size, style,
                                            name)
    ...
    def create_menu(self, figure_help=FIGURE_HELP):
        ...
        if wx.Platform == '__WXMAC__':
            ...
            self.Bind(wx.EVT_MENU, lambda evt: close_all(self.Parent),
                      id=MENU_CLOSE_ALL)
    ...
    def clf(self):
        ...
        for (x, y) in self.event_bindings:
            [self.figure.canvas.mpl_disconnect(b) for b in
    ...
    def get_pixel_data_fields_for_status_bar(self, im, xi, yi):
        ...
        if im.ndim == 2:
            ...
            fields += ["Channel %d: %.4f" % (idx + 1, im[y, x, idx]) for idx in
                       range(im.shape[2])]
        return fields
    ...
    def get_imshow_menu(self, (x, y)):
        ...
        def show_hist(evt):
            ...
            fig = create_or_find(self, -1, new_title, subplots=(1, 1),
                                 name=new_title)
            fig.subplot_histogram(0, 0, self.images[(x, y)].flatten(), bins=200,
        ...
        if is_color_image(self.images[x, y]):
            submenu = wx.Menu()
            rgb_mask = match_rgbmask_to_image(params['rgb_mask'],
                                              self.images[x, y])
        ...
        self.Bind(wx.EVT_MENU, open_image_in_new_figure,
                  open_in_new_figure_item)
        ...
        self.Bind(wx.EVT_MENU, change_interpolation,
                  id=MENU_INTERPOLATION_NEAREST)
        self.Bind(wx.EVT_MENU, change_interpolation,
                  id=MENU_INTERPOLATION_BICUBIC)
        self.Bind(wx.EVT_MENU, change_interpolation,
                  id=MENU_INTERPOLATION_BILINEAR)
        self.Bind(wx.EVT_MENU,
    ...
    def subplot_imshow(self, x, y, image, title=None, clear=True, colormap=None,
        ...
        if cplabels is None:
            ...
            for i, d in enumerate(cplabels):
                ...
                if CPLD_OUTLINE_COLOR not in d:
                    if i == 0:
                        d[
                            CPLD_OUTLINE_COLOR] = cpprefs.get_primary_outline_color()
                    elif i == 1:
                        d[
                            CPLD_OUTLINE_COLOR] = cpprefs.get_secondary_outline_color()
                    elif i == 2:
                        d[
                            CPLD_OUTLINE_COLOR] = cpprefs.get_tertiary_outline_color()
        ...
        if colorbar and not is_color_image(image):
            ...
            if not subplot in self.colorbar:
                ...
                bar = subplot.figure.colorbar(colormap, cax, subplot,
                                              use_gridspec=False)
        ...
        for cplabels in kwargs['cplabels']:
            ...
            if cplabels[CPLD_MODE] == CPLDM_LINES:
                subplot.add_collection(CPOutlineArtist(
                    ...
                    colors=np.array(cplabels[CPLD_OUTLINE_COLOR],
                                    float) / 255.))
    ...
    def subplot_imshow_ijv(self, x, y, ijv, shape=None, title=None,
        ...
        if len(ijv) > 0:
            ...
            r, g, b, a = [
                coo_matrix((colors[ijv[:, 2], i], (ijv[:, 0], ijv[:, 1])),
                           shape=shape).toarray()
                for i in range(4)]
    ...
    def subplot_table(self, x, y, statistics,
        ...
        **kwargs - for backwards compatibility, old argument values
        ...
        if row_labels is not None:
            self.table += [[a] + list(b) for a, b in
                           zip(row_labels, statistics)]
        else:
    ...
    def draw_platemap(self):
        ...
        data = format_plate_data_as_array(plates_dict[platemap_plate],
                                          plate_type)
    ...
    def __init__(self, name, labels, *args, **kwargs):
        ...
        for l in labels:
            ...
            for off, count in zip(offs, counts):
                lines.append(
                    np.vstack((pts[off:off + count], pts[off:off + 1])))
...
class CPImageArtist(matplotlib.artist.Artist):
    ...
    def draw(self, renderer):
        ...
        #
        view_x0 = int(
            min(numcols - 1, max(0, self.axes.viewLim.x0 - self.filterrad)))
        view_x1 = int(
            min(numcols, max(0, self.axes.viewLim.x1 + self.filterrad)))
...
if __name__ == "__main__":
    ...
    f.subplot_imshow_grayscale(0, 0, img[:, :, 0], "1-channel grayscale",
                               sharex=f.subplot(0, 0), sharey=f.subplot(0, 0))
