...
class DirectoryView(object):
    ...
    def __init__(self, panel):
        ...
        """
        self.__image_extensions = ['bmp', 'cur', 'fts', 'fits', 'gif', 'hdf',
                                   'ico', 'jpg', 'jpeg', 'pbm', 'pcx', 'pgm',
                                   'png', 'pnm', 'ppm', 'ras', 'tif', 'tiff',
                                   'xwd', 'dib', 'c01', 'mat', 'fig', 'zvi']
        ...
        cellprofiler.preferences.add_image_directory_listener(
            self.__on_image_directory_changed)
        panel.Bind(wx.EVT_LISTBOX_DCLICK, self.__on_list_box_d_click,
                   self.__list_box)
    ...
    def refresh(self):
        try:
            ...
            logger.warning(
                "Warning: GUI not available during directoryview refresh\n")
            return
        try:
            files = [x
                     for x in os.listdir(
    ...
    def __on_list_box_d_click(self, event):
        ...
        if len(selections) > 0:
            ...
            selection = self.__list_box.GetItems()[
                self.__list_box.GetSelection()]
        filename = os.path.join(
            cellprofiler.preferences.get_default_image_directory(), selection)

        try:
            if os.path.splitext(selection)[1].lower() == '.cp':
                self.notify_pipeline_listeners(
                    LoadPipelineRequestEvent(filename))
            elif os.path.splitext(selection)[1].lower() == '.mat':
                # A matlab file might be an image or a pipeline
                handles = scipy.io.matlab.mio.loadmat(filename,
