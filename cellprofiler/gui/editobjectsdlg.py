...
class EditObjectsDialog(wx.Dialog):
    ...
    def display(self):
        ...
        if self.guide_image is not None and self.wants_image_display:
            image, _ = cellprofiler.object.size_similarly(
    ...
    def join_objects(self, event):
        all_labels = numpy.unique([v[self.K_LABEL] for v in self.artists.values()])
        if len(all_labels) < 2:
            return
    ...
    def delete_artist(self, artist):
        ...
        if not any([d[self.K_LABEL] == object_number for d in self.artists.values()]):
            self.remove_label(object_number)
            self.init_labels()
            self.display()
        else:
