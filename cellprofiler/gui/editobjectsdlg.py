...
class EditObjectsDialog(wx.Dialog):
    ...
    def init_labels(self):
        ...
        for label_and_touching in self.labels:
            ...
            for color in range(1, np.max(colored_labels) + 1):
                ...
                coords, offsets, counts = get_outline_pts(l,
                                                          np.arange(1, ct + 1))
    ...
    def on_context_menu(self, event):
        ...
        if isinstance(event, wx.MouseEvent):
            ...
            if location_event.inaxes == self.orig_axes:
                if self.get_mouse_event_object_number(
                        location_event) is not None:
                    event.Skip(False)
                    return
        ...
        for mid, state, help in (
                (self.ID_CONTRAST_RAW, self.SM_RAW,
                 "Display raw intensity image"),
        ...
        for mid, label, help in (
                (self.ID_LABELS_OUTLINES, "Outlines",
                 "Show the outlines of objects"),
                (self.ID_LABELS_FILL, "Fill",
                 "Show objects with a solid fill color")):
    ...
    def on_delete_click(self, event):
        if event.button == 1:
            if event.inaxes == self.orig_axes:
                ...
                logger.info(
                    "Starting delete at %f, %f" % self.delete_mode_start)
            else:
    ...
    def make_control_points(self, object_number):
        ...
        for polarity, structure in (
            ...
            coords, offsets, counts = get_outline_pts(labels,
                                                      sub_object_numbers)
