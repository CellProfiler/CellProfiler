...
base_dir_choices = ['Default Input Folder', 'Default Output Folder',
                    'Other Directory...']
...
match_modes = ['Common Substring', 'Shell pattern', 'Regular expression',
               'Numeric Position']
...
class LocationPanel(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        self.bottom_panel = bottom_panel = scrollable_text.ScrollableText(
            splitter, -1)
        splitter.SplitHorizontally(top_panel, bottom_panel)
        self.splitter.SashGravity = 0.0

        # Base directory
        base_dir, base_dir_sizer = labeled_thing(
        ...
        # descend?
        descend_dirs, descend_sizer = labeled_thing(
            "Descend into subdirectories?",
            wx.Choice(advanced, -1, choices=descend_dir_choices), advanced)
        self.descend_dirs = descend_dirs
        # descend force update
        self.descend_update_filelist = descend_update_filelist = wx.Button(
            advanced, -1, "Update file list...")
        # descent tree chooser...
        box = wx.StaticBox(advanced, -1,
                           "Choose directories to search for files...")
        self.dirtree, self.dirtree_boxsizer = dirtree, dirtree_boxsizer = boxed_thing(
            box, DirTree(advanced, self), flag=wx.EXPAND)

        # exclude some files?
        exclude, exclude_sizer = labeled_thing(
            "Exclude some files by substring in filename?",
            wx.CheckBox(advanced, -1, ""), advanced)
        self.exclude = exclude

        box = wx.StaticBox(advanced, -1, "Exclude substrings...")
        self.exclude_list, self.exclude_boxsizer = exclude_list, exclude_boxsizer = boxed_thing(
            box, wx.TextCtrl(advanced, -1, "", size=(300, 30),
        ...
        top_sizer.Add(otherdir_sizer, 0, wx.EXPAND | wx.LEFT | wx.RIGHT,
                      border=20)
        ...
        advanced_sizer.Add(dirtree_boxsizer, 1, wx.EXPAND | wx.LEFT | wx.RIGHT,
                           border=20)
    ...
    def change_basedir(self, evt):
        ...
        self.set_shown(self.top_sizer, self.otherdir_sizer,
                       (base_dir_choices[idx] == FS_OTHER_DIR))
        if os.path.isdir(self.get_current_directory()):
            self.dirtree.set_directory(self.get_current_directory())
            if descend_dir_choices[
                self.descend_dirs.GetSelection()] == FS_DESCEND_NO:
                self.update_file_list()
        self.Layout()
        self.Refresh()
    ...
    def update_list_display(self):
        self.bottom_panel.set_text(
            [self.format_file(f) for f in self.get_file_list()])
        ...
        if self.exclude.GetValue():
            for exclude_substring in self.exclude_list.GetValue().split("\n"):
                if exclude_substring:
                    temp_list = [(d, s, f) for (d, s, f) in temp_list if
                                 exclude_substring not in f]
        return temp_list

    def browse_otherdir(self, evt):
        default = self.otherdir.GetValue() or default_input
        dlg = wx.DirDialog(self, "Choose a directory:",
                           style=wx.DD_DEFAULT_STYLE, defaultPath=default)
    ...
    def change_descend(self, evt):
        ...
        self.set_shown(self.advanced_sizer, self.dirtree_boxsizer,
                       show=(descend_dir_choices[idx] == FS_DESCEND_CHOOSE))
        self.Layout()
        self.Refresh()

    def change_exclude(self, evt):
        self.set_shown(self.advanced_sizer, self.exclude_boxsizer,
                       show=self.exclude.GetValue())
...
class NamesPanel(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        imagebook.AddPage(
            ImagePage(imagebook, self.file_selector, default_image_name(0)),
            default_image_name(0))
    ...
    def page_changing(self, evt):
        ...
        if new == (imagebook.GetPageCount() - 1):
            newpage = ImagePage(imagebook, self.file_selector,
                                default_image_name(new))
...
class ImagePage(wx.Panel):
    def __init__(self, parent, file_selector, initial_name):
        ...
        # controls
        nlabel = wx.StaticText(self, -1,
                               "Name for this image in CellProfiler: ")
        self.name = name = wx.TextCtrl(self, -1, initial_name)

        mode, mode_sizer = labeled_thing("Identify channels by:",
                                         wx.Choice(self, -1,
                                                   choices=match_modes),
                                         parent=self)
        ...
        self.pattern_label = pattern_label = wx.StaticText(self, -1,
                                                           pattern_label_strings[
        ...
        self.matches_only = matches_only = wx.CheckBox(self, -1,
        ...
        pattern.Bind(wx.EVT_KEY_UP,
                     self.update_file_list)  # some editing keys don't cause EVT_TEXT (e.g., control-D)
    ...
    def format_file(self, file_info):
        ...
        if modestr == FS_SUBSTRING:
            ...
            if match:
                ...
                return prefix + [('black', file_string[:start]),
                                 ('red', file_string[start:end]),
    ...
    def format_file_list(self):
        flist = [self.format_file(f) for f in
                 self.file_selector.location.get_file_list()]
...
class DirTree(CT.CustomTreeCtrl):
    ...
    def expand(self, evt):
        ...
        if not already_visited:
            ...
            subdirs = [d for d in os.listdir(dirname) if
