...
class ModuleView:
            ...
            for selection in selections:
                ...
                if selection not in v.choices:
                    control.SetItemForegroundColour(
                            index, cellprofiler.preference.get_error_color())
            ...
            for i in range(len(choices)):
                if control.IsSelected(i):
                    ...
                    if choices[i] not in v.choices:
                        control.SetItemForegroundColour(
                                i, cellprofiler.preference.get_error_color())
        return control
    ...
    def make_colormap_control(self, v, control_name, control):
        ...
        try:
            if v.value == cellprofiler.setting.DEFAULT:
                cmap_name = cellprofiler.preference.get_default_colormap()
            else:
                cmap_name = v.value
    ...
    def make_callback_controls(self, v, control_name, control):
        ...
        if not control:
            ...
            control.BackgroundColour = cellprofiler.preference.get_background_color()
            ...
            def on_button_pressed(event, setting=v, control=text_ctrl):
                ...
                try:
                    if setting.get_example_fn is None:
                        path = cellprofiler.preference.get_default_image_directory()
                ...
                if v.guess == cellprofiler.setting.RegexpText.GUESS_FOLDER:
                    guess_file = cellprofiler.preference.get_pathname_re_guess_file()
                    guesses = regexp_editor.RE_FOLDER_GUESSES
                else:
                    guess_file = cellprofiler.preference.get_filename_re_guess_file()
    ...
    def make_directory_path_control(self, v, control_name, control):
        ...
        if v.dir_choice in (cellprofiler.setting.DEFAULT_INPUT_FOLDER_NAME,
            ...
            folder_label.Label = \
                "( %s )" % cellprofiler.preference.get_default_image_directory()
        elif v.dir_choice in (cellprofiler.setting.DEFAULT_OUTPUT_FOLDER_NAME,
                              cellprofiler.setting.DEFAULT_OUTPUT_SUBFOLDER_NAME):
            folder_label.Label = \
                "( %s )" % cellprofiler.preference.get_default_output_directory()
        else:
    ...
    def on_validation(self, setting_idx, message, level):
        ...
        default_bg_color = cellprofiler.preference.get_background_color()
                ...
                if static_text is not None:
                    ...
                    if setting is bad_setting:
                        if level == logging.ERROR:
                            desired_fg = cellprofiler.preference.get_error_color()
...
class FileCollectionDisplayController(object):
    ...
    def update(self):
        ...
        self.manage_expansion()
        cellprofiler.preference.report_progress(operation_id, 1, None)
...
class BinaryMatrixController(object):
    ...
    def on_matrix_ctrl_paint(self, event):
        ...
        paint_dc.Background = wx.Brush(cellprofiler.preference.get_background_color())
        paint_dc.Clear()
        pShadow = wx.Pen(
...
class TableController(wx.grid.PyGridTableBase):
    ...
    ERROR_ATTR.TextColour = cellprofiler.preference.get_error_color()
