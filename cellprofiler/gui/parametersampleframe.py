...
class ParameterSampleFrame(wx.Frame):
    ...
    def __init__(self, parent, module, pipeline, id=-1,
        ...
        wx.Frame.__init__(self, self.__frame, id, frame_title, pos, size, style,
                          name)
        ...
        for i in range(len(self.__parameters_to_widgets_list)):
            ...
            for j in range(len(self.__parameters_to_widgets_list[i])):
                # Checkbox & label
                if j == 0:
                    check_box = wx.CheckBox(self.__settings_scrolled_window, -1,
                                            label)
                ...
                if self.is_parameter_int(setting):
                    ...
                    upper_spin_ctrl.SetIncrement(
                        10 ** -FLOAT_DIGITS_TO_ROUND_TO)
                    upper_spin_ctrl.SetRange(FLOAT_LOWER_BOUND,
                                             FLOAT_UPPER_BOUND)
                    upper_spin_ctrl.Bind(fs.EVT_FLOATSPIN,
            ...
            if checkbox.IsChecked():
                ...
                if self.get_parameter_input_size(setting) == 1:
                    ...
                    upper_bound = self.__upper_bound_spin_ctrl_list[
                        index].GetValue()
                    number_samples = self.__number_spin_ctrl_list[
                        index].GetValue()
        ...
        for i, setting in enumerate(self.__module.visible_settings()):
            ...
            if setting.get_text() != '':
                ...
                if isinstance(setting, settings.ImageNameProvider):
                    ...
                    # ~^~
            if i < len(
                    self.__module.visible_settings()) - 1 and value_to_write != '':
                value_to_write += '\t'
    ...
    def on_lower_spin_ctrl(self, event):
        if self.__initialized:
            for i in range(len(self.__parameters_to_widgets_list)):
                for j in self.__parameters_to_widgets_list[i]:
                    if event.GetId() == self.__lower_bound_spin_ctrl_list[
                        j].GetId():
                        lower_bound = self.__lower_bound_spin_ctrl_list[
                            j].GetValue()
                        upper_bound = self.__upper_bound_spin_ctrl_list[
                            j].GetValue()
                        increment = self.__number_spin_ctrl_list[
                                        j].GetValue() - 1
    ...
    def on_upper_spin_ctrl(self, event):
        if self.__initialized:
            for i in range(len(self.__parameters_to_widgets_list)):
                for j in self.__parameters_to_widgets_list[i]:
                    if event.GetId() == self.__upper_bound_spin_ctrl_list[
                        j].GetId():
                        lower_bound = self.__lower_bound_spin_ctrl_list[
                            j].GetValue()
                        upper_bound = self.__upper_bound_spin_ctrl_list[
            ...
            for i in range(len(self.__parameters_to_widgets_list)):
                for j in self.__parameters_to_widgets_list[i]:
                    if event.GetId() == self.__lower_bound_spin_ctrl_list[
                        ...
                        upper_bound = self.__upper_bound_spin_ctrl_list[
                            j].GetValue()
                        increment = self.__upper_bound_spin_ctrl_list[
                            j].GetIncrement()
                        if upper_bound < lower_bound + increment:
                            self.__upper_bound_spin_ctrl_list[j].SetValue(
                                lower_bound + increment)
    ...
    def on_number_spin_ctrl(self, event):
        if self.__initialized:
            for i in range(len(self.__parameters_to_widgets_list)):
                ...
                if self.is_parameter_int(setting):
                    for j in self.__parameters_to_widgets_list[i]:
                        if event.GetId() == self.__number_spin_ctrl_list[
                            j].GetId():
                            lower_bound = self.__lower_bound_spin_ctrl_list[
                                j].GetValue()
                            upper_bound = self.__upper_bound_spin_ctrl_list[
