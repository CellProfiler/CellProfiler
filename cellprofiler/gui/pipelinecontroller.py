...
RECENT_PIPELINE_FILE_MENU_ID = [wx.NewId() for i in range(cellprofiler.preference.RECENT_FILE_COUNT)]
RECENT_WORKSPACE_FILE_MENU_ID = [wx.NewId() for i in range(cellprofiler.preference.RECENT_FILE_COUNT)]
...
class PipelineController:
    ...
    def __init__(self, workspace, frame):
        ...
        self.pipeline_list = []
        cellprofiler.preference.add_image_directory_listener(self.__on_image_directory_change)
        cellprofiler.preference.add_output_directory_listener(self.__on_output_directory_change)
    ...
    def start(self, workspace_file, pipeline_path):
        ...
        cellprofiler.preference.clear_image_set_file()
    ...
    def attach_to_test_controls_panel(self, panel):
        ...
        panel.
        """
        bkgnd_color = cellprofiler.preference.get_background_color()
    ...
    def __on_revert_workspace(self, event):
        path = cellprofiler.preference.get_current_workspace_path()
        if path is not None:
            self.do_open_workspace(path, True)
    ...
    def do_open_workspace_dlg(self):
        ...
        wildcard = "CellProfiler project (%s)|%s|All files (*.*)|*.*" % (
            ",".join(["*.%s" % x for x in cellprofiler.preference.EXT_PROJECT_CHOICES]),
            ";".join(["*.%s" % x for x in cellprofiler.preference.EXT_PROJECT_CHOICES]))
        with wx.FileDialog(
                self.__frame,
    ...
    def do_open_workspace(self, filename, load_pipeline=True):
        ...
        with wx.ProgressDialog(
            ...
            try:
                ...
                cellprofiler.preference.add_progress_callback(progress_callback)
                progress_callback_fn = progress_callback

                self.__workspace.load(filename, load_pipeline)
                cellprofiler.preference.set_workspace_file(filename)
                cellprofiler.preference.set_current_workspace_path(filename)
                ...
                cellprofiler.preference.remove_progress_callback(progress_callback_fn)
    ...
    def do_create_workspace(self):
        ...
        self.__workspace.save_pipeline_to_measurements()
        self.__dirty_workspace = False
        cellprofiler.preference.set_current_workspace_path(None)
    ...
    def do_save_as_workspace(self):
        wildcard = "CellProfiler project (*.%s)|*.%s" % (
            cellprofiler.preference.EXT_PROJECT, cellprofiler.preference.EXT_PROJECT)
        with wx.FileDialog(
                self.__frame,
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                fullname = dlg.Path
                dot_cpproj_ext = "." + cellprofiler.preference.EXT_PROJECT
                if sys.platform == "darwin" and not filename.endswith(dot_cpproj_ext):
                    fullname += dot_cpproj_ext
                self.do_save_workspace(fullname)
                cellprofiler.preference.set_current_workspace_path(fullname)
                cellprofiler.preference.set_workspace_file(fullname)
                self.set_title()
                return True
            return False
    ...
    def do_save_workspace(self, filename):
        ...
        self.__dirty_workspace = False
        self.set_title()
        others = cellprofiler.preference.get_save_pipeline_with_project()
        if others in (cellprofiler.preference.SPP_PIPELINE_ONLY,
                      cellprofiler.preference.SPP_PIPELINE_AND_FILE_LIST):
            pipeline_path = \
                os.path.splitext(filename)[0] + "." + cellprofiler.preference.EXT_PIPELINE
            self.__pipeline.save(pipeline_path,
                                 save_image_plane_details=False)
        if others in (cellprofiler.preference.SPP_FILE_LIST_ONLY,
                      cellprofiler.preference.SPP_PIPELINE_AND_FILE_LIST):
    ...
    def __on_load_pipeline(self, event):
        wildcard = "CellProfiler pipeline (%s)|%s" % (
            ",".join([".%s" % x for x in cellprofiler.preference.EXT_PIPELINE_CHOICES]),
            ";".join(["*.%s" % x for x in cellprofiler.preference.EXT_PIPELINE_CHOICES]))
    ...
    def do_load_pipeline(self, pathname):
        ...
        try:
            ...
            if self.__pipeline.can_convert_legacy_input_modules():
                ...
                with wx.Dialog(self.__frame,
                    ...
                    dir_ctrl = filebrowse.DirBrowseButton(
                            ...
                            startDirectory=cellprofiler.preference.get_default_image_directory())
                    dir_ctrl.SetValue(cellprofiler.preference.get_default_image_directory())
            ...
            self.display_pipeline_message_for_user()
            target_project_path = \
                os.path.splitext(pathname)[0] + "." + cellprofiler.preference.EXT_PROJECT
            if not os.path.exists(target_project_path) and \
                            cellprofiler.preference.get_current_workspace_path() is None:
                cellprofiler.preference.set_current_workspace_path(target_project_path)
                self.set_title()
    ...
    def do_save_pipeline(self):
        ...
        """
        default_filename = cellprofiler.preference.get_current_workspace_path()
        if default_filename is None:
            default_filename = "pipeline.%s" % cellprofiler.preference.EXT_PIPELINE
            default_path = None
        else:
            default_path, default_filename = os.path.split(default_filename)
            default_filename = \
                os.path.splitext(default_filename)[0] + "." + cellprofiler.preference.EXT_PIPELINE
        wildcard = ("CellProfiler pipeline (*.%s)|*.%s|"
                    "CellProfiler pipeline and file list (*.%s)|*.%s") % (
                       cellprofiler.preference.EXT_PIPELINE, cellprofiler.preference.EXT_PIPELINE,
                       cellprofiler.preference.EXT_PIPELINE, cellprofiler.preference.EXT_PIPELINE)
        with wx.FileDialog(self.__frame,
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if not sys.platform.startswith("win"):
                    dot_ext_pipeline = "." + cellprofiler.preference.EXT_PIPELINE
    ...
    def set_current_pipeline_path(self, pathname):
        cellprofiler.preference.set_current_pipeline_path(pathname)
        cellprofiler.preference.add_recent_file(pathname)
        self.populate_recent_files()
    ...
    def populate_recent_files(self):
        ...
        for menu, ids, file_names, fn in (
                (self.__frame.recent_pipeline_files,
                 ...
                 cellprofiler.preference.get_recent_files(),
                 self.do_load_pipeline),
                (self.__frame.recent_workspace_files,
                 RECENT_WORKSPACE_FILE_MENU_ID,
                 cellprofiler.preference.get_recent_files(cellprofiler.preference.WORKSPACE_FILE),
    ...
    def set_title(self):
        ...
        pathname = cellprofiler.preference.get_current_workspace_path()
        if pathname is None:
    ...
    def check_close(self):
        ...
        if self.__dirty_workspace:
            ...
            try:
                dialog.ShowModal()
                if answer[0] == SAVE_ID:
                    workspace_path = cellprofiler.preference.get_current_workspace_path()
    ...
    def on_pathlist_show(self, event=None):
        ...
        if ext.lower() == ".mat":
            ...
            try:
                ...
                pass
        if len(ext) > 1 and ext[1:] in cellprofiler.preference.EXT_PROJECT_CHOICES:
            result = wx.MessageBox(
        ...
        if len(ext) > 1 and ext[1:] in cellprofiler.preference.EXT_PIPELINE_CHOICES:
            result = wx.MessageBox(
    ...
    def exit_test_mode(self):
        ...
        if self.is_in_debug_mode():
            self.stop_debugging()
            if cellprofiler.preference.get_show_exiting_test_mode_dlg():
    ...
    def run_next_pipeline(self, event):
        ...
        cellprofiler.preference.set_default_image_directory(pipeline_details.default_input_folder)
        cellprofiler.preference.set_default_output_directory(pipeline_details.default_output_folder)
        cellprofiler.preference.set_output_file_name(pipeline_details.measurements_file)
    ...
    def do_analyze_images(self):
        ...
        try:
            ...
            if cellprofiler.preference.get_write_MAT_files() == cellprofiler.preference.WRITE_HDF5:
                measurements_file_path = self.get_output_file_path()

            num_workers = min(
                    len(self.__workspace.measurements.get_image_numbers()),
                    cellprofiler.preference.get_max_workers())
    ...
    def on_stop_analysis(self, event):
        ...
        """
        try:
            if cellprofiler.preference.get_write_MAT_files() is True:
            ...
            self.stop_running()
            if cellprofiler.preference.get_show_analysis_complete_dlg():
    ...
    def on_debug_choose_image_set(self, event):
        ...
        def feature_cmp(x, y):
            ...
            # Put metadata first.
            #
            file_md_order = (cellprofiler.modules.loadimages.C_FILE_NAME, cellprofiler.modules.loadimages.C_PATH_NAME, cellprofiler.modules.loadimages.C_FRAME)
        ...
        features = sorted(
                [f for f in m.get_feature_names(cellprofiler.measurement.IMAGE) if f.split("_")[0] in
                 (cellprofiler.measurement.C_METADATA, cellprofiler.modules.loadimages.C_FILE_NAME, cellprofiler.modules.loadimages.C_PATH_NAME, cellprofiler.modules.loadimages.C_FRAME)],
                cmp=feature_cmp)
        ...
        class ChooseImageSetDialog(wx.Dialog, wx.lib.mixins.listctrl.ColumnSorterMixin):
            def __init__(self, parent):
                dlg_size = cellprofiler.preference.get_choose_image_set_frame_size()
                if dlg_size is None:
            ...
            def on_size(event):
                ...
                cellprofiler.preference.set_choose_image_set_frame_size(
    ...
    def show_analysis_complete(self, n_image_sets):
        ...
        if sys.platform in ('darwin', 'win32'):
            ...
            def on_open_default_output_folder(event):
                ...
                if sys.platform == 'darwin':
                    subprocess.call([
                        "open", cellprofiler.preference.get_default_output_directory()])
                elif sys.platform == 'win32':
                    subprocess.call([
                        "cmd", "/C", "start", "explorer",
                        cellprofiler.preference.get_default_output_directory()])
        ...
        def on_save_workspace(event):
            ...
            wx.MessageBox(
                    "Saved project %s" % cellprofiler.preference.get_current_workspace_path(),
        ...
        try:
            ...
            if dont_show_again.Value:
                cellprofiler.preference.set_show_analysis_complete_dlg(False)
        finally:
            dlg.Destroy()
    ...
    def show_exiting_test_mode(self):
        ...
        try:
            dlg.ShowModal()
            if dont_show_again.Value:
                cellprofiler.preference.set_show_exiting_test_mode_dlg(False)
        finally:
            dlg.Destroy()

    def get_output_file_path(self):
        path = os.path.join(cellprofiler.preference.get_default_output_directory(),
                            cellprofiler.preference.get_output_file_name())
        if os.path.exists(path) and not cellprofiler.preference.get_allow_output_file_overwrite():
            ...
            if user_choice & wx.YES:
                path = alternate_name
                cellprofiler.preference.set_output_file_name(os.path.split(alternate_name)[1])
            else:
                return None
        return path
