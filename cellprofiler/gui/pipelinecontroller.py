...
RECENT_PIPELINE_FILE_MENU_ID = [wx.NewId() for i in
                                range(cpprefs.RECENT_FILE_COUNT)]
RECENT_WORKSPACE_FILE_MENU_ID = [wx.NewId() for i in
                                 range(cpprefs.RECENT_FILE_COUNT)]
...
class PipelineController:
    ...
    def __init__(self, workspace, frame):
        ...
        wx.EVT_MENU(frame, cpframe.ID_FILE_URL_LOAD_PIPELINE,
                    self.__on_url_load_pipeline)
        wx.EVT_MENU(frame, cpframe.ID_FILE_IMPORT_FILE_LIST,
                    self.__on_import_file_list)
        wx.EVT_MENU(frame, cpframe.ID_FILE_SAVE_PIPELINE,
        ...
        frame.Bind(wx.EVT_UPDATE_UI, self.on_update_undo_ui,
                   id=cpframe.ID_EDIT_UNDO)
        ...
        wx.EVT_MENU(frame, cpframe.ID_DEBUG_NEXT_IMAGE_SET,
                    self.on_debug_next_image_set)
        wx.EVT_MENU(frame, cpframe.ID_DEBUG_NEXT_GROUP,
                    self.on_debug_next_group)
        wx.EVT_MENU(frame, cpframe.ID_DEBUG_CHOOSE_GROUP,
                    self.on_debug_choose_group)
        wx.EVT_MENU(frame, cpframe.ID_DEBUG_CHOOSE_IMAGE_SET,
                    self.on_debug_choose_image_set)
        wx.EVT_MENU(frame, cpframe.ID_DEBUG_CHOOSE_RANDOM_IMAGE_SET,
        ...
        wx.EVT_MENU(frame, cpframe.ID_WINDOW_SHOW_ALL_WINDOWS,
                    self.on_show_all_windows)
        wx.EVT_MENU(frame, cpframe.ID_WINDOW_HIDE_ALL_WINDOWS,
    ...
    def attach_to_path_list_ctrl(self,
        ...
        it changes.
        :param path_list_filtered_files_checkbox:
        :param path_list_ctrl:
        """
    ...
    def attach_to_module_controls_panel(self, module_controls_panel):
        ...
        self.__mcp_text = wx.StaticText(self.__module_controls_panel, -1,
                                        "Adjust modules:")
        self.__mcp_add_module_button = wx.Button(self.__module_controls_panel,
                                                 -1, "+", (0, 0), (30, -1))
        ...
        self.__mcp_remove_module_button = wx.Button(
            ...
            "-", (0, 0), (30, -1))
        self.__mcp_remove_module_button.SetToolTipString(
        ...
        self.__mcp_module_down_button = wx.Button(
            ...
            "v", (0, 0), (30, -1))
        self.__mcp_module_down_button.SetToolTipString(
        ...
        mcp_sizer.AddMany([(self.__help_button, 0, wx.ALIGN_CENTER | wx.ALL, 3),
                           ...
                           (self.__mcp_add_module_button, 0,
                            wx.ALIGN_CENTER | wx.ALL, 3),
                           (self.__mcp_remove_module_button, 0,
                            wx.ALIGN_CENTER | wx.ALL, 3),
                           (self.__mcp_module_up_button, 0,
                            wx.ALIGN_CENTER | wx.ALL, 3),
                           (self.__mcp_module_down_button, 0,
                            wx.ALIGN_CENTER | wx.ALL, 3)])
        self.__module_controls_panel.SetSizer(mcp_sizer)
        self.__module_controls_panel.Bind(wx.EVT_BUTTON, self.__on_add_module,
                                          self.__mcp_add_module_button)
        self.__module_controls_panel.Bind(wx.EVT_BUTTON, self.on_remove_module,
                                          self.__mcp_remove_module_button)
        self.__module_controls_panel.Bind(wx.EVT_BUTTON, self.on_module_up,
                                          self.__mcp_module_up_button)
        self.__module_controls_panel.Bind(wx.EVT_BUTTON, self.on_module_down,
    ...
    def attach_to_test_controls_panel(self, panel):
        ...
        self.__tcp_next_imageset.SetToolTip(
            wx.ToolTip("Jump to next image set"))
        self.__tcp_next_imageset.Bind(wx.EVT_BUTTON,
                                      self.on_debug_next_image_set)
    ...
    def do_save_as_workspace(self):
        ...
        with wx.FileDialog(
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if sys.platform == "darwin" and not filename.endswith(
                        dot_cpproj_ext):
        ...
        try:
            ...
            if self.__pipeline.can_convert_legacy_input_modules():
                ...
                with wx.Dialog(self.__frame,
                    ...
                    dont_convert_button.Bind(wx.EVT_BUTTON,
                                             on_dont_convert_pressed)
                    dlg.Fit()
    ...
    def do_save_pipeline(self):
        ...
        if default_filename is None:
            ...
            default_filename = \
                os.path.splitext(default_filename)[
                    0] + "." + cpprefs.EXT_PIPELINE
        wildcard = ("CellProfiler pipeline (*.%s)|*.%s|"
    ...
    def display_plate_viewer_help(self, message, caption):
        ...
        with wx.Dialog(self.__frame, title=caption) as dlg:
            ...
            bmpInfo = wx.ArtProvider.GetBitmap(wx.ART_INFORMATION,
                                               wx.ART_CMN_DIALOG)
    ...
    def check_close(self):
        ...
        if self.__dirty_workspace:
            ...
            text = wx.StaticText(dialog,
                                 label="Do you want to save your project?")
            sizer.Add(text, 0, wx.EXPAND | wx.ALL, 5)
            super_sizer.Add(wx.StaticLine(dialog), 0,
                            wx.EXPAND | wx.LEFT | wx.RIGHT, 20)
            #
            # These are the buttons
            #
            button_sizer = wx.BoxSizer(wx.HORIZONTAL)
            super_sizer.Add(button_sizer, 0,
    ...
    def on_pathlist_show(self, event=None):
        ...
        if len(paths) == 0:
            wx.MessageBox("No image selected.", caption="No image selected",
                          parent=self.__frame)
            return
    ...
    def enable_module_controls_panel_buttons(self):
        ...
        for menu_id, control, state in (
                (cpframe.ID_EDIT_MOVE_DOWN, self.__mcp_module_down_button,
                 enable_down),
                (cpframe.ID_EDIT_MOVE_UP, self.__mcp_module_up_button,
                 enable_up),
                (cpframe.ID_EDIT_DELETE, self.__mcp_remove_module_button,
                 enable_delete),
        ...
        if active_module is None:
            ...
            for module in self.__pipeline.modules(False):
                if module.is_input_module():
                    ...
                    break
            module_num = max(active_module.module_num,
                             last_input_module_num) + 1
    ...
    def analysis_event_handler(self, evt):
        ...
        if isinstance(evt, cpanalysis.AnalysisStarted):
            ...
            wx.CallAfter(self.handle_analysis_feedback)
                        ...
                        (cpanalysis.DebugWaiting, cpanalysis.DebugComplete)):
    ...
    def module_display_request(self, evt):
        ...
        try:
            module = self.__pipeline.modules(exclude_disabled=False)[
                module_num - 1]
            ...
            display_error_dialog(None, exc, self.__pipeline, tb=tb,
                                 continue_only=True,
    ...
    def module_interaction_request(self, evt):
        ...
        kwargs = dict((name, evt.__dict__['kwarg_%s' % name]) for name in
                      evt.kwargs_names)
        result = ""
        try:
    ...
    def on_stop_analysis(self, event):
        ...
        try:
            ...
            status = m[cpm.IMAGE, cpanalysis.AnalysisRunner.STATUS,
                       m.get_image_numbers()]
            n_image_sets = sum([
                                   x == cpanalysis.AnalysisRunner.STATUS_DONE
                                   for x in status])
            self.stop_running()
        ...
        try:
            ...
            if workspace.disposition == cpw.DISPOSITION_SKIP:
                self.last_debug_module()
                      ...
                      select_next_module):
                self.__pipeline_list_view.select_one_module(
                    module.module_num + 1)
            failure = 0
    ...
    def on_debug_random_image_set(self, event):
        group_index = 0 if len(self.__groupings) == 1 else np.random.randint(0,
                                                                             len(
                                                                                 self.__groupings) - 1,
                                                                             size=1)
    ...
    def debug_choose_group(self, index):
        ...
        self.__pipeline.prepare_group(workspace,
                                      self.__groupings[self.__grouping_index][
                                          0],
                                      self.__groupings[self.__grouping_index][
                                          1])
    ...
    def on_debug_choose_group(self, event):
        ...
        dialog.SetSizer(super_sizer)
        super_sizer.Add(
            wx.StaticText(dialog, label="Select a group set for testing:"), 0,
            wx.EXPAND | wx.ALL, 5)
        choices = []
    ...
    def on_debug_choose_image_set(self, event):
        ...
        features = sorted(
            ...
            cmp=feature_cmp)
        image_numbers = np.array(self.__groupings[self.__grouping_index][1],
                                 int)
        columns = dict([
                           (f, m[cpm.IMAGE, f, image_numbers]) for f in
                           features])
        choices = {}
        ...
        class ChooseImageSetDialog(wx.Dialog, ColumnSorterMixin):
            def __init__(self, parent):
                ...
                self.list_ctrl.SetMinSize(
                    wx.Size(min(total_width, 640),
                            self.list_ctrl.GetMinHeight()))
                self.itemDataMap = dict([
                                            (k,
                                             [u"%06d" % v if isinstance(v,
                                                                        int) else
                                              u"%020.10f" % v if isinstance(v,
                                                                            float) else
                                              unicode(v) for v in
                                              [k] + choices[k]]) for k in
                                            choices])
        ...
        if self.__module_view is not None:
            if self.__module_view.get_current_module() is not None:
                self.show_parameter_sample_options(
                    self.__module_view.get_current_module().get_module_num(),
                    event)
            else:
    ...
    def get_output_file_path(self):
        ...
        if os.path.exists(
                path) and not cpprefs.get_allow_output_file_overwrite():
