...
class PreferencesView:
    ...
    def close(self):
        cpprefs.remove_output_file_name_listener(
    ...
    def __make_folder_panel(self, panel, value, list_fn, text, help_text,
        ...
        if refresh_action is not None:
            ...
            refresh_button.SetToolTipString(
                "Refresh the Default Input Folder list")
        ...
        def on_edit_box_change(event):
            if os.path.isdir(edit_box.Value):
                ...
                new_button.Enable()
                new_button.SetToolTipString(
                    "Press button to create the %s folder" %
                    edit_box.Value)
            self.__on_edit_box_change(event, edit_box, text, actions)
            event.Skip()

        panel.Bind(wx.EVT_BUTTON,
                   lambda event: self.__on_help(event, help_text),
                   help_button)
        panel.Bind(wx.EVT_BUTTON,
    ...
    def __on_browse(self, event, edit_box, text):
        dir_dialog = wx.DirDialog(self.__panel, string.capitalize(text),
                                  edit_box.GetValue())
...
class ProgressWatcher:
    ...
    def remaining_time(self):
        ...
        if self.end_times is None:
            ...
            per_module_estimates[:module_index] = np.median(
                durations[:self.image_set_index + 1, :module_index], 0)
            current_module_so_far = self.elapsed_time() - self.end_times[
                1 + index - 1]
            if self.image_set_index > 0:
                ...
                per_module_estimates[module_index:] = per_module_estimates[
                                                      :module_index + 1].max()
            per_module_estimates[
            :module_index] *= self.num_image_sets - self.image_set_index - 1
            per_module_estimates[
            module_index:] *= self.num_image_sets - self.image_set_index
            per_module_estimates[module_index] -= current_module_so_far
    ...
    def remaining_time_multiprocessing(self):
        ...
        if (self.end_times is None) or (self.num_received == 0):
            ...
            expected_per_job = self.end_times[
                                   self.num_received] / self.num_received
