...
class RunMultplePipelinesDialog(wx.Dialog):
    ...
    def add_file_chooser(self, sizer):
        ...
        self.file_chooser.SetColumnWidth(
            FC_MODULE_COUNT_COLUMN, wx.LIST_AUTOSIZE_USEHEADER
        )
    ...
    def add_directory_picker(self, sizer):
        ...
        sub_sizer.Add(
            wx.StaticText(self, label="Folder:"), 0, wx.ALIGN_RIGHT | wx.RIGHT, 3
        )
    ...
    def add_pipeline_list_box(self, sizer):
        ...
        self.pipeline_list_view.InsertColumn(
            P_INPUT_DIRECTORY_COLUMN, "Default input folder"
        )
        self.pipeline_list_view.InsertColumn(
            P_OUTPUT_DIRECTORY_COLUMN, "Default output folder"
        )
        self.pipeline_list_view.InsertColumn(P_OUTPUT_FILE_COLUMN, "Measurements file")
    ...
    def hookup_events(self):
        ...
        self.Bind(
            wx.EVT_TEXT, self.on_path_changed, self.directory_picker.GetTextCtrl()
        )
    ...
    def set_file_chooser_path(self, path):
        ...
        for file_name in file_names:
            ...
            if module_count[0] is not None:
                index = self.file_chooser.InsertStringItem(sys.maxsize, file_name)
                self.file_chooser.SetStringItem(index, FC_DATE_COLUMN, mtime)
                self.file_chooser.SetStringItem(
                    index, FC_MODULE_COUNT_COLUMN, str(module_count[0])
                )
    ...
    def on_add(self, event):
        for i in range(self.file_chooser.GetItemCount()):
            if self.file_chooser.IsSelected(i):
                path = os.path.join(
                    self.directory_picker.GetPath(), self.file_chooser.GetItemText(i)
                )
                index = self.pipeline_list_view.InsertStringItem(sys.maxsize, path)
        ...
        self.pipeline_list_view.SetColumnWidth(
            P_INPUT_DIRECTORY_COLUMN, wx.LIST_AUTOSIZE
        )
        self.pipeline_list_view.SetColumnWidth(
            P_OUTPUT_DIRECTORY_COLUMN, wx.LIST_AUTOSIZE
        )
    ...
    def on_pipeline_list_down(self, event):
        ...
        if 0 <= item < self.pipeline_list_view.GetItemCount() and (
            ...
            if subitem == P_REMOVE_BUTTON_COLUMN:
                ...
                dlg.SetWildcard(
                    "CellProfiler pipeline (*.cp)|*.cp|Measurements file (*.mat)|*.mat"
                )
                ...
                dlg = wx.DirDialog(
                    self, "Choose the default %s folder to use for the pipeline"
                )
                ...
                dlg = wx.FileDialog(
                    self,
                    "Choose an output measurements file",
    ...
    def on_help(self, event):
        ...
        content = cellprofiler.gui.help.content.read_content(
            "other_multiple_pipelines.rst"
        )
        with cellprofiler.gui.htmldialog.HTMLDialog(
            self,
