...
class RunMultplePipelinesDialog(wx.Dialog):
    ...
    def add_file_chooser(self, sizer):
        ...
        self.file_chooser.SetColumnWidth(FC_MODULE_COUNT_COLUMN,
    ...
    def add_pipeline_list_box(self, sizer):
        ...
        self.pipeline_list_view.SetImageList(self.plv_image_list,
                                             wx.IMAGE_LIST_SMALL)
    ...
    def hookup_events(self):
        self.Bind(wx.EVT_DIRPICKER_CHANGED, self.on_path_changed,
                  self.directory_picker)
        self.Bind(wx.EVT_TEXT, self.on_path_changed,
                  self.directory_picker.TextCtrl)
        self.Bind(wx.EVT_BUTTON, self.on_add, self.file_add_button)
        self.Bind(wx.EVT_BUTTON, self.on_select_all,
                  self.file_select_all_button)
        self.Bind(wx.EVT_BUTTON, self.on_deselect_all,
                  self.file_deselect_all_button)
        self.Bind(wx.EVT_BUTTON, self.on_help, id=wx.ID_HELP)
        self.pipeline_list_view.Bind(wx.EVT_LEFT_DOWN,
                                     self.on_pipeline_list_down)
    ...
    def on_pipeline_list_down(self, event):
        ...
        if sys.platform.startswith("win"):
            ...
            widths = [self.pipeline_list_view.GetColumnWidth(i) for i in
                      range(4)]
            start = 0
        ...
        if (0 <= item < self.pipeline_list_view.ItemCount and
            ...
            if subitem == P_REMOVE_BUTTON_COLUMN:
                ...
                dlg = wx.FileDialog(self, "Choose a pipeline file",
                                    style=wx.FD_OPEN)
            ...
            P_INPUT_DIRECTORY_COLUMN, P_OUTPUT_DIRECTORY_COLUMN):
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    self.pipeline_list_view.SetStringItem(item, subitem,
                                                          dlg.Path)
    ...
    def get_pipelines(self):
        ...
        class PipelineDetails(object):
            def __init__(self, path, default_input_folder,
