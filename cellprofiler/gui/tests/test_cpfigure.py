...
class TestCPFigure(unittest.TestCase):
    ...
    def test_01_05_imshow_log_normalized(self):
        ...
        normed = (np.log(image.clip(min, max)) - np.log(min)) / (
    ...
    def test_02_01_show_pixel_data(self):
        ...
        assert expected in [str(f) for f in
                            my_frame.status_bar.GetFields()], 'Did not find "%s" in StatusBar fields' % (
        expected)
        my_frame.Destroy()
    ...
    def test_02_02_show_pixel_data_rgb(self):
        ...
        for field in expected:
            assert field in [str(f) for f in
                             my_frame.status_bar.GetFields()], 'Did not find "%s" in StatusBar fields' % (
            field)
        my_frame.Destroy()
    ...
    def test_03_01_menu_order(self):
        ...
        assert len(
            f.menu_subplots.MenuItems) == 0, 'Subplot menus should be empty after clf().'
