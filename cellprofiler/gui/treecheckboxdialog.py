...
class TreeCheckboxDialog(wx.Dialog):
        ...
        try:
            ...
            if self.tree_ctrl.GetChildrenCount(item_id, False) == 0:
                for key in sorted([x for x in d.keys() if x is not None]):
                    ...
                    sub_id = self.tree_ctrl.AppendItem(item_id, key,
                                                       image_index,
                                                       selected_index,
                                                       wx.TreeItemData(d1))
    ...
    def set_parent_state(self, item_id):
        if item_id != self.root_id:
            ...
            if d_parent[None] is not state:
                ...
                self.tree_ctrl.SetItemImage(parent_id, image_index,
                                            wx.TreeItemIcon_Normal)
                self.tree_ctrl.SetItemImage(parent_id, selected_index,
                                            wx.TreeItemIcon_Selected)
                self.tree_ctrl.SetItemImage(parent_id, image_index,
                                            wx.TreeItemIcon_Expanded)
                self.tree_ctrl.SetItemImage(parent_id, selected_index,
    ...
    def set_item_state(self, item_id, state):
        ...
        self.tree_ctrl.SetItemImage(item_id, image_index,
                                    wx.TreeItemIcon_Normal)
        self.tree_ctrl.SetItemImage(item_id, selected_index,
                                    wx.TreeItemIcon_Selected)
        self.tree_ctrl.SetItemImage(item_id, image_index,
                                    wx.TreeItemIcon_Expanded)
        self.tree_ctrl.SetItemImage(item_id, selected_index,
