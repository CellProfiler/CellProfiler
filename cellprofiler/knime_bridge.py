...
class KnimeBridgeServer(threading.Thread):
    ...
    def run_request(self, session_id, message_type, message):
        ...
        if pipeline is None:
            return
        m[cellprofiler.measurements.IMAGE, cellprofiler.measurements.GROUP_NUMBER] = 1
        m[cellprofiler.measurements.IMAGE, cellprofiler.measurements.GROUP_INDEX] = 1
        input_modules, other_modules = self.split_pipeline(pipeline)
    ...
    def run_group_request(self, session_id, message_type, message):
        ...
        for image_number in image_numbers:
            m[cellprofiler.measurements.IMAGE, cellprofiler.measurements.GROUP_NUMBER, image_number] = 1
            m[cellprofiler.measurements.IMAGE, cellprofiler.measurements.GROUP_INDEX, image_number] = image_number
        input_modules, other_modules = self.split_pipeline(pipeline)
        workspace = cellprofiler.workspace.Workspace(
        ...
        for object_name, features in feature_dict.items():
            ...
            sf = []
            string_features.append((object_name, sf))
            if object_name == cellprofiler.measurements.IMAGE:
                object_counts = [] * n_image_sets
            else:
                object_numbers = m[object_name, cellprofiler.measurements.OBJECT_NUMBER,
                                   image_numbers]
            ...
            for feature, data_type in features:
                ...
                if type_names[data_type] == 'java.lang.Double':
                    ...
                    if len(data) > 0:
                        int_data.append(data.astype('<i4'))
        data = numpy.hstack([
                                numpy.frombuffer(numpy.ascontiguousarray(numpy.hstack(ditem)).data, numpy.uint8)
    ...
    def find_measurements(self, modules, pipeline):
        ...
        for module in modules:
            ...
            for column in module.get_measurement_columns(pipeline):
                ...
                if objects == cellprofiler.measurements.EXPERIMENT and \
                                qualifiers.get(cellprofiler.measurements.MCA_AVAILABLE_POST_RUN, False) == True:
                    continue
                if dbtype == cellprofiler.measurements.COLTYPE_FLOAT:
                    jtype = "java.lang.Double"
                elif dbtype == cellprofiler.measurements.COLTYPE_INTEGER:
                    jtype = "java.lang.Integer"
                elif dbtype.startswith(cellprofiler.measurements.COLTYPE_VARCHAR):
        ...
        for key in features:
            features[key][cellprofiler.measurements.IMAGE_NUMBER] = 0
