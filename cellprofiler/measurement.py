            ...
            for feature_name in omeas.dtype.fields.keys():
                if object_name == IMAGE:
                    ...
                    object_counts[: len(values)] = np.maximum(
                        object_counts[: len(values)], np.array([len(x) for x in values])
                    )
    ...
    def add_measurement(
        ...
        if object_name == EXPERIMENT:
            ...
            for n, d in (
                ...
                self.hdf5_dict[object_name, OBJECT_NUMBER, n] = np.arange(1, len(d) + 1)
