...
class Measurements(object):
            ...
            for feature_name in omeas.dtype.fields.keys():
                if object_name == IMAGE:
                    ...
                    if len(object_counts) < len(values):
                        temp, object_counts = object_counts, np.zeros(
                            len(values), int)
                        if len(temp) > 0:
    ...
    def get_groupings(self, features):
        ...
        values = [[unicode(x) for x in
                   self.get_measurement(IMAGE, feature, image_numbers)]
    ...
    def add_measurement(self, object_name, feature_name, data,
        ...
        if object_name == EXPERIMENT:
            ...
            data = [d if d is None or d is np.NaN
                    ...
                    for d in data]
            self.hdf5_dict[
                IMAGE, feature_name, image_set_number, data_type] = data
            ...
            for n, d in (
            ((image_set_number, data),) if np.isscalar(image_set_number)
    ...
    def get_measurement_columns(self):
        ...
        for object_name in self.get_object_names():
            for feature_name in self.get_feature_names(object_name):
                dtype = self.hdf5_dict.get_feature_dtype(object_name,
                                                         feature_name)
    ...
    def has_measurements(self, object_name, feature_name, image_set_number):
        ...
        return self.hdf5_dict.has_data(object_name, feature_name,
                                       image_set_number)

    def has_current_measurements(self, object_name, feature_name):
        return self.has_measurements(object_name, feature_name,
                                     self.image_set_number)
    ...
    def load_image_sets(self, fd_or_file, start=None, stop=None):
        ...
        self.hdf5_dict.add_all(IMAGE, IMAGE_NUMBER, image_numbers,
                               image_numbers)
        for feature, column, all_none in zip(header, columns,
                                             column_is_all_none):
            if not all_none:
    ...
    def alter_url_post_create_batch(self, url):
        ...
        os_url2pathname = __import__(
            d[K_URL2PATHNAME_PACKAGE_NAME]).url2pathname
    ...
    def get_image_provider(self, name):
        ...
        assert len(providers) == 1, "More than one provider of the %s image" % (
        name)
        return providers[0]
        ...
        for feature_name in self.get_feature_names(EXPERIMENT):
            if feature_name.startswith(C_CHANNEL_TYPE):
                ...
                if url_feature not in self.get_feature_names(IMAGE):
                    continue
                iscds.append(
                    ImageSetChannelDescriptor(channel_name, channel_type))
        return iscds
    ...
    def cache_image_set(self,
        ...
        for i, name in enumerate(self.image_names):
            ...
            for feature, values in (
                    ...
                    (C_CHANNEL,
                     [isd.ipds[i].channel for isd in image_set_data])):
                self.image_set_table.add_column(
                    feature + "_" + name,
