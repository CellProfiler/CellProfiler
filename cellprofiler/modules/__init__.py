...
do_not_override = ['__init__', 'set_settings', 'create_from_handles',
                   'test_valid', 'module_class']
...
def check_module(module, name):
    if hasattr(module, 'do_not_check'):
        return
    assert name == module.module_name, "Module %s should have module_name %s (is %s)" % (
    ...
    if not all_modules.has_key(module_class):
        ...
        if replaced_modules.has_key(module_class):
            raise ValueError(("The %s module no longer exists. You can find "
                             ...
                             (module_class,
                              ", ".join(replaced_modules[module_class])))
