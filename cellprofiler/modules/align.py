...
class Align(cpm.CPModule):
    ...
    def settings(self):
        ...
        for additional in self.additional_images:
            result += [additional.input_image_name,
                       additional.output_image_name, additional.align_choice]
        return result
    ...
    def run(self, workspace):
        ...
        #
        for (input_name, output_name), (y, x), shape in zip(names, offsets,
                                                            shapes):
            self.apply_alignment(workspace, input_name, output_name, x, y,
                                 shape)
    ...
    def display(self, workspace, figure):
        ...
        for j, (
        input_name, input_pixels, output_name, output_pixels, off_x, off_y,
        shape) \
            ...
            img[:first_pixels.shape[0], :first_pixels.shape[1],
            0] = first_pixels
            img[:other_pixels.shape[0], :other_pixels.shape[1],
            1] = other_pixels
            figure.subplot_imshow(0, j, img, unaligned_title,
                                  sharexy=figure.subplot(0, 0))

            aligned_title = (
            "Aligned images: %s and %s\nX offset: %d, Y offset: %d" %
