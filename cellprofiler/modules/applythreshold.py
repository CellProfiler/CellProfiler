...
from centrosome.threshold import TM_GLOBAL, TM_ADAPTIVE, TM_PER_OBJECT, \
    TM_BINARY_IMAGE
...
class ApplyThreshold(Identify):
    ...
    def get_measurement_columns(self, pipeline):
        return get_threshold_measurement_columns(
            self.thresholded_image_name.value)
    ...
    def upgrade_settings(self, setting_values,
        ...
        if (not from_matlab) and variable_revision_number == 6:
            ...
            assign_middle_to_foreground, thresholding_measurement = \
                setting_values[:16]
            setting_values = [
                                 image_name, thresholded_image_name, binary,
                                 low_or_high,
                                 shift,
                                 dilation] + self.upgrade_legacy_threshold_settings(
