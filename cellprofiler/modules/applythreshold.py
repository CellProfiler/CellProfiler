...
class ApplyThreshold(cellprofiler.modules.identify.Identify):
    ...
    def create_settings(self):
        threshold_methods = [method for method in centrosome.threshold.TM_METHODS
                             if method != centrosome.threshold.TM_BINARY_IMAGE]
    ...
    def run(self, workspace):
        ...
        if self.binary != 'Grayscale':
            ...
            if self.low_or_high == TH_BELOW_THRESHOLD:
                ...
                if self.shift.value:
                    pixels[input_image.mask & binary_image] -= local_thresh if self.threshold_modifier == centrosome.threshold.TM_GLOBAL else local_thresh[input_image.mask & binary_image]
            elif self.low_or_high == TH_ABOVE_THRESHOLD:
                undilated = input_image.mask & binary_image
                dilated = scipy.ndimage.morphology.binary_dilation(undilated, centrosome.cpmorphology.strel_disk(self.dilation.value), mask=input_image.mask)
                pixels[dilated] = 0
            else:
                raise NotImplementedError("""Threshold setting, "%s" is not "%s" or "%s".""" % (self.low_or_high.value, TH_BELOW_THRESHOLD, TH_ABOVE_THRESHOLD))

        output = cellprofiler.image.Image(pixels, parent=input_image)
    ...
    def upgrade_settings(self, setting_values, variable_revision_number, module_name, from_matlab):
        ...
        if from_matlab and variable_revision_number == 4:
            ...
            setting_values[3] = (cellprofiler.setting.YES if float(setting_values[5]) > 0 else cellprofiler.setting.NO)  # low threshold set
            setting_values[4] = (cellprofiler.setting.YES if float(setting_values[7]) > 0 else cellprofiler.setting.NO)  # high threshold set
            variable_revision_number = 2
            from_matlab = False

        if (not from_matlab) and variable_revision_number == 1:
            setting_values = (setting_values[:9] + [centrosome.threshold.TM_MANUAL, setting_values[9], "O,1", "1", ".2", cellprofiler.setting.NONE])
            variable_revision_number = 2
