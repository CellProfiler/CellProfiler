        ...
        class Operand(object):
            ...
            def settings(self):
                ...
                return [self.operand_choice, self.operand_objects,
                        self.operand_measurement, self.multiplicand,
                        self.exponent]
        ...
        for operand in all_operands:
            ...
            if isinstance(value, str) or isinstance(value, unicode):
                try:
                    ...
                    raise ValueError(
                        "Unable to use non-numeric value in measurement, %s" % operand.measurement.value)
        ...
        if ((not has_image_measurement) and
            ...
            for gg in g:
                if gg.relationship == R_PARENT:
                    ...
                    v1 = bincount(i0, values[1][i1],
                                  minlength=len(values[0])) / c0
                    v0 = bincount(i1, values[0][i0],
                                  minlength=len(values[1])) / c1
                    break
            else:
                logger.warning(
