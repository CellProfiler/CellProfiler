...
class CalculateStatistics(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.add_dose_button = cps.DoSomething("",
                                               "Add another dose specification",
                                               self.add_dose_value)
    ...
    def prepare_settings(self, setting_values):
        ...
        if (value_count - FIXED_SETTING_COUNT) % VARIABLE_SETTING_COUNT != 0:
            raise ValueError(
                ...
                value_count)
        dose_count = (
                     value_count - FIXED_SETTING_COUNT) / VARIABLE_SETTING_COUNT
    ...
    def include_feature(self, measurements, object_name, feature_name,
        ...
        if (object_name == cpmeas.IMAGE and
            ...
            return False
        if (object_name == cpmeas.IMAGE and
                    feature_name in [g.measurement.value for g in
                                     self.dose_values]):
            return False
...
def z_factors(xcol, ymatr):
    ...
    for i in range(sort_avers.shape[1]):
        ...
        stds[0, i] = np.sqrt(
            np.sum((vals1 - sort_avers[0, i]) ** 2) / len(vals1))
        stds[1, i] = np.sqrt(
            np.sum((vals2 - sort_avers[1, i]) ** 2) / len(vals2))
...
def calc_init_params(x, y):
    ...
    if x0 == x1:
        ...
        raise ValueError(
            "All doses or labels for all image sets are %s. Can't calculate dose/response curves." % x0)
    elif y1 > y0:
        hillc = -1
    else:
        hillc = 1
