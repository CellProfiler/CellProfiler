            ...
            if not os.path.isfile(path_):
                ...
                if not file_.endswith('.txt'):
                    ...
                    if d[path_][2] == "FastGentleBoosting":
                        ...
                        d[path_] = (rules, d[path_][1], "Rules", [f"{rule.object_name}_{rule.feature}" for rule in rules.rules])
                else:
                    # Probably a rules list
                    rules = cellprofiler.utilities.rules.Rules()
                    rules.parse(path_)
                    # Construct a classifier-like object
                    d[path_] = (rules, rules.get_classes(), "Rules", [f"{rule.object_name}_{rule.feature}" for rule in rules.rules])
        return d[path_]
        ...
        if self.contrast_choice == BY_SINGLE_MEASUREMENT:
            ...
            for feature in features:
                if feature not in available_features:
                    raise ValidationError(
                        f"""The classifier {self.model_file_name}, requires the measurement, {feature[1]} for object 
