...
class DisplayPlatemap(cpm.CPModule):
    ...
    def run(self, workspace):
        ...
        if self.show_window:
            ...
            # Get wells
            if self.well_format == WF_NAME:
                wells = m.get_all_measurements(cpmeas.IMAGE,
                                               self.well_name.value)
            elif self.well_format == WF_ROWCOL:
                wells = ['%s%s' % (x, y) for x, y in zip(
                    m.get_all_measurements(cpmeas.IMAGE, self.well_row.value),
                    m.get_all_measurements(cpmeas.IMAGE, self.well_col.value))]
            # Get data to plot
            data = m.get_all_measurements(self.get_object(),
                                          self.plot_measurement.value)
