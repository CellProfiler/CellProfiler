        ...
        if self.x_source.value == self.y_source.value:
            if self.x_source.value == cpmeas.IMAGE:
                ...
                xvals, yvals = np.array([
                                            (x if np.isscalar(x) else x[0],
                                             y if np.isscalar(y) else y[0])
                ...
                xvals = m.get_current_measurement(self.get_x_object(),
                                                  self.x_axis.value)
                yvals = m.get_current_measurement(self.get_y_object(),
                                                  self.y_axis.value)
                title = '%s (cycle %d)' % (
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if not from_matlab and variable_revision_number == 1:
            if setting_values[0] == cpmeas.IMAGE:
                ...
                new_setting_values = [setting_values[0], cps.NONE,
                                      setting_values[1], cpmeas.IMAGE,
