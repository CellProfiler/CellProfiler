...
O_SHRINK_BY_MEASUREMENT = "Shrink objects by a previously calculated value"
O_EXPAND = "Expand objects by a specified number of pixels"
O_EXPAND_BY_MEASUREMENT = "Expand objects by a previously calculated value"
...
""".format(
    ...
    def do_labels(self, labels, workspace):
        ...
        if self.operation == O_SHRINK_BY_MEASUREMENT:
            m = workspace.measurements
            shrink_measurement = m.get_current_image_measurement(self.exp_shr_measurement.value)

            return centrosome.cpmorphology.binary_shrink(
                labels, iterations=int(shrink_measurement)
            )

        if self.operation in (O_EXPAND, O_EXPAND_INF, O_EXPAND_BY_MEASUREMENT):
