...
from cellprofiler.gui.help import USING_METADATA_TAGS_REF, \
    USING_METADATA_HELP_REF, MEASUREMENT_NAMING_HELP
...
class ExportToSpreadsheet(cpm.CPModule):
    ...
    def validate_module(self, pipeline):
        ...
        if (len(self.delimiter.value) != 1 and
            ...
            raise cps.ValidationError(
                "The CSV field delimiter must be a single character",
                self.delimiter)
    ...
    def validate_module_warnings(self, pipeline):
        ...
        if not self.wants_everything.value:
            all_extensions = [os.path.splitext(group.file_name.value)[1] for
                              group in self.object_groups]
            is_valid_extension = [
                not group.wants_automatic_file_name.value and (
                (extension == ".csv" and self.delimiter == DELIMITER_COMMA) or (
                extension == ".txt" and self.delimiter == DELIMITER_TAB))
                for (extension, group) in
                zip(all_extensions, self.object_groups)]
            if not all(is_valid_extension):
                raise (cps.ValidationError(
                    ...
                    self.object_groups[
                        is_valid_extension.index(False)].file_name))
                ...
                for i in range(len(self.object_groups)):
                    ...
                    if first:
                        filename = self.make_objects_file_name(
                            group.name.value, workspace, image_set_number,
                            group)
                        first = False
            ...
            if len(object_names) == 1 and object_names[0] == IMAGE:
                ...
                if self.wants_genepattern_file.value:
                    self.make_gct_file(metadata_group.image_numbers,
                                       workspace, settings_group)
            elif len(object_names) == 1 and object_names[
                0] == OBJECT_RELATIONSHIPS:
    ...
    def make_image_file(self, image_set_numbers, workspace,
                        settings_group=None):
        ...
        try:
            ...
            for img_number in image_set_numbers:
                ...
                if img_number == image_set_numbers[0]:
                    ...
                    num_measures = 0
                    for feature_name in image_features:
                        if not ignore_feature(
                                feature_name) or agg_measurements.has_key(
                                feature_name):
                            num_measures += 1
                    ...
                    measurement_feature_names = [x for x in image_features if
                                                 not ignore_feature(x)]

                    # The first headers need to be 'NAME' and 'Description'
                    written_image_names = ['NAME',
                                           'Description'] + measurement_feature_names
                    writer.writerow(written_image_names)

                    # Place the one of the paths and desired info column up front in image feature list
                    description_feature = [x for x in image_features if
                                           x.startswith(C_PATH_NAME + "_")]
                    if self.how_to_specify_gene_name == GP_NAME_METADATA:
                        name_feature = [self.gene_name_column.value]
                    elif self.how_to_specify_gene_name == GP_NAME_FILENAME:
                        name_feature = [x for x in image_features if
                                        x.startswith("_".join((C_FILE_NAME,
                                                               self.use_which_image_for_gene_name.value)))]
                    image_features = [name_feature[0], description_feature[
                        0]] + measurement_feature_names
...
Do you want to save it anyway?""" %
            ...
            if message is not None:
                ...
                if wx.MessageBox(message, "Excel limits exceeded",
                                 wx.YES_NO) == wx.ID_NO:
                    return False
        return True
    ...
    def make_relationships_file(self, image_set_numbers, workspace,
        ...
        try:
            ...
            writer.writerow([
                ...
                "First Object Number",
                "Second Object Name", "Second Image Number",
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if variable_revision_number == 4 and not from_matlab:
            ...
            setting_values = setting_values[:12] + [
                'None|None'] + setting_values[12:]
...
class EEObjectNameSubscriber(cps.ObjectNameSubscriber):
    ...
    def get_choices(self, pipeline):
        choices = [
            (s, '', 0, False) for s in
            [IMAGE, EXPERIMENT, OBJECT_RELATIONSHIPS]]
