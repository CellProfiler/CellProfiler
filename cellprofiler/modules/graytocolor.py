...
class GrayToColor(cpm.CPModule):
        ...
        if self.scheme_choice == SCHEME_RGB:
            ...
            return [ColorSchemeSettings(self.cyan_image_name,
                    ...
                    ColorSchemeSettings(self.magenta_image_name,
                                        self.magenta_adjustment_factor, .5, .5,
                                        0),
                    ColorSchemeSettings(self.yellow_image_name,
                                        self.yellow_adjustment_factor, .5, 0,
                                        .5),
    ...
    def settings(self):
        result = [self.scheme_choice,
                  self.red_image_name, self.green_image_name,
    ...
    def run(self, workspace):
        ...
        if self.scheme_choice not in (SCHEME_STACK, SCHEME_COMPOSITE):
            ...
            input_image_names = [sc.image_name.value for sc in
                                 self.stack_channels]
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if from_matlab and variable_revision_number == 1:
            ...
            temp[OFF_RED_ADJUSTMENT_FACTOR] = setting_values[
                OFF_BLUE_ADJUSTMENT_FACTOR]
            temp[OFF_BLUE_ADJUSTMENT_FACTOR] = setting_values[
                OFF_RED_ADJUSTMENT_FACTOR]
            setting_values = temp
