...
from cellprofiler.gui.help import PROTIP_RECOMEND_ICON, PROTIP_AVOID_ICON, \
    TECH_NOTE_ICON, GROUPS_DISPLAY_TABLE
...
class Groups(cpm.CPModule):
    ...
    def update_tables(self):
        if self.wants_groups:
            ...
            image_set_feature_names = [
                                          cpmeas.GROUP_NUMBER,
                                          cpmeas.GROUP_INDEX] + metadata_feature_names
            ...
            for i, iscd in enumerate(channel_descriptors):
                ...
                self.image_set_list.insert_column(idx + 1,
                                                  "File: %s" % image_name)
    ...
    def prepare_run(self, workspace):
        ...
        new_image_numbers[image_numbers[order]] = np.arange(
            len(image_numbers)) + 1
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if variable_revision_number == 1:
            ...
            for i in range(
                    int(setting_values[self.IDX_GROUPING_METADATA_COUNT])):
                new_setting_values.append(
                    setting_values[
                        self.IDX_GROUPING_METADATA_COUNT + 2 + i * 2])
