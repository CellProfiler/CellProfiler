...
def get_image_name(feature):
    ...
    raise ValueError(
        '"%s" is not a path feature or file name feature' % feature)
...
class LoadData(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.wants_images = cps.Binary("Load images based on this data?", True,
                                       doc="""
    ...
    def convert(self):
        ...
        for i in range(len(waves)):
            if len(filename[i]) != len(filename[0]):
                raise RuntimeError(
                    ...
                    (wave_fnames[i], len(filename[i]), wave_fnames[0],
                     len(filename[0])))
        ...
        data_onewave_nofilepaths = matplotlib.mlab.rec_keep_fields(data_onewave,
                                                                   metadata_names)
        ...
        final_data = matplotlib.mlab.rec_append_fields(final_data,
    ...
    def get_image_names(self, do_not_cache=False):
        ...
        image_names = set([
                              ...
                              for field in header
                              if is_file_name_feature(
                field) or is_url_name_feature(field)])
        ...
        for row in reader:
            ...
            for index, field in enumerate(row):
                if already_output[index]:
                    continue
                if ((not self.wants_images) and key_is_path_or_file_name[
                    index]):
                    continue
                try:
                    len_field = len(field)
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if variable_revision_number == 4 and (not from_matlab):
            ...
            image_directory, wants_rows, row_range, wants_image_groupings, \
            metadata_fields = setting_values
            dir_choice, custom_dir = cps.DirectoryPath.split_string(
                csv_directory)
