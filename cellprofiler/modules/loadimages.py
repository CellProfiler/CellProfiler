...
FF_OTHER_MOVIES_OLD = ['tif,tiff,flex movies',
                       'tif,tiff,flex movies, zvi movies']
...
class LoadImages(cpmodule.CPModule):
    ...
    def create_settings(self):
        ...
        # Add another image
        self.add_image = cps.DoSomething("", "Add another image",
                                         self.add_imagecb)
    ...
    def add_channel(self, image_settings, can_remove=True):
        ...
        group.append("channel_number", cps.Choice(
            ...
            channels[len(image_settings.channels) - 1], doc="""
            <i>(Used only if a movie image format is selected as file type and movie frame grouping is selected)</i><br>
        ...
        if can_remove:
            group.append("remover", cps.RemoveSettingButton(
                "Remove this channel", "Remove channel",
    ...
    def visible_settings(self):
        ...
        for i, fd in enumerate(self.images):
            is_multichannel = (
            self.is_multichannel or fd.wants_movie_frame_grouping)
            varlist += [fd.divider]
    ...
    def validate_module_warnings(self, pipeline):
        ...
        for i, fd in enumerate(self.images):
            ...
            if not is_multichannel:
                if self.channel_wants_images(fd.channels[0]):
                    if not re.match(invalid_chars_pattern,
                                    fd.channels[0].image_name.value):
                        raise cps.ValidationError(warning_text,
                                                  fd.channels[0].image_name)
            else:
                for channel in fd.channels:
                    if self.channel_wants_images(channel):
                        if not re.match(invalid_chars_pattern,
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        def upgrade_new_1_to_2(setting_values):
            ...
            for i in range((len(
                    setting_values) - self.SLOT_FIRST_IMAGE_V1) / self.SLOT_IMAGE_FIELD_COUNT_V1):
        ...
        def upgrade_new_6_to_7(setting_values):
            ...
            for i in range(image_count):
                ...
                channel_count = int(
                    setting_values[self.SLOT_OFFSET_CHANNEL_COUNT_V6])
                setting_values = setting_values[self.SLOT_IMAGE_FIELD_COUNT_V5:]
        ...
        def upgrade_new_7_to_8(setting_values):
            ...
            for i in range(image_count):
                ...
                channel_count = int(
                    setting_values[self.SLOT_OFFSET_CHANNEL_COUNT_V7])
        ...
        def upgrade_new_8_to_9(setting_values):
            ...
            for i in range(image_count):
                ...
                channel_count = int(
                    setting_values[self.SLOT_OFFSET_CHANNEL_COUNT_V8])
        ...
        def upgrade_new_9_to_10(setting_values):
            ...
            for i in range(image_count):
                ...
                channel_count = int(
                    setting_values[self.SLOT_OFFSET_CHANNEL_COUNT_V9])
                setting_values = setting_values[self.SLOT_IMAGE_FIELD_COUNT_V9:]
                for j in range(channel_count):
                    new_values += setting_values[
                                  :self.SLOT_OFFSET_OBJECT_NAME_V9] + \
        ...
        setting_values[self.SLOT_LOCATION] = \
            cps.DirectoryPath.upgrade_setting(
                setting_values[self.SLOT_LOCATION])
    ...
    def get_image_sets(self, d):
        ...
        for value in sorted(values):
            subgroup = tuple(
                (dd and dd.has_key(None) and dd[None]) or  # wildcard metadata
                ...
                for i in range(image_count):
                    ...
                    if image_settings.wants_movie_frame_grouping:
                        ...
                        for idx in range(nsets):
                            ...
                            frame_metadata[
                                M_Z] = 0  # so sorry, real Z obliterated
                            ...
                            if match_metadata:
                                ...
                                if not md_dict.has_key(key):
                                    ...
                                    pipeline.report_prepare_run_error(self,
                                                                      message)
                                    return False
                            ...
                            for image_number in image_numbers:
                                for channel_settings in image_settings.channels:
                                    ...
                                    channel = int(
                                        channel_settings.channel_number.value) - 1
                                    ...
                                    t = int(
                                        cidx / channel_count / stack_count) % timepoint_count
                                    m.add_measurement(
                                        cpmeas.IMAGE,
            ...
            for channel in fd.channels:
                ...
                if wants_images:
                    ...
                    if image_size is None:
                        ...
                        warning = bad_sizes_warning(image_size,
                    ...
                    if channel.wants_outlines:
                        ...
                        outline_image = cpimage.Image(outlines,
                                                      path_name=path,
                                                      file_name=filename)
                        workspace.image_set.add(channel.outlines_name.value,
                                                outline_image)
        ...
        if not use_cached:
            ...
            if self.analyze_sub_dirs():
                ...
                for dirpath, dirnames, filenames in w:
                    ...
                    if (path in prohibited) or (
                        os.path.realpath(dirpath) in seen_dirs):
    ...
    def image_name_vars(self):
        ...
        for image in self.images:
            if (self.is_multichannel or
                ...
                result += [channel.get_image_name() for channel in
                           image.channels]
            else:
    ...
    def get_measurement_columns(self, pipeline):
        ...
        if needs_well_metadata(all_tokens):
            cols += [
                (cpmeas.IMAGE, '_'.join((cpmeas.C_METADATA, cpmeas.FTR_WELL)),
                 cpmeas.COLTYPE_VARCHAR_FILE_NAME)]
        ...
        for group in self.images:
            for channel in group.channels:
                ...
                for metadata_choice, source, value in (
                        (M_FILE_NAME, cpmetadata.XM_FILE_NAME,
                         group.file_metadata.value),
                        (M_PATH, cpmetadata.XM_FOLDER_NAME,
    ...
    def cache_file(self):
        ...
        if len(path) == 0:
            ...
            if len(parsed_path.scheme) < 2:
                raise IOError(
                    "Test for access to file failed. File: %s" % filename)
        ...
        if self.__is_cached:
            if self.is_matlab_file():
                try:
                    ...
                    logger.warning("Could not delete file %s",
                                   self.__cached_file,
                                   exc_info=True)
            else:
