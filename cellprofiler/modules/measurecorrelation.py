...
class MeasureCorrelation(cellprofiler.module.Module):
    ...
    def add_object(self, can_delete=True):
        ...
        group.append("object_name", cellprofiler.setting.ObjectNameSubscriber(
                'Select an object to measure', cellprofiler.setting.NONE, doc='''
    ...
    def run_image_pair_images(self, workspace, first_image_name,
        ...
        if numpy.any(mask):
            ...
            # A * i1 + B = i2
            #
            coeffs = scipy.linalg.lstsq(numpy.array((fi, numpy.ones_like(fi))).transpose(), si)[0]
            slope = coeffs[0]
    ...
    def run_image_pair_objects(self, workspace, first_image_name,
        ...
        try:
            ...
            first_pixels, m1 = cellprofiler.region.size_similarly(labels, first_image.pixel_data)
            first_mask, m1 = cellprofiler.region.size_similarly(labels, first_image.mask)
            first_mask[~m1] = False
        try:
            ...
            second_pixels, m1 = cellprofiler.region.size_similarly(labels, second_image.pixel_data)
            second_mask, m1 = cellprofiler.region.size_similarly(labels, second_image.mask)
        ...
        if n_objects == 0:
            ...
            area = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum(numpy.ones_like(labels), labels, lrange))
            mean1 = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.mean(first_pixels, labels, lrange))
            mean2 = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.mean(second_pixels, labels, lrange))
            #
            # Calculate the standard deviation times the population.
            #
            std1 = numpy.sqrt(centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum((first_pixels - mean1[labels - 1]) ** 2,
                                                                                                   labels, lrange)))
            std2 = numpy.sqrt(centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum((second_pixels - mean2[labels - 1]) ** 2,
                                                                                                   labels, lrange)))
            x = first_pixels - mean1[labels - 1]  # x - mean(x)
            y = second_pixels - mean2[labels - 1]  # y - mean(y)
            corr = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum(x * y / (std1[labels - 1] * std2[labels - 1]),
                                                                                        labels, lrange))
            # Explicitly set the correlation to NaN for masked objects
            corr[scipy.ndimage.sum(1, labels, lrange) == 0] = numpy.NaN
            result += [
                [first_image_name, second_image_name, object_name, "Mean Correlation coeff", "%.3f" % numpy.mean(corr)],
            ...
            tff = (self.thr.value / 100) * centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.maximum(first_pixels, labels, lrange))
            tss = (self.thr.value / 100) * centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.maximum(second_pixels, labels, lrange))
            ...
            tot_fi_thr = scipy.ndimage.sum(first_pixels[first_pixels > tff[labels - 1]], labels[first_pixels > tff[labels - 1]],
                                           lrange)
            ...
            if numpy.any(fi_above_thr):
                tot_fi_thr_c = scipy.ndimage.sum(first_pixels[first_pixels > thr_fi_c], labels[first_pixels > thr_fi_c], lrange)
            else:
                tot_fi_thr_c = numpy.zeros(len(lrange))
            if numpy.any(si_above_thr):
                tot_si_thr_c = scipy.ndimage.sum(second_pixels[second_pixels > thr_si_c], labels[second_pixels > thr_si_c],
                                                 lrange)
            else:
            ...
            result += [[first_image_name, second_image_name, object_name,"Mean Manders coeff","%.3f" % numpy.mean(M1)],
                       [first_image_name, second_image_name, object_name,"Median Manders coeff","%.3f" % numpy.median(M1)],
                       [first_image_name, second_image_name, object_name,"Min Manders coeff","%.3f" % numpy.min(M1)],
                       [first_image_name, second_image_name, object_name,"Max Manders coeff","%.3f" % numpy.max(M1)]]
            result += [[second_image_name, first_image_name, object_name,"Mean Manders coeff","%.3f" % numpy.mean(M2)],
                       [second_image_name, first_image_name, object_name,"Median Manders coeff","%.3f" % numpy.median(M2)],
                       [second_image_name, first_image_name, object_name,"Min Manders coeff","%.3f" % numpy.min(M2)],
                       [second_image_name, first_image_name, object_name,"Max Manders coeff","%.3f" % numpy.max(M2)]]
            ...
            result += [[first_image_name, second_image_name, object_name, "Mean RWC coeff", "%.3f" % numpy.mean(RWC1)],
                       [first_image_name, second_image_name, object_name, "Median RWC coeff", "%.3f" % numpy.median(RWC1)],
                       [first_image_name, second_image_name, object_name, "Min RWC coeff", "%.3f" % numpy.min(RWC1)],
                       [first_image_name, second_image_name, object_name, "Max RWC coeff", "%.3f" % numpy.max(RWC1)]]
            result += [[second_image_name, first_image_name, object_name, "Mean RWC coeff", "%.3f" % numpy.mean(RWC2)],
                       [second_image_name, first_image_name, object_name, "Median RWC coeff", "%.3f" % numpy.median(RWC2)],
                       [second_image_name, first_image_name, object_name, "Min RWC coeff", "%.3f" % numpy.min(RWC2)],
                       [second_image_name, first_image_name, object_name, "Max RWC coeff", "%.3f" % numpy.max(RWC2)]]
            ...
            result += [[first_image_name, second_image_name, object_name,"Mean Manders coeff (Costes)","%.3f" % numpy.mean(C1)],
                       [first_image_name, second_image_name, object_name,"Median Manders coeff (Costes)","%.3f" % numpy.median(C1)],
                       [first_image_name, second_image_name, object_name,"Min Manders coeff (Costes)","%.3f" % numpy.min(C1)],
                       [first_image_name, second_image_name, object_name,"Max Manders coeff (Costes)","%.3f" % numpy.max(C1)]
                       ]
            result += [[second_image_name, first_image_name, object_name,"Mean Manders coeff (Costes)","%.3f" % numpy.mean(C2)],
                       [second_image_name, first_image_name, object_name,"Median Manders coeff (Costes)","%.3f" % numpy.median(C2)],
                       [second_image_name, first_image_name, object_name,"Min Manders coeff (Costes)","%.3f" % numpy.min(C2)],
                       [second_image_name, first_image_name, object_name,"Max Manders coeff (Costes)","%.3f" % numpy.max(C2)]
                       ]
            ...
            if numpy.any(combined_thresh):
                overlap = centrosome.cpmorphology.fixup_scipy_ndimage_result(
                    scipy.ndimage.sum(first_pixels[combined_thresh] * second_pixels[combined_thresh],
                                      labels[combined_thresh],
                                      lrange) / pdt)
                K1 = centrosome.cpmorphology.fixup_scipy_ndimage_result((scipy.ndimage.sum(first_pixels[combined_thresh] * second_pixels[combined_thresh],
                                                                                           labels[combined_thresh], lrange)) / (numpy.array(fpsq)))
                K2 = centrosome.cpmorphology.fixup_scipy_ndimage_result(
                    scipy.ndimage.sum(first_pixels[combined_thresh] * second_pixels[combined_thresh],
                                      labels[combined_thresh],
            ...
            result += [
                [first_image_name, second_image_name, object_name, "Mean Overlap coeff", "%.3f" % numpy.mean(overlap)],
                [first_image_name, second_image_name, object_name, "Median Overlap coeff", "%.3f" % numpy.median(overlap)],
                [first_image_name, second_image_name, object_name, "Min Overlap coeff", "%.3f" % numpy.min(overlap)],
                [first_image_name, second_image_name, object_name, "Max Overlap coeff", "%.3f" % numpy.max(overlap)]]
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if from_matlab and variable_revision_number == 3:
            ...
            wants_image_measured = numpy.any([x == cellprofiler.measurement.IMAGE
                                              for x in setting_values[4:]])
            object_names = [x for x in setting_values[4:]
                            if not x in (cellprofiler.measurement.IMAGE, cellprofiler.setting.DO_NOT_USE)]
            ...
            if len(object_names) == 0:
                object_names = [cellprofiler.setting.NONE]
