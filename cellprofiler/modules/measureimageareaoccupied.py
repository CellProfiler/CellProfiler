...
class MeasureImageAreaOccupied(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.add_operand_button = cps.DoSomething("", "Add another area",
                                                  self.add_operand)
    ...
    def validate_module(self, pipeline):
        ...
        for group in self.operands:
            if (group.operand_choice.value,
                group.operand_objects.value) in settings:
                if group.operand_choice.value == O_OBJECTS:
                    raise cps.ValidationError(
                        "%s has already been selected" % group.operand_objects.value,
                        group.operand_objects)
            settings[(
            group.operand_choice.value, group.operand_objects.value)] = True

        settings = {}
        for group in self.operands:
            if (
            group.operand_choice.value, group.binary_name.value) in settings:
                if group.operand_choice.value == O_BINARY_IMAGE:
                    raise cps.ValidationError(
                        "%s has already been selected" % group.binary_name.value,
                        group.binary_name)
            settings[
                (group.operand_choice.value, group.binary_name.value)] = True

    def settings(self):
        result = [self.count]
        for op in self.operands:
            result += [op.operand_choice, op.operand_objects,
                       op.should_save_image, op.image_name, op.binary_name]
        return result
    ...
    def visible_settings(self):
        ...
        for op in self.operands:
            ...
            result += ([op.operand_objects,
                        op.should_save_image] if op.operand_choice == O_OBJECTS else [
                op.binary_name])
    ...
    def measure_objects(self, operand, workspace):
        ...
        if objects.has_parent_image:
            ...
            perimeter = np.sum(outline(np.logical_and(objects.segmented != 0,
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if variable_revision_number == 2:
            ...
            for i in range(0, count):
                new_setting_values += ['Objects', setting_values[(i * 3)],
