...
def surface_area(label_image, spacing=None, index=None):
    ...
    if index is None:
        verts, faces, _, _ = skimage.measure.marching_cubes_classic(label_image, spacing=spacing, level=0)
...
def _label_surface_area(label_image, label, spacing):
    verts, faces, _, _ = skimage.measure.marching_cubes_classic(label_image == label, spacing=spacing, level=0)
