...
ALL_MEASUREMENTS = ["TotalIntensity", "MeanIntensity", "StdIntensity",
                    "MADIntensity", "MedianIntensity",
...
class MeasureImageIntensity(cpm.CPModule):
    ...
    def validate_module(self, pipeline):
        ...
        for group in self.images:
            if (group.image_name.value, group.wants_objects.value,
                group.object_name.value) in settings:
                if not group.wants_objects.value:
                    ...
                    raise cps.ValidationError(
                        ...
                        group.object_name)
            settings[(group.image_name.value, group.wants_objects.value,
                      group.object_name.value)] = True
    ...
    def measure(self, im, workspace):
        ...
        if pixel_count == 0:
            ...
            pixels = pixels[
                np.nonzero(np.isfinite(pixels))[0]]  # Ignore NaNs, Infs
            pixel_count = np.product(pixels.shape)
            ...
            pixel_lower_qrt = sorted_pixel_data[
                int(len(sorted_pixel_data) * 0.25)]
            pixel_upper_qrt = sorted_pixel_data[
                int(len(sorted_pixel_data) * 0.75)]

        m = workspace.measurements
        m.add_image_measurement(F_TOTAL_INTENSITY % (measurement_name),
                                pixel_sum)
        m.add_image_measurement(F_MEAN_INTENSITY % (measurement_name),
                                pixel_mean)
        m.add_image_measurement(F_MEDIAN_INTENSITY % (measurement_name),
                                pixel_median)
        ...
        m.add_image_measurement(F_PERCENT_MAXIMAL % (measurement_name),
                                pixel_pct_max)
        m.add_image_measurement(F_LOWER_QUARTILE % (measurement_name),
                                pixel_lower_qrt)
        m.add_image_measurement(F_UPPER_QUARTILE % (measurement_name),
                                pixel_upper_qrt)
    ...
    def get_measurement_columns(self, pipeline):
        ...
        for im in self.get_non_redundant_image_measurements():
            for feature, coltype in ((F_TOTAL_INTENSITY, cpmeas.COLTYPE_FLOAT),
                ...
                columns.append(
                    (cpmeas.IMAGE, feature % measurement_name, coltype))
        return columns
