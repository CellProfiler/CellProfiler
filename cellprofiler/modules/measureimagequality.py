...
INTENSITY_FEATURES = [F_TOTAL_AREA, F_TOTAL_INTENSITY, F_MEAN_INTENSITY,
                      F_MEDIAN_INTENSITY, F_STD_INTENSITY, F_MAD_INTENSITY,
                      F_MAX_INTENSITY, F_MIN_INTENSITY]
...
class MeasureImageQuality(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.add_image_button = cps.DoSomething("", "Add another image list",
    ...
    def add_image_group(self, can_remove=True):
        ...
        group.threshold_count = cps.HiddenCount(group.threshold_groups,
                                                "Threshold count")
        ...
        if can_remove:
            group.append("remove_button",
                         cps.RemoveSettingButton("", "Remove this image list",
    ...
    def add_threshold_group(self, image_group=None, can_remove=True):
        ...
        group.append("threshold_method",
                     cps.Choice("Select a thresholding method",
                                cpthresh.TM_METHODS,
        ...
        if can_remove and image_group is not None:
            group.append("remove_button", cps.RemoveSettingButton(
                ...
                image_group.threshold_groups, group))
    ...
    def prepare_settings(self, setting_values):
        ...
        for index, image_group in enumerate(self.image_groups):
            for count, group, fn in \
                    ((int(
                        setting_values[IMAGE_GROUP_SETTING_OFFSET + 2 * index]),
                      image_group.scale_groups, self.add_scale_group),
                     (int(setting_values[
                                      IMAGE_GROUP_SETTING_OFFSET + 2 * index + 1]),
                      image_group.threshold_groups, self.add_threshold_group)):
                del group[:]
    ...
    def validate_module(self, pipeline):
        ...
        for m, s in zip(measurements, sources):
            ...
            if m in d:
                raise cps.ValidationError(
                    "Measurement %s for image %s made twice." % (m[1], s[1]),
                    s[0])
            d[m] = True
    ...
    def get_measurement_columns(self, pipeline, return_sources=False):
        ...
        sources = []
        for image_group in self.image_groups:
            selected_images = self.images_to_process(image_group, None,
                                                     pipeline)
            # Image scalings
            if image_group.include_image_scalings.value:
                for image_name in selected_images:
                    ...
                    sources.append(
                        [image_group.include_image_scalings, image_name])
            ...
            if image_group.check_blur.value:
                for image_name in selected_images:
                    ...
                    columns.append((cpmeas.IMAGE,
                                    '%s_%s_%s' % (
                                    C_IMAGE_QUALITY, F_POWER_SPECTRUM_SLOPE,
                                    image_name),
                                    cpmeas.COLTYPE_FLOAT))
                    sources.append([image_group.check_blur, image_name])
                    ...
                    for scale_group in image_group.scale_groups:
                        columns.append((cpmeas.IMAGE,
                                        '%s_%s_%s_%d' % (
                                        C_IMAGE_QUALITY, F_LOCAL_FOCUS_SCORE,
                                        image_name,
                ...
                for image_name in selected_images:
                    for feature in INTENSITY_FEATURES:
                        ...
                        columns.append((cpmeas.IMAGE,
                                        ...
                                        cpmeas.COLTYPE_FLOAT))
                        sources.append(
                            [image_group.check_intensity, image_name])
                ...
                for image_name in selected_images:
                    for feature in SATURATION_FEATURES:
                        columns.append((cpmeas.IMAGE,
                                        ...
                                        cpmeas.COLTYPE_FLOAT))
                        sources.append(
                            [image_group.check_saturation, image_name])

            # Threshold measurements
            if image_group.calculate_threshold.value:
                all_threshold_groups = self.get_all_threshold_groups(
                    image_group)
                for image_name in selected_images:
                    for threshold_group in all_threshold_groups:
                        feature = threshold_group.threshold_feature_name(
                            image_name)
                        columns.append(
                            (cpmeas.IMAGE, feature, cpmeas.COLTYPE_FLOAT))
                        for agg in ("Mean", "Median", "Std"):
                            feature = threshold_group.threshold_feature_name(
                                image_name, agg)
                            columns.append(
                                (cpmeas.EXPERIMENT, feature,
                                 cpmeas.COLTYPE_FLOAT,
    ...
    def get_measurements(self, pipeline, object_name, category):
        if object_name == cpmeas.IMAGE and category == C_IMAGE_QUALITY:
            ...
            if self.any_scaling():
                result += [C_SCALING]
            if self.any_blur():
                result += [F_FOCUS_SCORE, F_LOCAL_FOCUS_SCORE,
                           F_POWER_SPECTRUM_SLOPE, F_CORRELATION]
    ...
    def get_measurement_images(self, pipeline, object_name, category,
        ...
        if object_name != cpmeas.IMAGE or category != C_IMAGE_QUALITY:
            return []
        if measurement in (
        F_FOCUS_SCORE, F_LOCAL_FOCUS_SCORE, F_POWER_SPECTRUM_SLOPE,
        F_CORRELATION):
            result = []
            for image_group in self.image_groups:
                if image_group.check_blur.value:
                    result += self.images_to_process(image_group, None,
                                                     pipeline)
            return result
    ...
    def get_measurement_scales(self, pipeline, object_name, category,
        ...
        if (object_name == cpmeas.IMAGE and category == C_IMAGE_QUALITY):
            if measurement in (F_LOCAL_FOCUS_SCORE, F_CORRELATION):
                ...
                for image_group in self.image_groups:
                    for scale_group in image_group.scale_groups:
                        if image_names in self.images_to_process(image_group,
                                                                 None,
                                                                 pipeline):
                            result += [scale_group.scale]
                return result
            if measurement.startswith(F_THRESHOLD):
                ...
                for image_group in self.image_groups:
                    ...
                    result += [threshold_group.threshold_scale for
                               threshold_group in all_threshold_groups
                               if ((
                                   measurement == F_THRESHOLD + threshold_group.threshold_algorithm) and
        ...
        for image_name in self.images_to_process(image_group, workspace):
            ...
            for scale_group in image_group.scale_groups:
                ...
                if len(pixel_data):
                    ...
                    squared_normalized_image = (
                                               pixel_data - mean_image_value) ** 2
                    if mean_image_value > 0:
                        focus_score = (np.sum(squared_normalized_image) /
                                       (np.product(
                                           pixel_data.shape) * mean_image_value))
                #
                ...
                #
                local_means = fix(
                    scind.mean(image.pixel_data, grid, grid_range))
                ...
                local_focus_score += [
                    0]  # assume the worst - that we can't calculate it
                if len(nz_grid_range):
                    sums = fix(scind.sum(local_squared_normalized_image, grid,
                                         nz_grid_range))
                    pixel_counts = fix(
                        scind.sum(np.ones(shape), grid, nz_grid_range))
                    local_norm_var = (sums /
                    ...
                    if np.isfinite(local_norm_median) and local_norm_median > 0:
                        local_focus_score[-1] = np.var(
                            local_norm_var) / local_norm_median
            ...
            for scale_group in image_group.scale_groups:
                ...
                workspace.add_measurement(cpmeas.IMAGE, "%s_%s_%s_%d" %
                                          (C_IMAGE_QUALITY, F_CORRELATION,
                                           image_name, scale),
                                          float(value))
                result += [["%s %s @%d" % (image_name, F_CORRELATION, scale),
                            "%.2f" % (float(value))]]
        return result
        ...
        for image_name in self.images_to_process(image_group, workspace):
            ...
            if pixel_count == 0:
                ...
                percent_minimal = (100.0 * float(number_pixels_minimal) /
                                   float(pixel_count))
            percent_maximal_name = "%s_%s_%s" % (
            C_IMAGE_QUALITY, F_PERCENT_MAXIMAL,
            image_name)
            percent_minimal_name = "%s_%s_%s" % (
            C_IMAGE_QUALITY, F_PERCENT_MINIMAL,
            image_name)
    ...
    def run_intensity_measurement(self, image_name, workspace):
        ...
        m.add_image_measurement(
            "_".join((C_IMAGE_QUALITY, F_TOTAL_AREA, image_name)), pixel_count)
        m.add_image_measurement(
            "_".join((C_IMAGE_QUALITY, F_TOTAL_INTENSITY, image_name)),
            pixel_sum)
        m.add_image_measurement(
            "_".join((C_IMAGE_QUALITY, F_MEAN_INTENSITY, image_name)),
            pixel_mean)
        m.add_image_measurement(
            "_".join((C_IMAGE_QUALITY, F_MEDIAN_INTENSITY, image_name)),
            pixel_median)
        m.add_image_measurement(
        ...
        for image_name in self.images_to_process(image_group, workspace):
            ...
            if sum(magnitude) > 0 and len(np.unique(pixel_data)) > 1:
                ...
                if radii.shape[0] > 1:
                    ...
                    powerslope = lstsq(np.hstack((np.log(radii)[idx][:,
                                                  np.newaxis],
                                                  np.ones(radii.shape)[idx][:,
                                                  np.newaxis])),
    ...
    def calculate_thresholds(self, image_group, workspace):
        ...
        for image_name in self.images_to_process(image_group, workspace):
            ...
            for threshold_group in all_threshold_groups:
                ...
                two_class_otsu = (
                threshold_group.two_class_otsu.value == O_TWO_CLASS)
                ...
                workspace.add_measurement(cpmeas.IMAGE,
                                          threshold_group.threshold_feature_name(
                                              image_name),
                                          global_threshold)
                result += [
                    ["%s %s threshold" % (image_name, threshold_description),
    ...
    def build_threshold_parameter_list(self):
        ...
        z = product([cpthresh.TM_OTSU], [0], [O_WEIGHTED_VARIANCE, O_ENTROPY],
                    [O_THREE_CLASS], [O_FOREGROUND, O_BACKGROUND])
        threshold_args += [i for i in z]
        z = product([cpthresh.TM_OTSU], [0], [O_WEIGHTED_VARIANCE, O_ENTROPY],
                    [O_TWO_CLASS], [O_FOREGROUND])
        threshold_args += [i for i in z]
        z = product([cpthresh.TM_MOG], object_fraction, [O_WEIGHTED_VARIANCE],
                    [O_TWO_CLASS], [O_FOREGROUND])
        threshold_args += [i for i in z]
        # Tack on the remaining simpler methods
        leftover_methods = [i for i in cpthresh.TM_METHODS if
                            i not in [cpthresh.TM_OTSU, cpthresh.TM_MOG]]
        z = product(leftover_methods, [0], [O_WEIGHTED_VARIANCE], [O_TWO_CLASS],
                    [O_FOREGROUND])
        threshold_args += [i for i in z]
        ...
        for threshold_method, object_fraction, use_weighted_variance, two_class_otsu, assign_middle_to_foreground in threshold_args:
            ...
            threshold_groups[
                -1].use_weighted_variance.value = use_weighted_variance
            threshold_groups[
                -1].assign_middle_to_foreground.value = assign_middle_to_foreground
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if (not from_matlab) and variable_revision_number == 3:
            ...
            for idx in range(num_images):
                ...
                im_settings = setting_values[(idx * SETTINGS_PER_GROUP_V3):(
            ...
            import itertools
            for image_name in d.keys():
                d[image_name]["blur_scales"] = list(
                    set(d[image_name]["blur_scales"]))
            ...
            new_settings = [O_SELECT, str(
                len(unique_image_names))]  # images_choice, image_count
            new_settings += [str(len(d[image_name]["blur_scales"])) for
                             image_name in unique_image_names]  # scale_count
            new_settings += [str(len(d[image_name]["threshold_methods"])) for
                             image_name in
                             unique_image_names]  # threshold_count
            for image_name in unique_image_names:
                new_settings += [image_name,  # image_name
                                 cps.YES if d[image_name][
                                     "wants_scaling"] else cps.NO,
                                 # include_image_scalings
                                 cps.YES if d[image_name][
                                     "wants_blur"]    else cps.NO]  # check_blur
                new_settings += [k for k in
                                 d[image_name]["blur_scales"]]  # scale
                new_settings += [
                    cps.YES if d[image_name]["wants_saturation"] else cps.NO,
                    # check_saturation
                    cps.YES if d[image_name]["wants_intensity"]  else cps.NO,
                    # check_intensity
                    cps.YES if d[image_name]["wants_threshold"]  else cps.NO,
                    # calculate_threshold,
        ...
        if (not from_matlab) and variable_revision_number == 4:
            ...
            thresh_dict = dict(
                zip(cpthresh.TM_GLOBAL_METHODS, cpthresh.TM_METHODS))
            # Naturally, this method assumes that the user didn't name their images "Otsu Global" or something similar 
            setting_values = [
                thresh_dict[x] if x in cpthresh.TM_GLOBAL_METHODS else x for x
                in setting_values]
            variable_revision_number = 5
