        ...
        for image_name in [im.name for im in self.images]:
            for object_name in [obj.name for obj in self.objects]:
                for category, features in (
                        ...
                        (cellprofiler.modules.identify.C_LOCATION, ALL_LOCATION_MEASUREMENTS)):
    ...
    def get_categories(self, pipeline, object_name):
        ...
        for object_name_variable in [obj.name for obj in self.objects]:
            if object_name_variable.value == object_name:
                return [INTENSITY, cellprofiler.modules.identify.C_LOCATION]
        return []
        ...
        for image_name in [img.name for img in self.images]:
            ...
            for object_name in [obj.name for obj in self.objects]:
                ...
                for labels, lindexes in objects.labels():
                    ...
                    labels, img = cellprofiler.region.crop_labels_and_image(labels, img)
                    _, masked_image = cellprofiler.region.crop_labels_and_image(labels, masked_image)
                    outlines = centrosome.outline.outline(labels)

                    if image.has_mask:
                        _, mask = cellprofiler.region.crop_labels_and_image(labels, image.mask)
                    ...
                    if has_objects:
                        ...
                        mesh_y, mesh_x = numpy.mgrid[0:masked_image.shape[0],
                        ...
                        integrated_intensity[lindexes - 1] = \
                            centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum(limg, llabels, lindexes))
                        mean_intensity[lindexes - 1] = \
                            integrated_intensity[lindexes - 1] / lcount
                        std_intensity[lindexes - 1] = numpy.sqrt(
                                centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.mean((limg - mean_intensity[llabels - 1]) ** 2,
                                                                                                      llabels, lindexes)))
                        min_intensity[lindexes - 1] = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.minimum(limg, llabels, lindexes))
                        max_intensity[lindexes - 1] = centrosome.cpmorphology.fixup_scipy_ndimage_result(
                        ...
                        mass_displacement[lindexes - 1] = \
                            numpy.sqrt(diff_x * diff_x + diff_y * diff_y)
                        #
                    ...
                    has_edge = len(eimg) > 0
                    if has_edge:
                        ecount = centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum(
                                numpy.ones(len(eimg)), elabels, lindexes))
                        integrated_intensity_edge[lindexes - 1] = \
                            centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.sum(eimg, elabels, lindexes))
                        mean_intensity_edge[lindexes - 1] = \
                            integrated_intensity_edge[lindexes - 1] / ecount
                        std_intensity_edge[lindexes - 1] = \
                            numpy.sqrt(centrosome.cpmorphology.fixup_scipy_ndimage_result(scipy.ndimage.mean(
                                    (eimg - mean_intensity_edge[elabels - 1]) ** 2,
                                    elabels, lindexes)))
                        min_intensity_edge[lindexes - 1] = centrosome.cpmorphology.fixup_scipy_ndimage_result(
                                scipy.ndimage.minimum(eimg, elabels, lindexes))
                        max_intensity_edge[lindexes - 1] = centrosome.cpmorphology.fixup_scipy_ndimage_result(
                ...
                for category, feature_name, measurement in \
                        ((INTENSITY, INTEGRATED_INTENSITY, integrated_intensity),
                         ...
                         (cellprofiler.modules.identify.C_LOCATION, LOC_CMI_X, cmi_x),
                         (cellprofiler.modules.identify.C_LOCATION, LOC_CMI_Y, cmi_y),
                         (cellprofiler.modules.identify.C_LOCATION, LOC_MAX_X, max_x),
                         (cellprofiler.modules.identify.C_LOCATION, LOC_MAX_Y, max_y)):
