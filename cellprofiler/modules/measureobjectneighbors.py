...
class MeasureObjectNeighbors(cpm.CPModule):
    ...
    def run(self, workspace):
        ...
        if np.any(touching_border) and \
            ...
            neighbor_labels[touching_border_mask] = \
            touching_border_object_number[
        ...
        if nneighbors > (1 if self.neighbors_are_objects else 0):
            ...
            if nneighbors > first_neighbor + 1:
                ...
                second_x_vector = ncenters[second_object_index, 1] - ocenters[:,
                                                                     1]
                second_y_vector = ncenters[second_object_index, 0] - ocenters[:,
                                                                     0]
            ...
            if nkept_objects > (1 if self.neighbors_are_objects else 0):
                ...
                if self.neighbors_are_objects:
                    ...
                    if nkept_objects > 2:
                        second_object_number[has_pixels] = order[
                                                               has_pixels, 2] + 1
                else:
    ...
    def get_measurement_objects(self, pipeline, object_name, category,
        ...
        if (self.neighbors_are_objects or
                    measurement not in self.get_measurements(pipeline,
                                                             object_name,
                                                             category)):
            return []
