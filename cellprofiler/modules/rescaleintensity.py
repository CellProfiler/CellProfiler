...
class RescaleIntensity(cpm.CPModule):
    ...
    def create_settings(self):
        ...
        self.source_low = cps.Float('Lower intensity limit for the input image',
                                    0)
        ...
        self.dest_scale = cps.FloatRange('Intensity range for the output image',
                                         (0, 1))
        ...
        for w in workspace.pipeline.run_group_with_yield(
            ...
            if self.wants_automatic_high == HIGH_ALL_IMAGES:
                if image.has_mask:
                    ...
                    max_value = vmax if max_value is None else max(max_value,
                                                                   vmax)
            ...
            if self.wants_automatic_low == LOW_ALL_IMAGES:
                if image.has_mask:
                    ...
                    min_value = vmin if min_value is None else min(min_value,
                                                                   vmin)
