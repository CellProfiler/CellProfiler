...
class Resize(cpm.CPModule):
    ...
    def run(self, workspace):
        self.apply_resize(workspace, self.image_name.value,
                          self.resized_image_name.value)
        for additional in self.additional_images:
            self.apply_resize(workspace, additional.input_image_name.value,
    ...
    def display(self, workspace, figure):
        ...
        for i, (input_image_pixels, output_image_pixels, input_image_name,
                output_image_name) in \
                enumerate(zip(input_images, output_images, input_image_names,
                              output_image_names)):
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        if (not from_matlab) and variable_revision_number == 3:
            ...
            setting_values = setting_values[:7] + [C_MANUAL,
                                                   cps.NONE] + setting_values[
                                                               7:]
            variable_revision_number = 4
