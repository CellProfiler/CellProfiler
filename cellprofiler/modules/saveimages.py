...
store images in the subfolder, "*date*\/*plate-name*".""",
    ...
    def visible_settings(self):
        ...
        supports_16_bit = (self.file_format in (FF_TIFF, FF_H5) and self.save_image_or_figure == IF_IMAGE) or \
    ...
    def save_image(self, workspace):
        ...
        if self.file_format == FF_NPY:
            ...
            if (not image.volumetric and len(pixels.shape) > 2 and
                pixels.shape[2] > 4 and self.file_format.value != FF_H5):
    ...
    def file_name_feature(self):
        return "_".join((cellprofiler_core.measurement.C_FILE_NAME, self.image_name.value))

    @property
    def path_name_feature(self):
        return "_".join((cellprofiler_core.measurement.C_PATH_NAME, self.image_name.value))
    ...
    def get_bit_depth(self):
        if (self.save_image_or_figure == IF_IMAGE and
            self.get_file_format() in (FF_TIFF, FF_H5)):
