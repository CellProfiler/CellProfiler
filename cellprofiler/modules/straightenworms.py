...
class StraightenWorms(cpm.CPModule):
    ...
    def measure_worms(self, workspace, labels, nworms, width):
        ...
        if nworms == 0:
            ...
            if nbins_horizontal > 1:
                ...
                if nbins_vertical > 1:
                    scales = np.array([
                                          [self.get_scale_name(h, v) for h in
                                           range(nbins_horizontal)]
                                          for v in range(nbins_vertical)])
            ...
            for i_dim, j_dim, scales in griddings:
                ...
                if not np.all(easy):
                    ...
                    #
                    i_span = np.maximum(
                        np.floor(i_mapping_end) - np.ceil(i_mapping), 0)
                    j_span = np.maximum(
                        np.floor(j_mapping_end) - np.ceil(j_mapping), 0)
                    #
                    ...
                    #
                    i_high_frac[(
                    i_mapping.astype(int) == i_mapping_end.astype(int))] = 1
                    j_high_frac[(
                    j_mapping.astype(int) == j_mapping_end.astype(int))] = 1
                    #
                    ...
                    #
                    i_count = (
                    np.ceil(i_mapping_end) - np.floor(i_mapping)).astype(int)
                    j_count = (
                    np.ceil(j_mapping_end) - np.floor(j_mapping)).astype(int)
                    #
                    ...
                    #
                    i_dest_hard = i_mapping[idx.rev_idx].astype(int) + idx.idx[
                        0]
                    j_dest_hard = j_mapping[idx.rev_idx].astype(int) + idx.idx[
                        1]
                    #
    ...
    def display(self, workspace, figure):
        ...
        src_axis = None
        for i, ((src_pix, src_name), (dest_pix, dest_name)) in enumerate(
                image_pairs):
            if src_pix.ndim == 2:
    ...
    def handle_interaction(self, straightened_images, labels, image_set_number):
        ...
        with wx.Dialog(None, -1,
            ...
            outline_indexes = np.hstack(
                ([0], np.cumsum(np.bincount(outline_l))))
            ...
            def refresh():
                ...
                if len(straightened_images) == 1:
                    ...
                    image[labels == 0, :] = 1
                    for i, straightened_image in enumerate(
                            straightened_images[:3]):
