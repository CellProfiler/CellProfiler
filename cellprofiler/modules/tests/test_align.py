...
class TestAlign(unittest.TestCase):
    ...
    def test_01_02_load_v1(self):
        ...
        (1, module.first_input_image, module.first_output_image),
        ...
        self.assertEqual(module.additional_images[0].align_choice.value,
                         A.A_SIMILARLY)
        self.assertEqual(module.additional_images[1].align_choice.value,
                         A.A_SEPARATELY)
...
"""
        ...
        (3, 5), (-3, 5), (3, -5), (-3, -5), (0, 5), (3, 0), (0, 0)):
            ...
            for mask1 in (None,
                ...
                for mask2 in (None,
                              np.random.uniform(size=shape) > .1):
                    for method in (
                    A.M_MUTUAL_INFORMATION, A.M_CROSS_CORRELATION):
                        if method == A.M_CROSS_CORRELATION and (
                                    (mask1 is not None) or (mask2 is not None)):
                            continue

                        image1 = np.random.randint(0, 10, size=shape).astype(
                            float) / 10.0
                        ...
                        if method == A.M_MUTUAL_INFORMATION:
                            ...
                            image2 += (np.random.uniform(
                                size=shape) - .5) * .1 * np.std(image2)
                        ...
                        i_slice = self.single_slice_helper(-off_i0,
                                                           out_shape[0])
                        j_slice = self.single_slice_helper(-off_j0,
                                                           out_shape[1])
                        ...
                        i_slice = self.single_slice_helper(-off_i1,
                                                           out_shape[0])
                        j_slice = self.single_slice_helper(-off_j1,
                                                           out_shape[1])
    ...
    def test_02_02_pad(self):
        ...
        i, j = np.mgrid[0:shape[0], 0:shape[1]]
        for offset in (
        (1, 0), (0, 1), (1, 1), (3, 5), (-3, 5), (3, -5), (-3, -5), (0, 5),
        (3, 0), (0, 0)):
            for mask1 in (None,
                          np.random.uniform(size=shape) > .1):
                for mask2 in (None,
                    ...
                    A.M_MUTUAL_INFORMATION, A.M_CROSS_CORRELATION):
                        ...
                        if mask1 is not None:
                            self.assertTrue(
                                np.all(output.mask[i_slice, j_slice] == mask1))
                        ...
                        if mask2 is not None:
                            self.assertTrue(
                                np.all(mask2 == output.mask[i_slice, j_slice]))
                        temp = output.mask.copy()
                        temp[i_slice, j_slice] = False
                        self.assertTrue(np.all(~ temp))
                    ...
                    A.M_MUTUAL_INFORMATION, A.M_CROSS_CORRELATION):
                        ...
                        if mask1 is not None:
                            self.assertTrue(np.all(
                                output.mask[si_out, sj_out] == mask1[
                                    si_in, sj_in]))
    ...
    def test_03_01_align_similarly(self):
        ...
        for offset in ((3, 5), (-3, 5), (3, -5), (-3, -5)):
            ...
            image1[np.sqrt(
                ((i - shape[0] / 2) ** 2 + (j - shape[1] / 2) ** 2)) < 20] = .5
            si1, si2 = self.slice_helper(offset[0], image1.shape[0])
            sj1, sj2 = self.slice_helper(offset[1], image1.shape[1])
            image2 = np.zeros(image1.shape)
                ...
                for mask2 in (None,
                    ...
                    A.M_MUTUAL_INFORMATION, A.M_CROSS_CORRELATION):
                        if method == A.M_CROSS_CORRELATION and (
                                    (mask1 is not None) or (mask2 is not None)):
                            continue
                        image1 = np.random.randint(0, 1, size=shape).astype(
                            bool)
                        image1[np.sqrt(((i - shape[0] / 2) ** 2 + (
                        j - shape[1] / 2) ** 2)) < 10] = True
                        si1, si2 = self.slice_helper(offset[0], image1.shape[0])
                        sj1, sj2 = self.slice_helper(offset[1], image1.shape[1])
    ...
    def test_05_01_align_self(self):
        ...
        m = workspace.measurements
        self.assertEqual(
            m.get_current_image_measurement('Align_Xshift_Aligned1'), 0)
        self.assertEqual(
