...
"""
    ...
    def test_02_02_add_image_object(self):
        ...
        measurements = self.run_workspace(C.O_ADD, True, 2, False,
                                          np.array([1, 4, 9]))
        self.assertFalse(
            measurements.has_feature(cpmeas.IMAGE, MATH_OUTPUT_MEASUREMENTS))
        self.assertTrue(
            measurements.has_feature(OBJECT[1], MATH_OUTPUT_MEASUREMENTS))
        data = measurements.get_current_measurement(OBJECT[1],
                                                    MATH_OUTPUT_MEASUREMENTS)
    ...
    def test_02_03_add_object_image(self):
        ...
        measurements = self.run_workspace(C.O_ADD, False, np.array([1, 4, 9]),
                                          True, 2)
        self.assertFalse(
            measurements.has_feature(cpmeas.IMAGE, MATH_OUTPUT_MEASUREMENTS))
        self.assertTrue(
            measurements.has_feature(OBJECT[0], MATH_OUTPUT_MEASUREMENTS))
    ...
    def test_02_09_add_object_object(self):
        ...
        for i in range(2):
            self.assertTrue(
                measurements.has_feature(OBJECT[i], MATH_OUTPUT_MEASUREMENTS))
            data = measurements.get_current_measurement(OBJECT[i],
                                                        MATH_OUTPUT_MEASUREMENTS)
    ...
    def test_10_1_img_1566(self):
        ...
        expected1 = [np.array([2.5, 6.5, 12]),
                     ...
                     np.array([7]),
                     np.array([5, 6, np.nan, 8, 9])]
        for oo0, oo1, ii0, ii1, e0, e1 in zip(o0, o1, in0, in1, expected0,
                                              expected1):
