...
class TestCorrectIlluminationApply(unittest.TestCase):
    ...
    def test_02_01_stretch(self):
        ...
        pipeline.add_module(module)
        i = module.images[0]
        i.image_name.value = "InputImage"
        i.illum_correct_function_image_name.value = "IllumImage"
        i.corrected_image_name.value = "OutputImage"
        i.divide_or_subtract.value = cpmcia.DOS_DIVIDE
        i.rescale_option = cpmcia.RE_STRETCH
        image_set_list = pipeline.prepare_run(None)
        input_module.prepare_group(pipeline, image_set_list, {}, [1])
        illum_module.prepare_group(pipeline, image_set_list, {}, [1])
        module.prepare_group(pipeline, image_set_list, {}, [1])
    ...
    def test_02_01_match(self):
        ...
        i.corrected_image_name.value = "OutputImage"
        i.divide_or_subtract.value = cpmcia.DOS_DIVIDE
        i.rescale_option = cpmcia.RE_MATCH
        image_set_list = pipeline.prepare_run(None)
        input_module.prepare_group(pipeline, image_set_list, {}, [1])
        illum_module.prepare_group(pipeline, image_set_list, {}, [1])
        module.prepare_group(pipeline, image_set_list, {}, [1])
