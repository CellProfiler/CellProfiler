...
class TestCrop(unittest.TestCase):
    def make_workspace(self, input_pixels, crop_image=None, cropping=None, crop_objects=None):
        ...
        if cropping is not None:
            image_set.add(CROPPING, cellprofiler.image.Image(numpy.zeros(cropping.shape),
                                                             crop_mask=cropping))
        ...
        workspace = cellprofiler.workspace.Workspace(pipeline,
                                                     ...
                                                     m,
                                                     image_set_list)
        m.add_measurement(cellprofiler.measurement.IMAGE, cellprofiler.measurement.GROUP_INDEX, 0, image_set_number=1)
    ...
    def test_00_00_zeros(self):
        ...
        self.assertTrue(numpy.all(output_image.pixel_data == 0))
        self.assertTrue(numpy.all(output_image.mask == output_image.pixel_data))
        self.assertTrue(numpy.all(output_image.crop_mask == output_image.pixel_data))
        m = workspace.measurements
        self.assertTrue('Image' in m.get_object_names())
        columns = module.get_measurement_columns(workspace.pipeline)
        self.assertEqual(len(columns), 2)
        self.assertTrue(all([x[0] == cellprofiler.measurement.IMAGE for x in columns]))
        self.assertTrue(all([x[2] == cellprofiler.measurement.COLTYPE_INTEGER for x in columns]))
    ...
    def test_00_01_zeros_and_remove_all(self):
        ...
        module.run(workspace)
        output_image = workspace.image_set.get_image(OUTPUT_IMAGE)
        self.assertEqual(numpy.product(output_image.pixel_data.shape), 0)
    ...
    def test_01_01_crop_edges_with_image(self):
        ...
        module.run(workspace)
        output_image = workspace.image_set.get_image(OUTPUT_IMAGE)
        self.assertTrue(numpy.all(output_image.pixel_data == expected_image))
    ...
    def test_05_03_crop_color_image_plate_fixup(self):
        x, y, z = numpy.mgrid[0:10, 0:10, 0:3]
        input_image = (x / 100.0 + y / 10.0 + z / 1000.0).astype(numpy.float32)
        crop_image = numpy.zeros((10, 10), bool)
        crop_image[2:, 1:9] = True
        crop_image[1, (1, 4)] = True  # A rough edge to be cropped
        expected_image = input_image[2:, 1:9.:]
    ...
    def test_06_01_mask_with_objects(self):
        ...
        self.assertTrue(numpy.all(input_objects == output_image.labels))
        self.assertTrue(numpy.all(output_image.mask == (input_objects > 0)))
    ...
    def test_06_02_crop_with_objects(self):
        ...
        output_image = workspace.image_set.get_image(OUTPUT_IMAGE)
        self.assertTrue(output_image.has_masking_objects)
        self.assertTrue(numpy.all(input_objects[2:17, 3:8] == output_image.labels))
        self.assertTrue(numpy.all(output_image.mask == (input_objects[2:17, 3:8] > 0)))
        self.assertTrue(numpy.all(output_image.crop_mask == (input_objects > 0)))
    ...
    def test_07_02_load_v2(self):
        ...
        self.assertTrue(all([isinstance(module, cellprofiler.modules.crop.Crop)
        ...
        self.assertEqual(module.shape.value, cellprofiler.modules.crop.SH_ELLIPSE)
        self.assertEqual(module.individual_or_once, cellprofiler.modules.crop.IO_FIRST)
        ...
        self.assertEqual(module.shape.value, cellprofiler.modules.crop.SH_CROPPING)
