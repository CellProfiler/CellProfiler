    ...
    def test_02_01_grid_automatic(self):
        ...
        for i in range(rows):
            for j in range(columns):
                ...
                labels[(ii - center_i) ** 2 + (
                jj - center_j) ** 2 <= 9] = i * columns + j + 1
        ...
        self.assertTrue(np.all(
            gridding.x_locations == first_x + np.arange(columns) * spacing_x))
        self.assertTrue(np.all(
            gridding.y_locations == first_y + np.arange(rows) * spacing_y))
        ...
        for feature, value in ((D.F_COLUMNS, columns),
            ...
            self.assertEqual(m.get_current_image_measurement(measurement),
                             value)
