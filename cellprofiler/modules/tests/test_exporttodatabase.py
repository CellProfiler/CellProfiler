...
"""
        ...
        self.assertEqual(module.properties_image_url_prepend,
                         "http://server.university.edu")
        ...
        self.assertEqual(g.group_statement,
...
"""
    ...
    def make_workspace(self, wants_files, alt_object=False,
        ...
        class TestModule(cpm.CPModule):
            ...
            def create_settings(self):
                ...
                if alt_object:
                    self.altobjects_name = cps.ObjectNameProvider("Baz",
                                                                  ALTOBJECT_NAME)
            ...
            def get_measurement_columns(self, pipeline):
                ...
                if self.in_module(long_measurement):
                    columns += [(cpmeas.IMAGE, LONG_IMG_MEASUREMENT,
                                 cpmeas.COLTYPE_INTEGER),
                                (OBJECT_NAME, LONG_OBJ_MEASUREMENT,
                                 cpmeas.COLTYPE_FLOAT)]
                if self.in_module(wierd_measurement):
                    columns += [(cpmeas.IMAGE, WIERD_IMG_MEASUREMENT,
                                 cpmeas.COLTYPE_INTEGER),
                                (OBJECT_NAME, WIERD_OBJ_MEASUREMENT,
                                 cpmeas.COLTYPE_FLOAT)]
                if self.in_module(well_metadata):
                    columns += [
                        (cpmeas.IMAGE, "Metadata_Plate",
                         cpmeas.COLTYPE_VARCHAR_FORMAT % 20),
                        (cpmeas.IMAGE, "Metadata_Well",
                         cpmeas.COLTYPE_VARCHAR_FORMAT % 3)]
                if self.in_module(group_measurement):
                    d = {cpmeas.MCA_AVAILABLE_POST_GROUP: True}
                    columns += [
                        (cpmeas.IMAGE, GROUP_IMG_MEASUREMENT,
                         cpmeas.COLTYPE_INTEGER, d),
                        (OBJECT_NAME, GROUP_OBJ_MEASUREMENT,
                         cpmeas.COLTYPE_FLOAT, d)]
                if post_run_test:
                ...
                if category == M_CATEGORY:
                    if object_name == OBJECT_NAME:
                        if self.in_module(long_measurement):
                            ...
                            return [OBJ_FEATURE]
                    elif (object_name == ALTOBJECT_NAME) and self.in_module(
                            alt_object):
                        return [OBJ_FEATURE]
                    else:
                        return ([INT_IMG_FEATURE, FLOAT_IMG_FEATURE,
                                 STRING_IMG_FEATURE] +
                                [LONG_IMG_FEATURE] if self.in_module(
                            long_measurement)
                                else [WIERD_IMG_FEATURE] if self.in_module(
                            wierd_measurement)
                        else [])
                elif category == I.C_NUMBER and object_name in (
        ...
        for i in range(image_set_count):
            ...
            if TestModule.in_measurements(long_measurement):
                ...
                m.add_measurement(OBJECT_NAME, LONG_OBJ_MEASUREMENT,
                                  OBJ_VALUE.copy())
            if TestModule.in_measurements(wierd_measurement):
                m.add_image_measurement(WIERD_IMG_MEASUREMENT, 100)
                m.add_measurement(OBJECT_NAME, WIERD_OBJ_MEASUREMENT,
                                  OBJ_VALUE.copy())
            ...
            if TestModule.in_measurements(group_measurement):
                ...
                m.add_measurement(OBJECT_NAME, GROUP_OBJ_MEASUREMENT,
                                  OBJ_VALUE.copy())
        r = np.random.RandomState()
        r.seed(image_set_count)
        if relationship_test_type == self.RTEST_SOME:
            n = 10
            i1, o1 = [x.flatten() for x in
                      np.mgrid[1:(image_set_count + 1), 1:(n + 1)]]
        ...
        module.wants_relationship_table_setting.value = (
    ...
    def load_database(self, output_dir, module, image_set_count=1):
        ...
        try:
            ...
            image_file = os.path.join(output_dir,
                                      base_name + "_" + cpmeas.IMAGE + ".CSV")
            if module.separate_object_tables == E.OT_PER_OBJECT:
                object_file = os.path.join(output_dir,
                                           base_name + "_" + OBJECT_NAME + ".CSV")
            else:
    ...
    def drop_tables(self, module, table_suffixes=None):
        ...
        for info_table, thing in (("VIEWS", "view"), ("TABLES", "table")):
            statement = (
                            ...
                            info_table, module.db_name.value,
                            module.table_prefix.value)
            self.cursor.execute(statement)
            for (table_name,) in self.cursor.fetchall():
                self.assertTrue(
                    table_name.startswith(module.table_prefix.value))
    ...
    def test_02_01_write_mysql_db(self):
        ...
        try:
            ...
            finally_fn()
            self.drop_tables(module,
                             ("Per_Image", "Per_Object", "Per_Experiment"))
    ...
    def test_02_06_write_direct_wierd_colname(self):
        ...
        try:
            ...
            wierd_obj_column = mappings[
                "%s_%s" % (OBJECT_NAME, WIERD_OBJ_MEASUREMENT)]
    ...
    def test_03_01_write_sqlite_direct(self):
        ...
        for with_interaction_handler in (False, True):
            ...
            try:
                ...
                self.check_experiment_table(cursor, module,
                ...
                if hasattr(module, "cursor") and module.cursor is not None:
                    module.cursor.close()
                if hasattr(module,
                           "connection") and module.connection is not None:
    ...
    def test_05_01_write_mysql_db(self):
        ...
        try:
            ...
            module.post_run(workspace)
            self.load_database(output_dir, module)
            self.check_experiment_table(self.cursor, module,
                                        workspace.measurements)
            #
            # Now read the image file from the database
            #
    ...
    def test_05_11_data_tool_and_get_measurement_columns(self):
        ...
        try:
            ...
            long_mean_col = mappings[
                "Mean_%s_%s" % (OBJECT_NAME, LONG_OBJ_MEASUREMENT)]
            long_obj_col = mappings[
                "%s_%s" % (OBJECT_NAME, LONG_OBJ_MEASUREMENT)]
            self.cursor.execute("use CPUnitTest")
            #
            # Now read the image file from the database
    ...
    def test_08_01_image_thumbnails(self):
        ...
        try:
            ...
            module.thumbnail_image_names.value = \
            ...
            image_table = module.table_prefix.value + "Per_Image"
            stmt = "select Image_%s from %s" % (
            expected_thumbnail_column, image_table)
    ...
    def test_10_01_properties_file(self):
        ...
        def get_measurement_columns(
                self, pipeline,
                old_get_measurement_columns=old_get_measurement_columns):
            result = [(cpmeas.IMAGE,
        ...
        try:
            ...
            for k, v in (
                    ...
                    ("image_path_cols",
                     "%s_%s_%s" % (cpmeas.IMAGE, E.C_PATH_NAME, IMAGE_NAME)),
                    ("image_file_cols",
                     "%s_%s_%s" % (cpmeas.IMAGE, E.C_FILE_NAME, IMAGE_NAME))):
