...
"""
    ...
    def test_01_04_img_887_no_experiment_file(self):
        ...
        m.add_all_measurements(cpmeas.IMAGE, "my_measurement",
                               image_measurements)
        image_set_list = cpi.ImageSetList()
        image_set = image_set_list.get_image_set(0)
        object_set = cpo.ObjectSet()
    ...
    def test_02_02_three_by_two_image_measurements(self):
        ...
        for i in range(2):
            if i:
                m.next_image_set()
            for j in range(3):
                m.add_image_measurement("measurement_%d" % j,
                                        "%d:%d" % (i, j))
        object_set = cpo.ObjectSet()
        workspace = cpw.Workspace(self.make_measurements_pipeline(m),
                                  module,
    ...
    def test_03_02_three_by_two_object_measurements(self):
        ...
        for i in range(3):
            m.add_measurement("my_object", "measurement_%d" % i,
                              mvalues[:, i])
    ...
    def test_03_03_get_measurements_from_two_objects(self):
        ...
        try:
            ...
            for oidx in range(2):
                for i in range(3):
                    self.assertEqual(header[i + oidx * 3 + 2],
                                     "object_%d" % oidx)
            header = reader.next()
            self.assertEqual(len(header), 8)
            self.assertEqual(header[0], "ImageNumber")
            ...
            for i in range(4):
                ...
                for j in range(3):
                    for k in range(2):
                        self.assertAlmostEqual(float(row[k * 3 + j + 2]),
                                               mvalues[i, j, k])
            self.assertRaises(StopIteration, reader.next)
        finally:
            fd.close()
    ...
    def test_04_01_01_object_with_metadata(self):
        ...
        image_set_list = cpi.ImageSetList()
        for index, measurement, metadata in zip(range(4), mvalues,
                                                ('foo', 'bar', 'bar', 'foo')):
            image_set = image_set_list.get_image_set(index)
            m.add_measurement("my_object", "my_measurement",
    ...
    def test_05_03_aggregate_and_filtered(self):
        ...
        try:
            ...
            self.assertEqual(row[d["Count_my_objects"]], "6")
            self.assertAlmostEqual(float(row[d["first_measurement"]]),
                                   np.sum(data))
            self.assertAlmostEqual(
                float(row[d["Mean_my_objects_my_measurement"]]),
                np.mean(data))
            self.assertRaises(StopIteration, reader.next)
        finally:
            fd.close()
        try:
            ...
            for index, row in enumerate(reader):
                ...
                self.assertEqual(int(float(row[d["Number_Object_Number"]])),
                                 index + 1)
                self.assertAlmostEqual(float(row[d["my_measurement"]]),
                                       data[index])
        finally:
    ...
    def test_06_02_object_index_columns(self):
        ...
        for image_idx in range(mvalues.shape[0]):
            ...
            m.add_measurement("my_objects", "my_measurement",
                              mvalues[image_idx, :])
        image_set_list = cpi.ImageSetList()
        image_set = image_set_list.get_image_set(0)
        object_set = cpo.ObjectSet()
        ...
        try:
            ...
            for image_idx in range(mvalues.shape[0]):
                for object_idx in range(mvalues.shape[1]):
                    ...
                    self.assertEqual(row[d["Metadata_Well"]],
                                     "C0%d" % (image_idx + 1))
                    self.assertAlmostEqual(float(row[d["my_measurement"]]),
                                           mvalues[image_idx, object_idx], 4)
            self.assertRaises(StopIteration, reader.next)
...
''' % info
    ...
    def test_08_02_make_gct_file_with_filename(self):
        ...
        metadata_name = "Metadata_Bar"
        info = (
        'Image_FileName_Foo', 'Image_PathName_Foo', metadata_name, input_dir,
        input_dir)
        csv_text = '''"%s","%s","%s"
"Channel1-01-A-01.tif","%s","Hi"
"Channel1-02-A-02.tif","%s","Hello"
