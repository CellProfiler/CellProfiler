...
"""
        ...
        self.assertTrue(isinstance(module, cellprofiler.modules.groups.Groups))
...
"""
    ...
    def make_image_sets(self, key_metadata, channel_metadata):
        ...
        """
        iscds = [cellprofiler.pipeline.Pipeline.ImageSetChannelDescriptor(channel_name,
                                                                          image_type)
        ...
        r.seed(numpy.frombuffer("".join(["%s=%s" % kv for kv in key_metadata]), numpy.uint8))
        image_sets = [image_sets[i] for i in r.permutation(len(image_sets))]

        m = cellprofiler.measurement.Measurements()
        m.set_metadata_tags(["_".join((cellprofiler.measurement.C_METADATA, k))
        ...
        for i, ipds in enumerate(image_sets):
            ...
            for (file_name, metadata), iscd in zip(ipds, iscds):
                for feature, value in (
                    ...
                    m[cellprofiler.measurement.IMAGE, feature + "_" + iscd.name, image_number] = value
    ...
    def test_02_00_compute_no_groups(self):
        groups, workspace = self.make_image_sets(
                (("Plate", ("P-12345", "P-23456")),
                 ("Well", ("A01", "A02", "A03")),
                 ("Site", ("1", "2", "3", "4"))),
                (("DNA", "Wavelength", "1", cellprofiler.pipeline.Pipeline.ImageSetChannelDescriptor.CT_GRAYSCALE),
                 ("GFP", "Wavelength", "1", cellprofiler.pipeline.Pipeline.ImageSetChannelDescriptor.CT_GRAYSCALE)))
        groups = cellprofiler.modules.groups.Groups()
        groups.wants_groups.value = False
        m = workspace.measurements
        assert isinstance(m, cellprofiler.measurement.Measurements)
        image_numbers = m.get_image_numbers()
        expected_file_names = m[cellprofiler.measurement.IMAGE, cellprofiler.measurement.C_FILE_NAME + "_" + "DNA", image_numbers]
        self.assertTrue(groups.prepare_run(workspace))
        self.assertEqual(len(image_numbers), 2 * 3 * 4)
        output_file_names = m[cellprofiler.measurement.IMAGE, cellprofiler.measurement.C_FILE_NAME + "_" + "DNA", image_numbers]
        ...
        for group_number, plate, (grouping, image_set_list) in zip(
            ...
            for image_number in range(1 + (group_number - 1) * 12,
                ...
                for image_name in ("DNA", "GFP"):
                    ...
                    self.assertTrue(
                            m[cellprofiler.measurement.IMAGE, ftr, image_number].startswith(plate))
    ...
    def test_03_02_get_measurement_columns_groups(self):
        ...
        column = columns[0]
        self.assertEqual(column[0], cellprofiler.measurement.EXPERIMENT)
        self.assertEqual(column[1], cellprofiler.measurement.M_GROUPING_TAGS)
        self.assertTrue(column[2].startswith(cellprofiler.measurement.COLTYPE_VARCHAR))
        column_metadata = []
        for column in columns[1:]:
            self.assertEqual(column[0], cellprofiler.measurement.IMAGE)
            self.assertEqual(column[2], cellprofiler.measurement.COLTYPE_VARCHAR)
            column_metadata.append(column[1])
        for choice in choices:
            self.assertTrue(cellprofiler.measurement.C_METADATA + "_" + choice in column_metadata)
