...
class TestIdentifyObjectsInGrid(unittest.TestCase):
    ...
    def make_rectangular_grid(self, gridding):
        ...
        g[mask[:g.shape[0], :g.shape[1]]] = gridding.spot_table[
            i[mask], j[mask]]
        return grid
    ...
    def test_02_01_forced_location(self):
        ...
        self.assertTrue(all([column[1] in (
        'Location_Center_X', 'Location_Center_Y', count_feature,
        ...
        self.assertTrue(np.all(outlines == (
        expected_outlines[0:outlines.shape[0], 0:outlines.shape[1]] > 0)))
        #
    ...
    def test_02_02_forced_location_auto(self):
        ...
        mask[np.abs(np.sqrt(idist ** 2 + jdist ** 2) - float(
            diameter + 1) / 2) <= 1] = False
        #
        ...
        self.assertTrue(np.all(labels[mask] ==
                               expected[0:labels.shape[0], 0:labels.shape[1]][
                                   mask]))
    ...
    def test_03_01_natural_circle(self):
        ...
        np.random.seed(0)
        x_locations += (
        np.random.uniform(size=x_locations.shape[0]) * 3 - 1).astype(int)
        y_locations += (
        np.random.uniform(size=y_locations.shape[0]) * 3 - 1).astype(int)
        random_diameters = np.random.uniform(
            size=y_locations.shape[0] + 1) * 4 * 3
        idist = (i - y_locations[expected])
        jdist = (j - x_locations[expected])
        guide_labels = expected.copy()
        expected[idist ** 2 + jdist ** 2 > (float(diameter + 1) / 2) ** 2] = 0
    ...
    def test_03_02_natural_circle_edges(self):
        ...
        for i_off in (-1, 0, 1):
            for j_off in (-1, 0, 1):
                guide_labels[
                    bad_y_locations + i_off, bad_x_locations + j_off] = np.arange(
        ...
        for i_off in (-1, 0, 1):
            for j_off in (-1, 0, 1):
                guide_labels[
                    bad_y_locations + i_off, bad_x_locations + j_off] = np.arange(
                    len(
                        bad_y_locations)) + gridding.rows * gridding.columns * 2 + 1
        #
