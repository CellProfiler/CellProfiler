...
"""
        ...
        module = pipeline.modules()[0]
        self.assertTrue(isinstance(module, I.ImageMath))
        self.assertEqual(len(module.images),
                         2)  # there are two, but only 1 shown
...
"""
        ...
        self.assertEqual(module.images[1].image_or_measurement,
                         I.IM_MEASUREMENT)
...
"""
    ...
    def test_03_02_add_mask(self):
        ...
        np.random.seed(0)
        for n in range(2, 5):
            images = [{'pixel_data': np.random.uniform(size=(50, 50)).astype(
                np.float32),
    ...
    def test_03_04_add_crop(self):
        ...
        for n in range(2, 3):
            for m in range(n):
                images = [{'pixel_data': np.random.uniform(
                    size=(20, 20)).astype(np.float32)}
                          for i in range(n)]
