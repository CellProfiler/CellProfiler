...
'''
        ...
        m = pipeline.run()
        data = m.get_current_image_measurement("Test_Measurement")
        self.assertTrue(isinstance(data, np.int),
...
'''
        ...
        self.assertTrue(isinstance(data, unicode),
                        "Expected <type 'unicode'> got %s" % type(data))
...
'''
        ...
        for feature, expected in (("Row", "C"),
            ...
            value = m.get_current_image_measurement(
                '_'.join((cpmeas.C_METADATA, feature)))
...
""" % (L.C_OBJECTS_FILE_NAME, OBJECTS_NAME,
       L.C_OBJECTS_PATH_NAME, OBJECTS_NAME)
        ...
        for column in columns:
            self.assertTrue(any([
                                    True for object_name, feature in
                                    expected_columns
                                    if object_name == column[0] and feature ==
                                    column[1]]))
