...
class testLoadImages(unittest.TestCase, ConvtesterMixin):
    ...
    def test_01_02load_image_text_match_many(self):
        ...
        for i in range(0, 4):
            ...
            l.images[i].channels[
                0].image_name.value = "my_image%(i)d" % locals()
        l.module_num = 1
        image_set_list = I.ImageSetList()
        pipeline = P.Pipeline()
        pipeline.add_module(l)
            ...
            for image_count in range(2, group_size + 1):
                ...
                for indexes in permutations(range(group_size), image_count):
                    ...
                    for i, index in enumerate(indexes[1:]):
                        ...
                        l.images[i + 1].channels[
                            0].image_name.value = "image%d" % (i + 2)
                    l.module_num = 1
                    image_set_list = I.ImageSetList()
                    pipeline = P.Pipeline()
                    pipeline.add_module(l)
...
"""
    ...
    def test_03_03_load_new_version_3(self):
        ...
        self.assertEqual(module.images[0].channels[0].image_name, 'DNA')
        self.assertEqual(module.images[0].common_text, 'Channel2')
        self.assertEqual(module.images[0].file_metadata,
                         '^.*-(?P<Row>.+)-(?P<Col>[0-9]{2})')
        self.assertEqual(module.images[1].channels[0].image_name, 'Cytoplasm')
        self.assertEqual(module.images[1].common_text, 'Channel1')
        self.assertEqual(module.images[1].file_metadata,
                         '^.*-(?P<Row>.+)-(?P<Col>[0-9]{2})')
        self.assertEqual(module.location.dir_choice,
                         LI.DEFAULT_INPUT_FOLDER_NAME)
    ...
    def test_03_04_load_new_version_4(self):
        ...
        self.assertEqual(module.images[0].file_metadata,
                         '^Channel[12]-[0-9]{2}-(?P<ROW>[A-H])-(?P<COL>[0-9]{2})')
        self.assertEqual(module.location.dir_choice,
                         LI.DEFAULT_INPUT_FOLDER_NAME)
...
"""
        ...
        self.assertEqual(module.images[0].path_metadata,
                         r".*[\\/](?P<Date>.*)$")
        self.assertEqual(module.images[1].channels[0].image_name, "Cytoplasm")
        ...
        self.assertEqual(module.location.custom_path,
                         "/imaging/analysis/People/Lee")
        ...
        self.assertTrue(isinstance(module, LI.LoadImages))
        self.assertEqual(module.location.dir_choice,
                         LI.DEFAULT_OUTPUT_SUBFOLDER_NAME)
...
"""
        ...
        self.assertEqual(module.descend_subdirectories, LI.SUB_NONE)
        self.assertEqual(module.location.dir_choice,
                         LI.cps.DEFAULT_INPUT_FOLDER_NAME)
        self.assertTrue(module.check_images)
        self.assertTrue(module.group_by_metadata)
...
"""
        ...
        for channel, choice, channel_number, image_name, object_name, rescale in (
                ...
                (image.channels[1], LI.IO_OBJECTS, 3, "Protein", "Cytoplasm",
                 False)):
...
"""
    ...
    def test_05_01_load_PNG(self):
        ...
        class CheckImage(CPM.CPModule):
            ...
            def run(self, workspace):
                ...
                check_image = np.fromstring(check_data, np.uint8).reshape(
                    T.raw_8_1_shape)
    ...
    def test_05_09_load_C01(self):
        ...
        self.assertEqual(m.digest(),
                         'SER\r\xc4\xd5\x02\x13@P\x12\x99\xe2(e\x85')
    ...
    def test_06_01_file_metadata(self):
        ...
        load_images.run(w)
        self.assertEqual(
            image_set.get_image_provider("Channel1").get_filename(),
            filenames[0])
        self.assertEqual(
            image_set.get_image_provider("Channel2").get_filename(),
            filenames[1])
        self.assertEqual(m.get_current_measurement("Image", "Metadata_plate"),
                         "MMD-ControlSet-plateA-2008-08-06")
        self.assertEqual(
            m.get_current_measurement("Image", "Metadata_well_row"),
            "A")
        self.assertEqual(
            m.get_current_measurement("Image", "Metadata_well_col"),
            "12")
        ...
        try:
            ...
            try:
                os.symlink(os.path.join(directory, filenames[0][0]),
                           os.path.join(directory, filenames[-1][0],
                                        filenames[0][0]))
            except Exception, e:
    ...
    def test_07_04_get_movie_measurements(self):
        ...
        for ft in (LI.FF_AVI_MOVIES, LI.FF_STK_MOVIES):
            ...
            module.images[
                0].file_metadata.value = "^.*-(?P<WellRow>.+)-(?P<WellCol>[0-9]{2})"
            module.images[
                0].path_metadata.value = "(?P<Year>[0-9]{4})-(?P<Month>[0-9]{2})-(?P<Day>[0-9]{2})"
    ...
    def test_07_06_get_object_measurement_columns(self):
        ...
        for object_name, feature in (
                (measurements.IMAGE,
                 LI.C_OBJECTS_FILE_NAME + "_" + OBJECTS_NAME),
                (measurements.IMAGE,
                 LI.C_OBJECTS_PATH_NAME + "_" + OBJECTS_NAME),
    ...
    def test_07_08_get_object_measurements(self):
        ...
        for object_name, expected in (
                ...
                (OBJECTS_NAME, (
                        (LI.I.C_LOCATION,
                         [LI.I.FTR_CENTER_X, LI.I.FTR_CENTER_Y]),
    ...
    def test_08_01_get_groupings(self):
        ...
        module.images[
            0].file_metadata.value = '^Channel1-[0-9]{2}-(?P<ROW>[A-H])-(?P<COL>[0-9]{2})'
        ...
        for grouping in groupings:
            ...
            for image_number in grouping[1]:
                ...
                self.assertTrue(
                    isinstance(provider, LI.LoadImagesImageProvider))
    ...
    def test_09_01_load_avi(self):
        ...
        t = m.get_current_image_measurement(
            "_".join((measurements.C_METADATA, LI.M_T)))
    ...
    def test_09_02_load_stk(self):
        for path in [
            ...
            sys.stderr.write(
                "WARNING: unknown path to stk file. Test not run.\n")
            return
        module = LI.LoadImages()
    ...
    def make_objects_workspace(self, image, mode="L", filename="myfile.tif"):
        ...
        workspace = W.Workspace(pipeline, module,
    ...
    def test_12_01_load_empty_objects(self):
        ...
        self.assertEqual(
            m.get_current_image_measurement(LI.I.FF_COUNT % OBJECTS_NAME), 0)
    ...
    def test_12_02_load_indexed_objects(self):
        ...
        assert isinstance(m, measurements.Measurements)
        self.assertEqual(
            m.get_current_image_measurement(LI.I.FF_COUNT % OBJECTS_NAME), 9)
    ...
    def test_13_01_batch_images(self):
        ...
        module.location.dir_choice = LI.ABSOLUTE_FOLDER_NAME
        orig_path = os.path.join(T.example_images_directory(),
                                 "ExampleSBSImages")
        module.location.custom_path = orig_path
        target_path = orig_path.replace("ExampleSBSImages",
                                        "ExampleTrackObjects")
