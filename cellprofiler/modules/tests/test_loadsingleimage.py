...
from cellprofiler.modules.identify import M_LOCATION_CENTER_X, \
    M_LOCATION_CENTER_Y, M_NUMBER_OBJECT_NUMBER
...
"""
    ...
    def test_03_03_measurements(self):
        ...
        measurements = module.get_measurements(workspace.pipeline, cpmeas.IMAGE,
                                               "bar")
        self.assertEqual(len(measurements), 0)
        measurements = module.get_measurements(workspace.pipeline, "foo",
                                               L.C_PATH_NAME)
        self.assertEqual(len(measurements), 0)
        categories = module.get_categories(workspace.pipeline, cpmeas.IMAGE)
        self.assertEqual(len(categories), 6)
    ...
    def test_03_05_object_categories(self):
        ...
        for object_name, expected_categories in (
                (cpmeas.IMAGE,
                 (L.C_COUNT, L.C_OBJECTS_FILE_NAME, L.C_OBJECTS_PATH_NAME)),
