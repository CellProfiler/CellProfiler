...
"""
        ...
        for method, module in zip(methods, pipeline.modules()):
            ...
            self.assertEqual(module.projection_image_name,
                             "ProjectionCh00Scale6")
    ...
    def test_05_01_power(self):
        ...
        np.testing.assert_almost_equal(
            image_out.pixel_data[(i != 5) & (j != 5)], 0)
    ...
    def test_06_01_brightfield(self):
        ...
        i, j = np.mgrid[:image.shape[0], :image.shape[1]]
        np.testing.assert_almost_equal(image_out.pixel_data[(i > 5) | (j < 5)],
                                       0)
        np.testing.assert_almost_equal(image_out.pixel_data[(i < 5) & (j >= 5)],
                                       1)
