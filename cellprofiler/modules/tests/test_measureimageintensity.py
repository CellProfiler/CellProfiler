...
class TestMeasureImageIntensity(unittest.TestCase):
    ...
    def test_01_01_image(self):
        ...
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                         ...
                         100)
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "Intensity_TotalIntensity_my_image"),
                         np.sum(pixels))
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "Intensity_MeanIntensity_my_image"),
                         np.sum(pixels) / 100.0)
        self.assertEqual(
            m.get_current_image_measurement('Intensity_MinIntensity_my_image'),
            np.min(pixels))
        self.assertEqual(
            m.get_current_image_measurement('Intensity_MaxIntensity_my_image'),
            np.max(pixels))
    ...
    def test_01_03_image_and_objects(self):
        ...
        m = workspace.measurements
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "Intensity_TotalArea_my_image_my_objects"),
                         64)
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "Intensity_TotalIntensity_my_image_my_objects"),
                         np.sum(pixels[1:9, 1:9]))
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "Intensity_MeanIntensity_my_image_my_objects"),
                         np.sum(pixels[1:9, 1:9]) / 64.0)
        self.assertAlmostEqual(m.get_current_measurement(
            cpmeas.IMAGE, "Intensity_PercentMaximal_my_image_my_objects"),
            400. / 64.)
    ...
    def test_03_01_get_measurement_columns(self):
        ...
        for expected_suffix in expected_suffixes:
            ...
            (M.F_TOTAL_INTENSITY, cpmeas.COLTYPE_FLOAT),
