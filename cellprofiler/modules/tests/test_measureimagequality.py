...
class TestMeasureImageQuality(unittest.TestCase):
    ...
    def test_00_00_zeros(self):
        ...
        for feature_name, value in (("ImageQuality_FocusScore_my_image", 0),
                                    ...
                                    ("ImageQuality_PercentMaximal_my_image",
                                     100),
                                    ("ImageQuality_PercentMinimal_my_image",
                                     100),
                                    ("ImageQuality_PowerLogLogSlope_my_image",
                                     0),
            ...
            self.assertTrue(
                m.has_current_measurements(cpmeas.IMAGE, feature_name),
    ...
    def test_01_02_local_focus_score(self):
        ...
        q.run(workspace)
        m = workspace.measurements
        value = m.get_current_measurement(cpmeas.IMAGE,
                                          "ImageQuality_LocalFocusScore_my_image_500")
    ...
    def test_02_02_maximal(self):
        ...
        self.assertAlmostEqual(expected_value,
                               m.get_current_measurement(cpmeas.IMAGE,
                                                         "ImageQuality_PercentMaximal_my_image"))
    ...
    def test_02_03_saturation_mask(self):
        ...
        ("ImageQuality_PercentMinimal_my_image", 200.0 / 3)):
            self.assertTrue(m.has_current_measurements(cpmeas.IMAGE,
                                                       feature_name))
            print feature_name, expected_value, m.get_current_measurement(
                cpmeas.IMAGE,
                feature_name)
            self.assertAlmostEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                             feature_name),
    ...
    def test_03_01_threshold(self):
        ...
        for tm, idx in zip(cpthresh.TM_GLOBAL_METHODS,
            ...
            if tm == cpthresh.TM_OTSU_GLOBAL:
                ...
                feature_name = "ImageQuality_Threshold%s_my_image" % \
                               tm.split(' ')[0]
            self.assertTrue(m.has_current_measurements(cpmeas.IMAGE,
                                                       feature_name))
        self.features_and_columns_match(m, q)
    ...
    def test_03_02_experiment_threshold(self):
        ...
        feature = module.image_groups[0].threshold_groups[
            0].threshold_feature_name(image_name)
...
"""
    ...
    def test_05_01_intensity_image(self):
        ...
        q.run(workspace)
        m = workspace.measurements
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "ImageQuality_TotalIntensity_my_image"),
                         np.sum(pixels))
        self.assertEqual(m.get_current_measurement(cpmeas.IMAGE,
                                                   "ImageQuality_MeanIntensity_my_image"),
                         np.sum(pixels) / 100.0)
