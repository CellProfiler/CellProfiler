...
"""
        ...
        for i, wants_gabor in enumerate((True, False)):
            ...
            self.assertEqual(
                len(module.scale_groups[0].angles.get_selections()), 1)
            self.assertEqual(module.scale_groups[0].angles.get_selections()[0],
                             M.H_HORIZONTAL)
            self.assertEqual(module.scale_groups[1].scale, 5)
            self.assertEqual(
                len(module.scale_groups[1].angles.get_selections()), 1)
            self.assertEqual(module.scale_groups[1].angles.get_selections()[0],
                             M.H_HORIZONTAL)
...
"""
    ...
    def test_02_02_many_objects(self):
        ...
        for measurement in M.F_HARALICK:
            ...
            for angle in M.H_ALL:
                mname = '%s_%s_%s_%d_%s' % (
                    M.TEXTURE, measurement, INPUT_IMAGE_NAME, 2,
                    M.H_TO_A[angle])
    ...
    def test_03_03_measurement_columns(self):
        ...
        for x in module.get_measurement_columns(pipeline):
            assert x[1] in measurements.get_feature_names(x[
                                                              0]), '%s does not match any measurement output by pipeline' % (
            str(x))
        for obname in measurements.get_object_names():
            for m in measurements.get_feature_names(obname):
                if m.startswith(M.TEXTURE):
                    assert (obname, m,
                            'float') in module.get_measurement_columns(
                        pipeline), 'no entry matching %s in get_measurement_columns.' % (
                    (obname, m, 'float'))
