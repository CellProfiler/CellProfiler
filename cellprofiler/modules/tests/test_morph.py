...
"""
    ...
    def test_02_07_binary_erode(self):
        self.binary_tteesstt('erode', lambda x: scind.binary_erosion(x, np.ones(
            (3, 3), bool)))
    ...
    def test_02_25_gray_close(self):
        np.random.seed(0)
        image = np.random.uniform(size=(20, 15)).astype(np.float32)
        result = self.execute(image, 'close',
    ...
    def test_03_01_color(self):
        ...
        self.assertTrue(
            np.all(result < image[:, :, 0] + np.finfo(np.float32).eps))
