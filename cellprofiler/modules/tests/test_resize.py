...
"""
    ...
    def test_02_01_rescale_triple_color(self):
        ...
        expected[:, :, 0] = i
        expected[:, :, 1] = j
        workspace, module = self.make_workspace(image, cellprofiler.modules.resize.R_BY_FACTOR, cellprofiler.modules.resize.I_NEAREST_NEIGHBOR)
        ...
        self.assertTrue(numpy.all(numpy.abs(result - expected) <= numpy.finfo(float).eps))
        self.assertTrue(result_image.parent is workspace.image_set.get_image(INPUT_IMAGE_NAME))
