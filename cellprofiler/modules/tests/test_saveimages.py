...
""" % image_folder_text
    ...
    def test_01_01_save_first_to_same_tif(self):
        ...
        img1_out_filename = os.path.join(self.new_image_directory,
                                         'img1OUT.tif')
        img2_filename = os.path.join(self.new_image_directory, 'img2.tif')
        img2_out_filename = os.path.join(self.new_image_directory,
                                         'img2OUT.tif')
        ...
        filenames = measurements.get_all_measurements('Image',
                                                      'FileName_Derived')
        pathnames = measurements.get_all_measurements('Image',
                                                      'PathName_Derived')
    ...
    def test_01_04_save_all_to_output_tif(self):
        img1_filename = os.path.join(self.new_image_directory, 'img1.tif')
        img1_out_filename = os.path.join(self.new_output_directory,
                                         'img1OUT.tif')
        img2_filename = os.path.join(self.new_image_directory, 'img2.tif')
        img2_out_filename = os.path.join(self.new_output_directory,
                                         'img2OUT.tif')
        make_file(img1_filename, cpmt.tif_8_1)
        make_file(img2_filename, cpmt.tif_8_2)
        pipeline = cpp.Pipeline()
    ...
    def test_01_15_create_subdirectories_inherit_path(self):
        ...
        self.assertEqual(pathnames[0], pn)
        self.assertTrue(os.path.isfile(img1_out_filename),
                        img1_out_filename + " does not exist")
        self.assertTrue(os.path.isfile(img2_out_filename),
    ...
    def test_02_02_regression_prepare_to_create_batch(self):
        ...
        self.assertEqual(module.pathname.custom_path,
                         './\\g<Test>Outlines/g<Run>_\\g<Plate>')
    ...
    def test_03_01_get_measurement_columns(self):
        ...
        for column in columns:
            ...
            self.assertTrue(
                column[1] in ("PathName_MyImage", "FileName_MyImage"))
    ...
    def make_workspace(self, image, filename=None, path=None,
        ...
        if filename is not None:
            m.add_image_measurement('_'.join(("FileName", IMAGE_NAME)),
                                    filename)
        if path is not None:
    ...
    def test_05_02_save_two_movies(self):
        ...
        for group in grouping[1]:
            path = os.path.join(self.custom_directory,
                                group[0]["Metadata_test"] + ".avi")
    ...
    def test_06_01_save_image(self):
        ...
        image8s[0, 0] = 245
        image8s[0, 1] = 0
        image16s = (np.random.uniform(size=(100, 100)) * 64535).astype(
            np.uint16)
        image16s[0, 0] = 64535
        image16s[0, 1] = 0
    ...
    def test_07_04_save_objects_color_png(self):
        ...
        different = np.hstack(
            ([False], np.any(im[order[:-1], :] != im[order[1:], :], 1)))
