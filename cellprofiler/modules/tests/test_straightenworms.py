...
'''
        ...
        self.assertTrue(isinstance(module, cellprofiler.modules.straightenworms.StraightenWorms))
...
'''
        ...
        for alignment, module in zip((cellprofiler.modules.straightenworms.FLIP_TOP, cellprofiler.modules.straightenworms.FLIP_BOTTOM, cellprofiler.modules.straightenworms.FLIP_NONE),
                                     pipeline.modules()):
            self.assertTrue(isinstance(module, cellprofiler.modules.straightenworms.StraightenWorms))
            self.assertEqual(module.objects_name, "OverlappingWorms")
            self.assertEqual(module.straightened_objects_name, "StraightenedWorms")
    ...
    def make_workspace(self, control_points, lengths, radii, image,
        ...
        for i, (y, x) in enumerate(control_points):
            for v, f in ((x, cellprofiler.modules.straightenworms.F_CONTROL_POINT_X),
                         (y, cellprofiler.modules.straightenworms.F_CONTROL_POINT_Y)):
                feature = "_".join((cellprofiler.modules.straightenworms.C_WORM, f, str(i + 1)))
                m.add_measurement(OBJECTS_NAME, feature, v)
        feature = "_".join((cellprofiler.modules.straightenworms.C_WORM, cellprofiler.modules.straightenworms.F_LENGTH))
    ...
    def test_02_01_straighten_nothing(self):
        ...
        self.assertFalse(numpy.any(image.mask))
        objectset = workspace.object_set
        self.assertTrue(isinstance(objectset, cellprofiler.region.Set))
    ...
    def test_02_02_straighten_straight_worm(self):
        ...
        control_points = numpy.array([[[21], [15]],
                                      [[23], [15]],
                                      [[25], [15]],
                                      [[27], [15]],
                                      [[29], [15]]])
        ...
        self.assertTrue(numpy.all(objects.segmented ==
    ...
    def test_02_03_straighten_diagonal_worm(self):
        ...
        r.seed(23)
        image = r.uniform(size=(60, 30))
        control_points = numpy.array([[[10], [10]],
                                      [[13], [14]],
                                      [[16], [18]],
                                      [[19], [22]],
                                      [[22], [26]]])
    ...
    def test_02_04_straighten_two_worms(self):
        ...
        r.seed(0)
        image = r.uniform(size=(60, 30))
        control_points = numpy.array([[[21, 10], [15, 10]],
                                      [[23, 13], [15, 14]],
                                      [[25, 16], [15, 18]],
                                      [[27, 19], [15, 22]],
                                      [[29, 22], [15, 26]]])
    ...
    def test_02_05_straighten_missing_worm(self):
        ...
        r.seed(0)
        image = r.uniform(size=(60, 30))
        control_points = numpy.array([[[21, numpy.nan, 10], [15, numpy.nan, 10]],
    ...
    def test_03_02_get_measurement_columns_wants_images_vertical(self):
        ...
        expected_columns = (
            [(OBJECTS_NAME,
              "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr, image, module.get_scale_name(None, segno))),
              cellprofiler.measurement.COLTYPE_FLOAT)
             for ftr, image, segno in zip(
                [cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY] * 10 + [cellprofiler.modules.straightenworms.FTR_STD_INTENSITY] * 10,
        ...
        self.assertTrue(cellprofiler.modules.straightenworms.C_WORM in categories)

        features = module.get_measurements(workspace.pipeline,
                                           OBJECTS_NAME, cellprofiler.modules.straightenworms.C_WORM)
        self.assertEqual(len(features), 2)
        self.assertTrue(cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY in features)
        self.assertTrue(cellprofiler.modules.straightenworms.FTR_STD_INTENSITY in features)

        for ftr in (cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, cellprofiler.modules.straightenworms.FTR_STD_INTENSITY):
            images = module.get_measurement_images(workspace.pipeline,
                                                   OBJECTS_NAME,
                                                   cellprofiler.modules.straightenworms.C_WORM, ftr)
            self.assertEqual(len(images), 2)
            self.assertTrue(STRAIGHTENED_IMAGE_NAME in images)
            self.assertTrue(AUX_STRAIGHTENED_IMAGE_NAME in images)

        for ftr, image in zip([cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, cellprofiler.modules.straightenworms.FTR_STD_INTENSITY] * 2,
    ...
    def test_03_03_get_measurement_columns_horizontal(self):
        ...
        expected_columns = (
            [(OBJECTS_NAME,
              "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr, image, module.get_scale_name(segno, None))),
              cellprofiler.measurement.COLTYPE_FLOAT)
             for ftr, image, segno in zip(
                [cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY] * 10 + [cellprofiler.modules.straightenworms.FTR_STD_INTENSITY] * 10,
                ([STRAIGHTENED_IMAGE_NAME] * 5 + [AUX_STRAIGHTENED_IMAGE_NAME] * 5) * 2,
                ...
                for vscale in vscales:
                    for hscale in hscales:
                        if vscale is None and hscale is None:
                            continue
                        meas = "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr, image,
        ...
        for ftr in (cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, cellprofiler.modules.straightenworms.FTR_STD_INTENSITY):
            for image in (STRAIGHTENED_IMAGE_NAME, AUX_STRAIGHTENED_IMAGE_NAME):
                scales = module.get_measurement_scales(
                        workspace.pipeline, OBJECTS_NAME, cellprofiler.modules.straightenworms.C_WORM, ftr, image)
    ...
    def test_04_00_measure_no_worms(self):
        ...
        for i in range(5):
            for ftr, function in (
                ...
                mname = "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr,
    ...
    def test_04_01_measure_one_worm(self):
        ...
        def weighted_std(img, segments, mask):
            ...
            return numpy.sqrt(accumulator / weight_accumulator /
    ...
    def test_04_02_measure_checkerboarded_worm(self):
        ...
        expected_sds = numpy.sqrt(
            s2(i_w * (image[:, :, numpy.newaxis] - expected_means[numpy.newaxis, numpy.newaxis, :]) ** 2) /
            weights * counts / (counts - 1))
        for i in range(4):
            for ftr, expected in ((cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, expected_means),
                                  (cellprofiler.modules.straightenworms.FTR_STD_INTENSITY, expected_sds)):
                value = m.get_current_measurement(
                        OBJECTS_NAME,
                        "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr, STRAIGHTENED_IMAGE_NAME,
                                  module.get_scale_name(None, i))))
        ...
        counts = s2(j_w > 0)
        expected_sds = numpy.sqrt(
            s2(j_w * (image[:, :, numpy.newaxis] -
                      expected_means[numpy.newaxis, numpy.newaxis, :]) ** 2) /
            weights * counts / (counts - 1))
        for i in range(3):
            for ftr, expected in ((cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, expected_means),
                                  (cellprofiler.modules.straightenworms.FTR_STD_INTENSITY, expected_sds)):
        ...
        counts = s2(ww > 0)
        expected_sds = numpy.sqrt(
            s2(ww * (image[:, :, numpy.newaxis, numpy.newaxis] -
                     expected_means[numpy.newaxis, numpy.newaxis, :, :]) ** 2) /
            weights * counts / (counts - 1))
        for stripe in range(3):
            for segment in range(4):
                for ftr, expected in ((cellprofiler.modules.straightenworms.FTR_MEAN_INTENSITY, expected_means),
                                      (cellprofiler.modules.straightenworms.FTR_STD_INTENSITY, expected_sds)):
                    mname = "_".join((cellprofiler.modules.straightenworms.C_WORM, ftr, STRAIGHTENED_IMAGE_NAME,
