...
class TestTile(unittest.TestCase):
    ...
    def test_01_02_load_matlab_place_adjacent(self):
        ...
        self.assertEqual(module.additional_images[0].input_image_name,
                         "Cytoplasm")
...
"""
    ...
    def test_02_01_manual_rows_and_columns(self):
        np.random.seed(0)
        images = [np.random.uniform(size=(20, 10)).astype(np.float32) for i in
                  range(96)]
    ...
    def test_02_05_color(self):
        np.random.seed(4)
        images = [np.random.uniform(size=(20, 10, 3)).astype(np.float32) for i
                  in range(96)]
        workspace, module = self.make_tile_workspace(images)
        self.assertTrue(isinstance(module, T.Tile))
        self.assertTrue(isinstance(workspace, cpw.Workspace))
        ...
        for i, image in enumerate(images):
            ...
            jjj = jj * 10
            self.assertTrue(
                np.all(pixel_data[iii:(iii + 20), jjj:(jjj + 10), :] ==
                       image))
        ...
        for i, image in enumerate(images):
            ...
            if i == 0:
                ...
                module.additional_images[
                    i - 1].input_image_name.value = image_name
    ...
    def test_03_01_some_images(self):
        np.random.seed(31)
        for i in range(1, 5):
            images = [np.random.uniform(size=(20, 10)).astype(np.float32) for ii
                      in range(i)]
    ...
    def test_03_02_mix_color_bw(self):
        ...
        for color in range(3):
            images = [np.random.uniform(
                size=(20, 10, 3) if i == color else (20, 10)).astype(np.float32)
                      for i in range(3)]
            workspace, module = self.make_place_workspace(images)
            module.run(workspace)
            image = workspace.image_set.get_image(OUTPUT_IMAGE_NAME)
            ...
            for j, p in enumerate(images):
                ...
                if j == color:
                    ...
                    for k in range(3):
                        self.assertTrue(
                            np.all(pixel_data[:, jj:(jj + 10), k] == p))
