...
class Objects(object):
    ...
    def get_segmented(self):
        ...
        assert len(
            dense) == 1, "Operation failed because objects overlapped. Please try with non-overlapping objects"
    ...
    def cache(self, hdf5_object_set, objects_name):
        ...
        for segmentation, segmentation_name in (
                ...
                (self.__small_removed_segmented,
                 Segmentation.SMALL_REMOVED_SEGMENTED)):
    ...
    def make_ijv_outlines(self, colors):
        ...
        # don't overlap.
        #
        all_labels = [(outline(label), indexes) for label, indexes in
                      self.get_labels()]
    ...
    def relate_histogram(self, histogram):
        ...
        children_per_parent = \
        np.histogram(parents_of_children[1:], np.arange(parent_count + 2))[0][
        1:]
    ...
    def histogram_from_ijv(parent_ijv, child_ijv):
        ...
        parent_count = 0 if (parent_ijv.shape[0] == 0) else np.max(
            parent_ijv[:, 2])
        child_count = 0 if (child_ijv.shape[0] == 0) else np.max(
            child_ijv[:, 2])
...
class Segmentation(object):
    ...
    def __convert_dense_to_sparse(self):
        ...
        sparse = np.core.records.fromarrays(list(coords) + [labels],
                                            dtype=dtype)
        if self.__cache is not None:
...
def check_consistency(segmented, unedited_segmented, small_removed_segmented):
    ...
    assert small_removed_segmented is None or np.all(
        small_removed_segmented >= 0)
...
class ObjectSet(object):
    ...
    def add_objects(self, objects, name):
        assert isinstance(objects,
                          Objects), "objects must be an instance of CellProfiler.Objects"
        assert ((not self.__objects_by_name.has_key(name)) or
                self.__can_overwrite), "The object, %s, is already in the object set" % (
        name)
