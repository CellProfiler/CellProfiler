...
def get_awt_headless():
    ...
    if __awt_headless is None:
        return get_headless() and not os.environ.has_key(
            "CELLPROFILER_USE_XVFB")
...
def get_config():
    ...
    if not config:
        wx.Config.Set(
            wx.Config('CellProfiler', 'BroadInstitute', 'CellProfilerLocal.cfg',
                      'CellProfilerGlobal.cfg', wx.CONFIG_USE_LOCAL_FILE))
        ...
        if not config.Exists(PREFERENCES_VERSION):
            ...
            try:
                preferences_version_number = int(
                    config_read(PREFERENCES_VERSION))
...
def get_default_image_directory():
    ...
    try:
        if os.path.isdir(default_image_directory):
            __default_image_directory = os.path.normcase(
                default_image_directory)
            return __default_image_directory
    except:
        logger.error(
            "Unknown failure when retrieving the default image directory",
            exc_info=True)
    logger.warning(
...
def get_default_output_directory():
    ...
    try:
        if os.path.isdir(default_output_directory):
            __default_output_directory = os.path.normcase(
                default_output_directory)
            return __default_output_directory
    except:
        logger.error(
...
def set_default_output_directory(path):
    ...
    assert os.path.isdir(
        path), 'Default Output Folder, "%s", is not a directory' % (path)
