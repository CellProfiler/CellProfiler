...
class Coordinates(Setting):
    ...
    def __init__(self, text, value=(0, 0), *args, **kwargs):
        ...
        """
        super(Coordinates, self).__init__(text, "%d,%d" % value, *args,
                                          **kwargs)
    ...
    def set_value(self, value):
        ...
        try:
            if len(value) == 2:
                super(Coordinates, self).set_value(
                    "%d,%d" % (value[0], value[1]))
                return
        except:
            pass
    ...
    def test_valid(self, pipeline):
        values = self.value_text.split(',')
        if len(values) < 2:
            raise ValidationError("X and Y values must be separated by a comma",
                                  self)
        if len(values) > 2:
            raise ValidationError("Only two values allowed", self)
        for value in values:
...
class IntegerOrUnboundedRange(IntegerRange):
    ...
    def test_valid(self, pipeline):
        ...
        if ((not self.unbounded_min) and
            ...
            raise ValidationError(
                "%s can't be less than %d" % (self.min_text, self._minval),
                self)
...
class ObjectNameProvider(NameProvider):
    ...
    def test_valid(self, pipeline):
        if self.value_text in cellprofiler.measurements.disallowed_object_names:
            raise ValidationError("Object names may not be any of %s" % (
            ", ".join(cellprofiler.measurements.disallowed_object_names)), self)
...
class NameSubscriber(Setting):
    ...
    def get_choices(self, pipeline):
        ...
        return choices + sorted(
            get_name_provider_choices(pipeline, self, self.group))
...
class ExternalImageNameSubscriber(ImageNameSubscriber):
    ...
    def __init__(self, text, value=DO_NOT_USE, can_be_blank=False,
                 blank_text=LEAVE_BLANK, *args, **kwargs):
        super(ExternalImageNameSubscriber, self).__init__(text, value,
                                                          can_be_blank,
                                                          blank_text, *args,
                                                          **kwargs)
...
class MeasurementMultiChoice(MultiChoice):
    ...
    def __init__(self, text, value='', *args, **kwargs):
        ...
        '''
        super(MeasurementMultiChoice, self).__init__(text, [], value, *args,
                                                     **kwargs)
...
class SubdirectoryFilter(MultiChoice):
    ...
    def __init__(self, text, value='', directory_path=None, **kwargs):
        ...
        assert (directory_path is None) or isinstance(directory_path,
                                                      DirectoryPath)
...
class TreeChoice(Setting):
    ...
    def get_leaves(self, path=[]):
        ...
        while len(path) > 0:
            ...
            if idx == -1 or current[idx][1] is None or len(
                    current[idx][1]) == 0:
                return []
...
class Colormap(Choice):
    ...
    def __init__(self, text, value=DEFAULT, *args, **kwargs):
        try:
            ...
            names = ['Spectral', 'copper', 'RdYlGn', 'Set2', 'summer', 'spring',
                     'Accent', 'OrRd', 'RdBu', 'autumn', 'Set1', 'PuBu', 'Set3',
                     'gist_rainbow', 'pink', 'binary', 'winter', 'jet', 'BuPu',
                     'Dark2', 'prism', 'Oranges', 'gist_yarg', 'BuGn', 'hot',
                     'PiYG', 'YlOrBr', 'Reds', 'spectral', 'RdPu', 'Greens',
                     'gist_ncar', 'PRGn', 'gist_heat', 'YlGnBu', 'RdYlBu',
                     'Paired', 'flag', 'hsv', 'BrBG', 'Purples', 'cool',
                     'Pastel2', 'gray', 'Pastel1', 'gist_stern', 'GnBu', 'YlGn',
                     'Greys', 'RdGy', 'YlOrRd', 'PuOr', 'PuRd', 'gist_gray',
                     'Blues', 'PuBuGn', 'gist_earth', 'bone']
        names.sort()
...
class PathListRefreshButton(DoSomething):
    ...
    def __init__(self, text, label, *args, **kwargs):
        DoSomething.__init__(self, text, label, self.fn_callback, *args,
                             **kwargs)
...
class Table(Setting):
    ...
    def get_data(self, row_index, columns):
        ...
        return [[row[ci] for ci in column_indices] for row in
                self.data[row_index]]
