...
""" % locals())
    ...
    try:
        ...
        statistics = test_matching_columns(test_measurements,
        ...
        if (test_measurements.has_key("ObjectNumber") and
            ...
            for feature in test_measurements_per_image.keys():
                ...
                reference_measurement = reference_measurements_per_image[
                    feature]
                fs_all = None
                for i, tm, rm in zip(image_numbers, test_measurement,
                    ...
                    if fs_all is None:
                        fs_all = [
                            (name, [attributes], error_type, message, body)
                            for name, attributes, error_type, message, body in
                            fs]
                    else:
...
def collect_measurements(rdr):
    ...
    for i, row in enumerate(rdr):
        if len(row) != len(header):
            raise ValueError(
                "Row size (%d) doesn't match header size (%d) at line %d" %
...
def test_deviations(test_measurements, reference_measurements,
    ...
    if tm_len + rm_len > 0:
        ...
        if deviance > max_obj_deviation:
            message = (
            "# of measurements is different: %d in test, %d in reference" %
            (tm_len, rm_len))
...
def test_float_deviation(feature, test_measurement, reference_measurement,
    ...
    rm_nan_fraction = 1.0 - float(len(rm_no_nan)) / float(
        len(reference_measurement))
    ...
    if mean_diff > max_deviation:
        message = (
        "Test and reference means differ: %f / %f test, %f / %f reference" %
