...
class TestAnalysis(unittest.TestCase):
            ...
            try:
                while True:
                    ...
                    if socks.get(self.work_socket, None) == zmq.POLLIN:
                        logger.debug(
                            "    Received response for %s" % str(type(req)))
    ...
    def make_pipeline_and_measurements(self,
        ...
        for i in range(1, nimage_sets + 1):
            if group_numbers is not None:
                ...
                group_index = i
            m[
                cpmeas.IMAGE, cpmeas.C_URL + "_" + IMAGE_NAME, i] = "file:/%d.tif" % i
    ...
    def test_01_01_start_and_stop(self):
        ...
        logger.debug("Entering %s" % inspect.getframeinfo(
        ...
        self.assertIsInstance(analysis_finished.measurements,
                              cpmeas.Measurements)
        logger.debug("Exiting %s" % inspect.getframeinfo(
            inspect.currentframe()).function)
    ...
    def test_04_05_exception(self):
        ...
        with self.FakeWorker() as worker:
            ...
            self.assertEqual(function, inspect.getframeinfo(
                inspect.currentframe()).function)
        ...
        with self.FakeWorker() as worker:
            ...
            for i in range(1, 4):
                if i == 2:
                    ...
                    self.assertEqual(
                        measurements[cpmeas.IMAGE, IMAGE_FEATURE, i],
                        "Hello %d" % i)
                    np.testing.assert_almost_equal(
                        measurements[OBJECTS_NAME, OBJECTS_FEATURE, i],
                        objects_measurements[i - 1])
