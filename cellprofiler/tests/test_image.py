...
class TestImageSetList(unittest.TestCase):
    ...
    def test_01_01_add_image_set_by_number(self):
        ...
        self.assertEqual(x.count(), 1,
                         "# of elements was %d, should be 1" % (x.count()))
        self.assertEqual(y.get_number(), 0,
                         "The image set should be #0, was %d" % (
                         y.get_number()))
        self.assertTrue(y.get_keys().has_key("number"),
                        "The image set was missing a number key")
        self.assertEqual(y.get_keys()["number"], 0,
                         "The number key should be zero, was %s" % (
                         repr(y.get_keys()["number"])))
    ...
    def test_01_02_add_image_set_by_key(self):
        ...
        self.assertEqual(y.get_number(), 0,
                         "The image set should be #0, was %d" % (
                         y.get_number()))
        self.assertEquals(y, x.get_image_set(0),
                          "The image set should be retrievable by index")
        self.assertEquals(y, x.get_image_set(key),
                          "The image set should be retrievable by key")
    ...
    def test_01_03_add_two_image_sets(self):
        ...
        self.assertEqual(x.count(), 2,
                         "# of elements was %d, should be 2" % (x.count()))
        self.assertEqual(y.number, 0, "The image set should be #0, was %d" % (
        y.get_number()))
        self.assertEqual(z.number, 1, "The image set should be #1, was %d" % (
        y.get_number()))
        self.assertEquals(y, x.get_image_set(0),
                          "The first image set was not retrieved by index")
        self.assertEquals(z, x.get_image_set(1),
