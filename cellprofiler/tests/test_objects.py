...
class TestObjects(unittest.TestCase):
    ...
    def test_01_03_set_unedited_segmented(self):
        ...
        self.assertTrue(
            (self.__unedited_segmented10 == x.unedited_segmented).all())
    ...
    def test_07_01_make_ijv_outlines(self):
        ...
        i1 = [i for i, color in enumerate(colors) if
              np.all(color == image[0, 5, :])]
        self.assertEqual(len(i1), 1)
        i2 = [i for i, color in enumerate(colors) if
              np.all(color == image[0, 12, :])]
        self.assertEqual(len(i2), 1)
        i3 = [i for i, color in enumerate(colors) if
              np.all(color == image[-1, 8, :])]
    ...
    def test_07_02_labels_same_as_ijv(self):
        ...
        np.testing.assert_array_equal(labels_parents_of_children,
                                      ijv_parents_of_children)
...
class TestSegmentation(unittest.TestCase):
    ...
    def test_02_01_sparse_to_dense(self):
        ...
        ijv = np.core.records.fromarrays([
                                         ...
                                         [(HDF5ObjectSet.AXIS_Y, np.uint32, 1),
                                          (HDF5ObjectSet.AXIS_X, np.uint32, 1),
                                          (HDF5ObjectSet.AXIS_LABELS, np.uint32,
                                           1)])
