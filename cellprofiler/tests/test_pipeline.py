...
def image_with_one_cell(size=(100, 100)):
    ...
    dist = g[0, :, :] * g[0, :, :] + g[1, :, :] * g[1, :,
                                                  :]  # squared Euclidean distance.
    img[dist < 25] = (25.0 - dist.astype(float)[
        dist < 25]) / 25  # A circle centered at (50, 50)
    return img
    ...
    def fn(pipeline, event):
        if isinstance(event, cpp.RunExceptionEvent):
            ...
            test.assertFalse(
                ...
                "\n".join(
                    [event.error.message] + traceback.format_tb(event.tb)))
...
class TestPipeline(unittest.TestCase):
    ...
    def test_01_01_load_mat(self):
        ...
        pipeline.load(
            cStringIO.StringIO(zlib.decompress(base64.b64decode(img_942_data))))
 ...
 """
    ...
    def test_10_01_all_groups(self):
        ...
        def prepare_run(workspace):
            ...
            for group_number_idx, (grouping, image_numbers) in enumerate(
                    groupings):
        ...
        def run(workspace):
            ...
            workspace.measurements.add_image_measurement("mymeasurement",
                                                         image_number)
        ...
        image_numbers = measurements.get_all_measurements("Image",
                                                          "mymeasurement")
        self.assertEqual(len(image_numbers), 4)
        self.assertTrue(np.all(image_numbers == np.array([1, 2, 3, 4])))
        group_numbers = measurements.get_all_measurements("Image",
                                                          "Group_Number")
        self.assertTrue(np.all(group_numbers == np.array([1, 1, 2, 2])))
        group_indexes = measurements.get_all_measurements("Image",
                                                          "Group_Index")
        ...
        def prepare_run(workspace):
            ...
            for i in range(1, 7):
                m[cpmeas.IMAGE, cpmeas.C_PATH_NAME + "_DNA", i] = \
                    "/imaging/analysis"
                m[
                    cpmeas.IMAGE, cpmeas.C_FILE_NAME + "_DNA", i] = "img%d.tif" % i
    ...
    def test_13_02_save_measurements(self):
        ...
        measurements.add_experiment_measurement("expt",
                                                my_experiment_measurement)
        for i in range(20):
            if i > 0:
        ...
        my_image_measurement_out = measurements.get_all_measurements(
            "Image", "img")
        self.assertEqual(len(my_image_measurement),
                         len(my_image_measurement_out))
        ...
        self.assertAlmostEqual(my_experiment_measurement,
                               my_experiment_measurement_out)
    ...
    def test_13_03_save_long_measurements(self):
        ...
        for name, expected in ((m1_name, my_measurement),
            ...
            my_measurement_out = measurements.get_all_measurements("Foo",
                                                                   map_name)
...
"""
        ...
        for m1setting, m2setting in zip(module.settings(),
                                        module_out.settings()):
    ...
    def test_14_02_unicode_save_and_load(self):
        ...
        self.assertEqual(module.my_variable.value,
                         result_module.my_variable.value)
    ...
    if False:
        ...
        def test_15_01_profile_example_all(self):
            ...
            if not example_dir:
                ...
                return
            pipeline_dir = os.path.join(example_dir,
                                        'ExampleAllModulesPipeline')
            pipeline_filename = os.path.join(pipeline_dir,
                                             'ExampleAllModulesPipeline.cp')
    ...
    def test_15_02_profile_example_fly(self):
        ...
        profile_pipeline(fd,
                         output_filename=os.path.join(build_dir, "profile.txt"))
        cpprefs.set_default_image_directory(old_image_dir)
    ...
    def test_16_00_get_provider_dictionary_nothing(self):
        for module in (ATestModule(),
            ...
            pipeline.add_module(module)
            for groupname in (
            cps.IMAGE_GROUP, cps.OBJECT_GROUP, cps.MEASUREMENTS_GROUP):
    ...
    def test_17_00_get_dependency_graph_empty(self):
        for module in (ATestModule(),
                       ...
                       ATestModule(
                           [cps.ImageNameSubscriber("foo", IMAGE_NAME)])):
            pipeline = cpp.Pipeline()
            module.module_num = 1
            pipeline.add_module(module)
    ...
    def test_17_01_get_dependency_graph_image(self):
        ...
        self.assertEqual(edge.source_setting,
                         pipeline.modules()[0].settings()[0])
        self.assertEqual(edge.image_name, IMAGE_NAME)
        self.assertEqual(edge.destination, pipeline.modules()[2])
        self.assertEqual(edge.destination_setting,
                         pipeline.modules()[2].settings()[0])
    ...
    def test_17_03_get_dependency_graph_measurement(self):
        ...
        measurement_columns = [
            (OBJECT_NAME, FEATURE_NAME, cpmeas.COLTYPE_FLOAT)]
        measurement_setting = cps.Measurement("text", lambda: OBJECT_NAME,
                                              FEATURE_NAME)
        for i, module in enumerate((
    ...
    def test_18_01_read_image_plane_details(self):
        ...
        for metadata_columns, body_lines, expected in test_data:
            ...
            s += '"' + '","'.join([
                                      cpp.H_URL, cpp.H_SERIES, cpp.H_INDEX,
                                      cpp.H_CHANNEL] +
                                  metadata_columns) + '"\n'
            s += "\n".join(body_lines) + "\n"
...
def profile_pipeline(pipeline_filename,
    ...
    if not output_filename:
        ...
        output_filename = os.path.join(cpprefs.get_default_output_directory(),
                                       pipeline_name + '_profile')

    if not os.path.exists(output_filename) or always_run:
        print 'Profiling %s' % pipeline_filename
        cProfile.runctx('run_pipeline(pipeline_filename)', globals(), locals(),
                        output_filename)
