...
class HDF5Dict(object):
    ...
    def __init__(self, hdf5_filename,
        ...
        open_mode = mode
        file_exists = (hdf5_filename is not None) and os.path.exists(
            hdf5_filename)
        ...
        logger.debug("HDF5Dict.__init__(): %s, temporary=%s, copy=%s, mode=%s",
                     self.filename, self.is_temporary, copy, open_mode)
        ...
        try:
                ...
                if image_numbers is None:
                    for object_name in copy.keys():
                        ...
                        for feature_name in object_group.keys():
                            ...
                            if 'index' in object_group[feature_name].keys():
                                hdf5_index = object_group[feature_name][
                                                 'index'][:]
                    ...
                    for object_name in copy.keys():
                        ...
                        if object_name == 'Experiment':
                            self.top_group.copy(src_object_group,
                                                self.top_group)
                            for feature_name in src_object_group.keys():
                                hdf5_index = src_object_group[feature_name][
                                                 'index'][:]
                                self.__cache_index(object_name, feature_name,
                                                   hdf5_index)
                            continue
                        dest_object_group = self.top_group.require_group(
                            object_name)
                        for feature_name in src_object_group.keys():
                            src_feature_group = src_object_group[feature_name]
                            if 'index' not in src_object_group[feature_name]:
                                dest_object_group.copy(src_feature_group,
                                                       dest_object_group)
                                continue
            ...
            logger.exception(
                "Failed during initial processing of %s" % self.filename)
            self.hdf5_file.close()
            raise

    def __del__(self):
        logger.debug("HDF5Dict.__del__(): %s, temporary=%s", self.filename,
                     self.is_temporary)
        self.close()
    ...
    def flush(self):
        logger.debug("HDF5Dict.flush(): %s, temporary=%s", self.filename,
                     self.is_temporary)
    ...
    def __is_positive_int(idx):
        ...
        return (
               isinstance(idx, int) or isinstance(idx, np.integer)) and idx >= 0

    def __getitem__(self, idxs):
        assert isinstance(idxs,
                          tuple), "Accessing HDF5_Dict requires a tuple of (object_name, feature_name[, integer])"
    ...
    def __setitem__(self, idxs, vals):
        ...
        with self.lock:
            if not self.has_feature(object_name, feature_name):
                if all_null:
                    self.__make_empty_feature(object_name, feature_name,
                                              num_idx)
                else:
    ...
    def __delitem__(self, idxs):
        ...
        assert isinstance(idxs[0], basestring) and isinstance(idxs[1],
                                                              basestring), "First two indices must be of type str."
        if len(idxs) == 3:
            assert isinstance(idxs[2], int) and idxs[
                                                    2] >= 0, "Third index must be a non-negative integer"
    ...
    def add_feature(self, object_name, feature_name):
        with self.lock:
            feature_group = self.top_group[object_name].require_group(
                feature_name)
        ...
        def fn(g, d, parts=[]):
            for k in d:
                ...
                if k is None:
                    to_remove = set(d[k])
                    dest = VStringArray(g, self.lock)
                    metadata = VStringArray(g.require_group("metadata"),
                                            self.lock)
                    leaves = list(dest)
                    order = [i for i, leaf in enumerate(leaves)
    ...
    def clear(self, objects_name, segmentation_name=None):
        ...
        if segmentation_name is None:
            for name in objects_group:
                ...
                if segmentation_group.attrs[
                    A_CLASS] == CLASS_SEGMENTATION_GROUP:
...
class HDFCSV(object):
    ...
    def __init__(self, group, name, lock=None):
        ...
        if name in self.parent_group:
            ...
            assert self.top_level_group.attrs[
...
class VStringArray(object):
    ...
    def set_all(self, strings):
        ...
        with self.lock:
            ...
            index = np.zeros(self.index.shape, self.index.dtype)
            if len(strings) > 0:
    ...
    def extend(self, strings):
        ...
        with self.lock:
            ...
            self.index.resize(target_len, 0)
            index = np.zeros((n_strings, 2), self.index.dtype)
            index[:, 1] = np.cumsum([len(s) for s in strings])
            index[0, 0] = 0
            if len(strings) > 1:
                index[1:, 0] = index[:-1, 1]
...
class StringReferencer(object):
    ...
    def get_string_refs(self, strings):
        ...
        for s in strings:
            ...
            if not found:
                if self.blockdesc[i][
                    self.SR_BLOCKDESC_IDX_LEN] == self.blocksize:
...
if __name__ == '__main__':
    ...
    def randtext():
        return "".join(["ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"[
                            np.random.randint(0, 52)] for _ in
