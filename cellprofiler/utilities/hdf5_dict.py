...
class HDF5Dict(object):
    ...
    def __all_null(vals):
        if np.isscalar(vals):
            return vals is None
        return all(
            [x is None or (not np.isscalar(x) and HDF5Dict.__all_null(x)) for x in vals]
        )
    ...
    def __setitem__(self, idxs, vals):
        ...
        if len(num_idx) > 0 and (np.isscalar(vals[0]) or vals[0] is None):
            ...
            vals = [[] if v is None else [v] if np.isscalar(v) else v for v in vals]
        all_null = True
            ...
            for vector in vals:
                if len(vector) > 0:
                    ...
                    if hdf5_type is None or hdf5_type == int:
                        ...
                        break
            hdf5_type_is_int = np.issubdtype(hdf5_type, int) or (
            ...
            if recast_dataset:
                ...
                if dataset.shape[0] > 0:
                    if hdf5_type_is_string:
                        kwds["data"] = np.array([str(v) for v in dataset[:]], object)
                    else:
                        kwds["data"] = dataset[:]
                else:
                    kwds["shape"] = (0,)
    ...
    def add_all(self, object_name, feature_name, values, idxs=None, data_type=None):
        ...
        with self.lock:
            ...
            if len(values) > 0 and (np.isscalar(values[0]) or values[0] is None):
            ...
            if dtype is None:
                ...
                values = [
                    np.array([str(v) for v in vector], object) for vector in values
                ]
            else:
...
class StringReferencer(object):
    ...
    def get_string_refs(self, strings):
        ...
        """
        strings, reverse_indices = np.unique(
            np.array(strings, object), return_inverse=True
        )
...
if __name__ == "__main__":
    ...
    hdest = HDF5Dict("temp1.hdf5", copy=h.top_group, image_numbers=np.arange(4, 15))
    for i in range(4, 15):
