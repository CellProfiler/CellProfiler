...
class TestHDF5Dict(unittest.TestCase):
    ...
    def test_02_03_write_numeric_imagesets(self):
        for dtype, code, ftr in ((int, 'i', FEATURE_NAME),
            ...
            self.assertTrue(
                all([np.all(d == e) for d, e in zip(d2[1:], data[1:])]))
    ...
    def test_02_06_upgrade_int_to_float(self):
        ...
        self.assertEqual(
            self.hdf5_dict[OBJECT_NAME, FEATURE_NAME, 1].dtype.kind, "i")
        self.hdf5_dict[OBJECT_NAME, FEATURE_NAME, 2] = np.arange(5).astype(
            float) / 2.0

        self.assertEqual(
            self.hdf5_dict[OBJECT_NAME, FEATURE_NAME, 1].dtype.kind, "f")
        self.assertSequenceEqual(
    ...
    def test_02_08_write_twice(self):
        ...
        index = self.hdf5_dict.top_group[OBJECT_NAME][FEATURE_NAME][
            H5DICT.INDEX]
...
class TestHDF5FileList(unittest.TestCase):
    def setUp(self):
        ...
        self.temp_fd_nocache, self.temp_filename_nocache = tempfile.mkstemp(
            ".h5")
    ...
    def test_08_00_list_directories_in_empty_dir(self):
        ...
        self.assertEqual(len(self.filelist.list_directories("file://foo/bar")),
                         0)
    ...
    def test_10_02_get_metadata(self):
        ...
        def fn_metadata(url):
            ...
            return r.randint(ord(" "), ord("~"), 300).astype(
                np.uint8).tostring()
