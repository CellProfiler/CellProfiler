...
def decode(s):
    return np.fromstring(zlib.decompress(b64decode(s)), dtype=np.float32)

def saveit(a):
  ww = b64encode(zlib.compress(a.astype(np.float32).tostring()))
  ww = np.fromstring(zlib.decompress(b64decode(ww)), dtype=np.float32)
  ww = ww.reshape(a.shape)
  np.testing.assert_array_almost_equal(a, ww)
