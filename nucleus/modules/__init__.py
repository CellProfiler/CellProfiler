...
def plugin_list():
    plugin_dir = nucleus.preferences.get_plugin_directory()
...
class PluginImporter(object):
    def find_module(self, fullname, path=None):
        if not fullname.startswith("nucleus.modules.plugins"):
            return None
        prefix, modname = fullname.rsplit(".", 1)
        if prefix != "nucleus.modules.plugins":
            return None
        if os.path.exists(
            os.path.join(nucleus.preferences.get_plugin_directory(), modname + ".py")
        ):
            return self
        return None
    ...
    def load_module(self, fullname):
        ...
        try:
            ...
            mod.__file__ = os.path.join(
                nucleus.preferences.get_plugin_directory(), modname + ".py"
            )
...
def check_module(module, name):
    ...
    for method_name in do_not_override:
        assert getattr(module, method_name) == getattr(
            nucleus.module.Module, method_name
...
def find_cpmodule(m):
    ...
    for v, val in list(m.__dict__.items()):
        if isinstance(val, type) and issubclass(val, nucleus.module.Module):
            return val
    raise ValueError("Could not find nucleus.module.Module class in %s" % m.__file__)
...
def fill_modules():
    ...
    for mod in builtin_modules:
        add_module("nucleus.modules." + mod, True)

    plugin_directory = nucleus.preferences.get_plugin_directory()
