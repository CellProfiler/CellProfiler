...
class LoadImages(nucleus.module.Module):
    ...
    def create_settings(self):
        # Settings
        self.file_types = nucleus.setting.Choice(
...
""".format(
        ...
        self.match_method = nucleus.setting.Choice(
...
""".format(
        ...
        self.exclude = nucleus.setting.Binary(
...
""",
        ...
        self.order_group_size = nucleus.setting.Integer(
...
""",
        ...
        self.descend_subdirectories = nucleus.setting.Choice(
...
""".format(
                **{"SUB_ALL": SUB_ALL, "SUB_NONE": SUB_NONE, "SUB_SOME": SUB_SOME}
            ),
        )

        # Location settings
        self.location = nucleus.setting.DirectoryPath(
        ...
        self.subdirectory_filter = nucleus.setting.SubdirectoryFilter(
...
""",
        ...
        self.check_images = nucleus.setting.Binary(
...
""".format(
        ...
        self.group_by_metadata = nucleus.setting.Binary(
...
""".format(
        ...
        self.metadata_fields = nucleus.setting.MultiChoice(
...
pair of fields.""",
        ...
        self.image_count = nucleus.setting.HiddenCount(self.images, text="Image count")

        # Add another image
        self.add_image = nucleus.setting.DoSomething(
    ...
    def add_imagecb(self, can_remove=True):
        ...
        def example_file_fn(path=None):
            ...
            filenames = [
                ...
                if x.find(".") != -1
                and os.path.splitext(x)[1].upper() in (".TIF", ".JPG", ".PNG", ".BMP")
            ]
            filtered_filenames = [x for x in filenames if self.filter_filename(x) == i]
        ...
        def example_path_fn():
            ...
            d = [
                x
                for x in [
                    os.path.abspath(os.path.join(root, x))
        ...
        group.append("divider", nucleus.setting.Divider(line=True))
        group.append(
            "common_text",
            nucleus.setting.Text(
...
channel 1, 5 through 8 to channel 2 and 9 through 12 to channel 3.""",
        ...
        group.append(
            "channel_count",
            nucleus.setting.HiddenCount(group.channels, "Channel count"),
        )
...
""".format(
        ...
        group.append(
            "outlines_name",
            nucleus.setting.OutlineNameProvider(
...
then save them with the **SaveImages** module.""",
        ...
        group.get_image_name = lambda: (
            ...
            if self.channel_wants_images(group)
            else IMAGE_FOR_OBJECTS_F % group.object_name.value
        )

        channels = [str(x) for x in range(1, max(10, len(image_settings.channels) + 2))]
...
""".format(
            ...
            for fd in self.images:
                for setting, tag in (
                    ...
                    if fd.metadata_choice in (tag, M_BOTH):
                        choices.update(
                            nucleus.measurement.find_metadata_tokens(setting.value)
                        )
            if (
                any([nucleus.measurement.is_well_column_token(x) for x in choices])
                and any([nucleus.measurement.is_well_row_token(x) for x in choices])
                and not any(
                    [x.lower() == nucleus.measurement.FTR_WELL.lower() for x in choices]
                )
            ):
                choices.add(nucleus.measurement.FTR_WELL)
        ...
        for i, fd in enumerate(self.images):
            ...
            if not is_multichannel:
                ...
                if self.channel_wants_images(fd.channels[0]):
                    ...
                    varlist += [
                        fd.channels[0].object_name,
                        fd.channels[0].wants_outlines,
                    ]
    ...
    def validate_module_warnings(self, pipeline):
        ...
        from nucleus.modules.loaddata import LoadData
    ...
    def settings(self):
        ...
        for image_group in self.images:
            ...
            for channel in image_group.channels:
                setting_values += [
                    channel.image_object_choice,
                    channel.image_name,
                    channel.object_name,
                    channel.wants_outlines,
                    channel.outlines_name,
    ...
    def has_metadata(self):
        ...
        return any(
            [
                self.has_file_metadata(fd) or self.has_path_metadata(fd)
                for fd in self.images
            ]
        )
    ...
    def upgrade_settings(
        ...
        def upgrade_new_4_to_5(setting_values):
            ...
            setting_values = nucleus.setting.standardize_default_folder_names(
            ...
            if location == nucleus.preferences.ABSOLUTE_FOLDER_NAME:
                if custom_location.startswith("."):
                    location = nucleus.setting.DEFAULT_INPUT_SUBFOLDER_NAME
                elif custom_location.startswith("&"):
                    location = nucleus.setting.DEFAULT_OUTPUT_SUBFOLDER_NAME
                    custom_location = "." + custom_location[1:]
            location = nucleus.setting.DirectoryPath.static_join_string(
        ...
        def upgrade_new_5_to_6(setting_values):
            ...
            for i in range(image_count):
                ...
                new_values += setting_values[
                    (
                        self.SLOT_OFFSET_IMAGE_NAME_V5 + 1
                ...
                # number of 1
                #
                new_values += ["1", setting_values[self.SLOT_OFFSET_IMAGE_NAME_V5], "1"]
        ...
        def upgrade_new_8_to_9(setting_values):
            ...
            for i in range(image_count):
                ...
                for j in range(channel_count):
                    ...
                    new_values += setting_values[
                        (
                            self.SLOT_OFFSET_IMAGE_NAME_V8 + 1
        ...
        if variable_revision_number == 2:
            setting_values, variable_revision_number = upgrade_new_2_to_3(
                setting_values
            )
        if variable_revision_number == 3:
            setting_values, variable_revision_number = upgrade_new_3_to_4(
                setting_values
            )
        if variable_revision_number == 4:
            setting_values, variable_revision_number = upgrade_new_4_to_5(
                setting_values
            )
        if variable_revision_number == 5:
            setting_values, variable_revision_number = upgrade_new_5_to_6(
                setting_values
            )
        if variable_revision_number == 6:
            setting_values, variable_revision_number = upgrade_new_6_to_7(
                setting_values
            )
        if variable_revision_number == 7:
            setting_values, variable_revision_number = upgrade_new_7_to_8(
                setting_values
            )
        if variable_revision_number == 8:
            setting_values, variable_revision_number = upgrade_new_8_to_9(
                setting_values
            )
        if variable_revision_number == 9:
            setting_values, variable_revision_number = upgrade_new_9_to_10(
                setting_values
            )
        if variable_revision_number == 10:
            setting_values, variable_revision_number = upgrade_new_10_to_11(
                setting_values
            )
        ...
        setting_values[
            self.SLOT_LOCATION
        ] = nucleus.setting.DirectoryPath.upgrade_setting(
            setting_values[self.SLOT_LOCATION]
        )
    ...
    def organize_by_order(self, workspace, files):
        ...
        for i in range(image_set_count):
            for j, image_name in enumerate(image_names):
                ...
                self.write_measurements(m, image_number, self.images[j], full_path)
    ...
    def organize_by_metadata(self, workspace, files):
            ...
            for path, filename in files_by_image_name[i]:
                ...
                if value in parent:
                    ...
                    if not self.check_images:
                        ...
                        old_stat = os.stat(
                            os.path.join(self.image_directory(), old_path, old_filename)
                        )
                        old_time = old_stat.st_mtime
                        new_stat = os.stat(
                            os.path.join(self.image_directory(), path, filename)
                        )
        ...
        missing_images = [image_set for image_set in image_sets if None in image_set[1]]
        image_sets = [image_set for image_set in image_sets if None not in image_set[1]]
        #
                ...
                if len(conflicts):
                    ...
                    for conflict in conflicts:
                        metadata = self.get_filename_metadata(
                            conflict[0], conflict[1][1], conflict[1][0]
                        )
                        message += (
                ...
                if len(missing_images):
                    ...
                    for mi in missing_images:
                        for i in range(len(self.images)):
                            ...
                            if mi[1][i] is None:
                                message += (
                                    "%s: missing " % fd.channels[0].image_name.value
                                )
                            else:
        ...
        for k, v in list(features.items()):
            workspace.measurements.add_all_measurements(nucleus.measurement.IMAGE, k, v)
    ...
    def get_image_sets(self, d):
        ...
        for value in sorted(values):
            subgroup = tuple(
                (dd and None in dd and dd[None])
                or (dd and dd.get(value))  # wildcard metadata
                or None  # fetch subvalue or None if missing  # metadata is missing
                for dd in d
            )
    ...
    def get_image_numbers_by_tags(self, workspace, tags):
        ...
        metadata_features = [
            x
            for x in measurements.get_feature_names(nucleus.measurement.IMAGE)
            if x.startswith(nucleus.measurement.C_METADATA)
        ]
        for tag in tags:
            if "_".join((nucleus.measurement.C_METADATA, tag)) not in metadata_features:
                message = (
                    "LoadImages needs the prior Load modules to define "
                    'the metadata tag, "%s", in order to match metadata' % tag
                )
            ...
            keys = tuple(
                [
                    measurements.get_measurement(
                        ...
                        "_".join((nucleus.measurement.C_METADATA, tag)),
                        i,
                    )
                    for tag in tags
                ]
    ...
    def report_errors(self, conflicts, missing_images, frame):
        ...
        my_frame = wx.Frame(
            frame,
            title="Load images: Error report",
            size=(600, 800),
            pos=(frame.Position[0], frame.Position[1] + frame.Size[1]),
        )
        ...
        if len(conflicts):
            title = wx.StaticText(
                panel, label="Conflicts (two files with same metadata)"
            )
            title.Font = font
            sizer.Add(
                title,
            ...
            for conflict in conflicts:
                ...
                row.extend(
                    [conflict[1][0], conflict[1][1], conflict[2][0], conflict[2][1]]
                )
                table.Append(row)

        if len(missing_images):
            title = wx.StaticText(panel, label="Missing images")
            title.Font = font
            sizer.Add(
                title,
                0,
    ...
    def prepare_run_of_flex(self, workspace):
        ...
        #
        file_list = [
            [column[i] for column in file_list] for i in range(len(file_list[0]))
        ]
        root = self.image_directory()
        ...
        for image_set_files in file_list:
            ...
            d = []
            for image_settings, file_pathname in zip(self.images, image_set_files):
                ...
                if len(d) == 0:
                    ...
                    message = "File %s has %d series, but file %s has %d series." % (
                        image_set_files[0],
                        len(d),
                        file_pathname,
                        image_count,
                ...
                for i in range(image_count):
                    ...
                    if "Z" not in d[i]:
                        ...
                        message = (
                            "File %s, series %d has %d Z stacks, " "but file %s has %d"
                        ) % (
                            image_set_files[0],
                            i,
                            d[i]["Z"],
                        ...
                        for idx in range(nsets):
                            ...
                            for image_number in image_numbers:
                                for channel_settings in image_settings.channels:
                                    ...
                                    channel = (
                                        int(channel_settings.channel_number.value) - 1
                                    )
                                    ...
                                    t = (
                                        int(cidx / channel_count / stack_count)
                                        % timepoint_count
                                    )
                                    m.add_measurement(
                                        nucleus.measurement.IMAGE,
                                    ...
                                    m.add_measurement(
                                        nucleus.measurement.IMAGE,
                                        "_".join(
                                            (nucleus.measurement.C_URL, image_name)
                                        ),
                                        url,
                                        image_set_number=image_number,
                                    )
                                ...
                                for k in list(frame_metadata.keys()):
                                    m.add_measurement(
                                        nucleus.measurement.IMAGE,
                                        "_".join((nucleus.measurement.C_METADATA, k)),
                                        frame_metadata[k],
                                        image_set_number=image_number,
                                    )
                                image_set_count += 1
                        ...
                        for z in range(pixels.SizeZ):
                            for t in range(pixels.SizeT):
                                ...
                                for image_number in image_numbers:
                                    for channel_settings in image_settings.channels:
                                        c = (
                                            int(channel_settings.channel_number.value)
                                            - 1
                                        )
                                        image_name = channel_settings.get_image_name()
                                        if c >= channel_count:
                                            message = (
                                                "The flex file, "
                                                "%s"
                                                ", series # %d, has only %d channels. "
                                                "%s is assigned to channel % d"
                                            ) % (
                                                file_pathname,
                                                i,
                                                channel_count,
                                    ...
                                    for k in list(frame_metadata.keys()):
                                        m.add_measurement(
                                            nucleus.measurement.IMAGE,
                                            "_".join(
                                                (nucleus.measurement.C_METADATA, k)
                                            ),
                                            frame_metadata[k],
                                            image_set_number=image_number,
                                        )
    ...
    def prepare_run_of_movies(self, workspace):
                ...
                for group_number in range(group_count):
                    for i, channel in enumerate(image.channels):
                        ...
                        if image.interleaving == I_INTERLEAVED:
                            frame_number = group_number * group_size + channel_idx
                        else:
                            frame_number = group_count * channel_idx + group_number
                        list_of_lists[image_index + i] += [
                            (pathname, frame_number, group_number, image_group_index)
                        ]
        ...
        for x, name in zip(list_of_lists[1:], image_names):
            if len(x) != image_set_count:
                message = "Image %s has %d frames, but image %s has %d frames" % (
                    image_names[0],
                    image_set_count,
                    name,
                    len(x),
        ...
        for i in range(0, image_set_count):
            for name, (file, frame, t, image_group_index) in zip(
                image_names, list_of_lists[:, i]
            ):
                ...
                m.add_measurement(
                    nucleus.measurement.IMAGE,
                    "_".join((nucleus.measurement.C_METADATA, M_T)),
                    t,
    ...
    def run(self, workspace):
        ...
        for fd in self.images:
            for channel in fd.channels:
                ...
                if wants_images:
                    feature = nucleus.measurement.C_URL + "_" + image_name
                    path_feature = nucleus.measurement.C_PATH_NAME + "_" + image_name
                    file_feature = nucleus.measurement.C_FILE_NAME + "_" + image_name
                else:
                    feature = nucleus.measurement.C_OBJECTS_URL + "_" + image_name
                    path_feature = (
                        nucleus.measurement.C_OBJECTS_PATH_NAME + "_" + image_name
                    )
                    file_feature = (
                        nucleus.measurement.C_OBJECTS_FILE_NAME + "_" + image_name
                    )
                row = [
                    image_name,
                ...
                url = m.get_measurement(nucleus.measurement.IMAGE, feature)
                ...
                if self.file_types == FF_STK_MOVIES:
                    index = m.get_measurement(
                        nucleus.measurement.IMAGE, "_".join((C_FRAME, image_name))
                    )
                    ...
                    series = m.get_measurement(
                        nucleus.measurement.IMAGE, "_".join((C_SERIES, image_name))
                    )
                    index = m.get_measurement(
                        nucleus.measurement.IMAGE, "_".join((C_FRAME, image_name))
                    )
                    ...
                    for f in (M_Z, M_T):
                        feature = nucleus.measurement.C_METADATA + "_" + f
                        metadata[f] = m.get_measurement(
                            nucleus.measurement.IMAGE, feature
                        )
                    ...
                    metadata[M_T] = m.get_measurement(
                        nucleus.measurement.IMAGE,
                        nucleus.measurement.C_METADATA + "_" + M_T,
                    )
                ...
                if wants_images:
                    ...
                    if image_size is None:
                        ...
                        if nucleus.preferences.get_headless():
                            print(warning)
                    ...
                    if (
                        mdpixels.channel_count == 1
                        and mdpixels.Channel().SamplesPerPixel == 3
                    ):
                        #
                    ...
                    assert isinstance(object_set, nucleus.object.ObjectSet)
                    ...
                    if channel.wants_outlines:
                        ...
                        workspace.image_set.add(
                            channel.outlines_name.value, outline_image
                        )
    ...
    def get_filename_metadata(self, fd, filename, path):
        ...
        if self.has_file_metadata(fd):
            metadata.update(
                nucleus.measurement.extract_metadata(fd.file_metadata.value, filename)
            )
        if self.has_path_metadata(fd):
            path = os.path.abspath(os.path.join(self.image_directory(), path))
            metadata.update(
                nucleus.measurement.extract_metadata(fd.path_metadata.value, path)
            )
        if needs_well_metadata(list(metadata.keys())):
            well_row_token, well_column_token = well_metadata_tokens(
                list(metadata.keys())
            )
            metadata[nucleus.measurement.FTR_WELL] = (
    ...
    def write_measurements(
        ...
        for key in list(metadata.keys()):
            add_fn("_".join((nucleus.measurement.C_METADATA, key)), metadata[key])
    ...
    def get_metadata_tags(self, fd=None):
        ...
        tags = []
        if self.has_file_metadata(fd):
            tags += nucleus.measurement.find_metadata_tokens(fd.file_metadata.value)
        if self.has_path_metadata(fd):
            tags += nucleus.measurement.find_metadata_tokens(fd.path_metadata.value)
        if self.file_types == FF_OTHER_MOVIES:
            tags += [M_Z, M_T, C_SERIES]
        elif self.file_types in (FF_AVI_MOVIES, FF_STK_MOVIES):
            tags += [M_T]
        if needs_well_metadata(tags):
            tags += [nucleus.measurement.FTR_WELL]
        return tags
    ...
    def get_groupings(self, workspace):
        ...
        url_feature = "_".join((nucleus.measurement.C_URL, image_name))
        if self.do_group_by_metadata:
            keys = [
                "_".join((nucleus.measurement.C_METADATA, s))
                for s in self.metadata_fields.selections
            ]
            mapping = dict(
                [(key, s) for key, s in zip(keys, self.metadata_fields.selections)]
            )
            if len(keys) == 0:
                return None
            ...
            mapping = dict(
                ((url_feature, nucleus.measurement.C_URL), (series_feature, C_SERIES))
            )

        elif self.load_movies():
            keys = (url_feature,)
            mapping = dict(((url_feature, nucleus.measurement.C_URL),))
        else:
            return None
        ...
        #
        groupings = [
            (dict([(mapping[k], g[k]) for k in g]), image_numbers)
    ...
    def collect_files(self, workspace):
                ...
                if (
                    wx.MessageBox(
                        (
                            ...
                            'files again? Choose "No" if you are in a hurry.'
                        )
                        % how_long,
            ...
            if self.analyze_sub_dirs():
                ...
                for dirpath, dirnames, filenames in w:
                    ...
                    if path == os.path.curdir:
                        files += [(file_name, file_name) for file_name in filenames]
                    else:
                        files += [
                            (join(path, file_name), file_name)
        ...
        if self.load_choice() == MS_EXACT_MATCH:
            files = [
                (path, self.assign_filename_by_exact_match(file_name))
                for path, file_name in files
            ]
        elif self.load_choice() == MS_REGEXP:
            files = [
                (path, self.assign_filename_by_regexp(file_name))
                for path, file_name in files
            ]
        else:
            # Load by order.
            files = [
                path for path, file_name in files if self.filter_filename(file_name)
            ]
            files = [
                (path, self.assign_filename_by_order(idx))
                for idx, path in enumerate(files)
            ]

        files = [(path, idx) for path, idx in files if idx is not None]
        files.sort()
        if len(files) == 0:
            message = (
                "CellProfiler did not find any image files that "
                'matched your matching pattern: "%s"' % self.images[0].common_text.value
            )
            workspace.pipeline.report_prepare_run_error(self, message)
        return files
        ...
        if object_name == nucleus.measurement.IMAGE:
            ...
            for fd in self.images:
                if fd.metadata_choice != M_NONE:
                    has_metadata = True
            if has_metadata:
                res += [nucleus.measurement.C_METADATA]
            ...
            res += [nucleus.measurement.C_LOCATION, nucleus.measurement.C_NUMBER]
        return res
    ...
    def get_measurements(self, pipeline, object_name, category):
        ...
        if object_name == nucleus.measurement.IMAGE:
            if category == nucleus.measurement.C_COUNT:
                result += object_names
            else:
                result += [
                    c[1].split("_", 1)[1]
            ...
            if category == nucleus.measurement.C_NUMBER:
                result += [nucleus.measurement.FTR_OBJECT_NUMBER]
    ...
    def get_measurement_columns(self, pipeline):
        ...
        for fd in self.images:
            ...
            if self.has_file_metadata(fd):
                tokens = nucleus.measurement.find_metadata_tokens(
                    fd.file_metadata.value
                )
                cols += [
                    (
                        nucleus.measurement.IMAGE,
                        "_".join((nucleus.measurement.C_METADATA, token)),
                        nucleus.measurement.COLTYPE_VARCHAR_FILE_NAME,
                    )
                    for token in tokens
        ...
        if needs_well_metadata(all_tokens):
            cols += [
                (
                    nucleus.measurement.IMAGE,
                    "_".join(
                        (nucleus.measurement.C_METADATA, nucleus.measurement.FTR_WELL)
                    ),
                    nucleus.measurement.COLTYPE_VARCHAR_FILE_NAME,
                )
            ]
        if self.file_types in (FF_AVI_MOVIES, FF_STK_MOVIES):
            ...
            cols += [
                (
                    nucleus.measurement.IMAGE,
                    "_".join((nucleus.measurement.C_METADATA, feature)),
                    nucleus.measurement.COLTYPE_INTEGER,
                )
    ...
    def convert(self, pipeline, metadata, namesandtypes, groups):
        ...
        """
        import nucleus.modules.metadata as cpmetadata
        import nucleus.modules.namesandtypes as cpnamesandtypes
        import nucleus.modules.groups as cpgroups
        ...
        for group in self.images:
            for channel in group.channels:
                ...
                if channel.image_object_choice == IO_IMAGES:
                    name = assignment.image_name.value = channel.image_name.value
                ...
                assert isinstance(rfilter, nucleus.setting.Filter)
                structure = [nucleus.setting.Filter.AND_PREDICATE]
                fp = nucleus.modules.images.FilePredicate()
                fp_does, fp_does_not = [
                ...
                for metadata_choice, source, value in (
                    ...
                    if group.metadata_choice in (metadata_choice, M_BOTH):
                        ...
                        my_tags.update(nucleus.measurement.find_metadata_tokens(value))
        ...
        if self.group_by_metadata:
            ...
            for i, field in enumerate(metadata_fields):
                ...
                md_group.metadata_choice.value = field
...
def well_metadata_tokens(tokens):
    ...
    for token in tokens:
        if nucleus.measurement.is_well_row_token(token):
            well_row_token = token
        if nucleus.measurement.is_well_column_token(token):
...
def needs_well_metadata(tokens):
    ...
    """
    if nucleus.measurement.FTR_WELL.lower() in [x.lower() for x in tokens]:
        return False
...
class LoadImagesImageProvider(nucleus.image.AbstractImageProvider):
    ...
    def cache_file(self):
        ...
        if parsed_path.scheme == "file":
            ...
            #
            temp_dir = nucleus.preferences.get_temporary_directory()
