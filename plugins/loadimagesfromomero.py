...
class OmeroLoadImages(cpm.CPModule):
    ...
    def add_channelfn(self, can_remove=True):
        ...
        group.append("channel_number", cps.Choice(
            "Channel number:", channel_numbers,
    ...
    def visible_settings(self):
        varlist = [self.omero_host, self.omero_port, self.omero_username,
                   ...
                   self.load_channels]
        for channel in self.channels:
            varlist += [channel.divider, channel.cpimage_name,
        ...
        for setting in (self.omero_host, self.omero_port, self.omero_username,
            ...
            if setting.value == '':
                raise cps.ValidationError(
                    "Cannot continue if \"%s\" is not set." % setting.get_text(),
                    setting)
            ...
            for channel in self.channels:
                for z in range(0, sizeZ):
                    for t in range(0, sizeT):
                        ...
                        self.save_image_set_info(image_set,
                                                 channel.cpimage_name.value,
                                                 P_OMERO, V_OMERO,
                                                 self.omero_gateway, pixels_id,
                                                 z, c, t)
        return True
    ...
    def save_image_set_info(self, image_set, image_name, provider, version,
                            *args):
    ...
    def load_image_set_info(self, image_set):
        ...
        for image_name in d.keys():
            ...
            if (provider, version) == (P_OMERO, V_OMERO):
                ...
                raise NotImplementedError(
                    "Can't restore file information: image provider %s and/or version %d not supported" % provider,
                    version)
    ...
    def run(self, workspace):
        ...
        if cpp.get_headless():  # headless mode
            for channel in self.channels:
                ...
                (header, row) = workspace.display_data.statistics[
                    channel_number]
    ...
    def get_measurement_columns(self, pipeline):
        ...
        for channel in self.channels:
            ...
            cols += [(cpmeas.IMAGE, "_".join((M_IMAGE_NAME, name)),
                      cpmeas.COLTYPE_VARCHAR_FORMAT % 255),
                     (cpmeas.IMAGE, "_".join((M_IMAGE_ID, name)),
                      cpmeas.COLTYPE_INTEGER),
                     (cpmeas.IMAGE, "_".join((M_PIXELS_ID, name)),
                      cpmeas.COLTYPE_INTEGER),
                     (cpmeas.IMAGE, "_".join((M_Z, name)),
                      cpmeas.COLTYPE_INTEGER),
                     (cpmeas.IMAGE, "_".join((M_C, name)),
                      cpmeas.COLTYPE_INTEGER),
                     (cpmeas.IMAGE, "_".join((M_T, name)),
                      cpmeas.COLTYPE_INTEGER)]
            if self.omero_object == MS_DATASET:
                cols += [(cpmeas.IMAGE, "_".join((M_DATASET_NAME, name)),
                          cpmeas.COLTYPE_VARCHAR_FORMAT % 255),
                         (cpmeas.IMAGE, "_".join((M_DATASET_ID, name)),
                          cpmeas.COLTYPE_INTEGER)]
            elif self.omero_object == MS_PLATE:
                cols += [(cpmeas.IMAGE, "_".join((M_PLATE_NAME, name)),
                          cpmeas.COLTYPE_VARCHAR_FORMAT % 255),
                         (cpmeas.IMAGE, "_".join((M_PLATE_ID, name)),
                          cpmeas.COLTYPE_INTEGER),
                         (cpmeas.IMAGE, "_".join((M_WELL_ROW, name)),
                          cpmeas.COLTYPE_INTEGER),
                         (cpmeas.IMAGE, "_".join((M_WELL_COLUMN, name)),
                          cpmeas.COLTYPE_INTEGER),
                         (cpmeas.IMAGE, "_".join((M_WELL_ID, name)),
                          cpmeas.COLTYPE_INTEGER)]
        return cols
