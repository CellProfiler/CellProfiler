...
'''
    ...
    def create_settings(self):
        ...
        self.add_button = cps.DoSomething("Add another image","Add image", self.add_image)
    
    def add_image(self):
        # The text for these settings will be replaced in renumber_settings()
        self.image_names.append(cps.ImageNameSubscriber("Temp", "None"))
        self.factors.append(cps.Float("Temp", 1.0))
        self.renumber_settings()

    def remove_image(self, text_to_remove):
        texts = [n.text for n in self.image_names]
        idx = texts.index(text_to_remove)
        del self.image_names[idx]
        del self.factors[idx]
        self.renumber_settings()

    def renumber_settings(self):
        for idx, name in enumerate(self.image_names):
            name.text = "Choose %s image:"%(ordinal(idx + 1))
        for idx, factor in enumerate(self.factors):
            factor.text = "Enter a factor to multiply the %s image by (before other operations):"%ordinal(idx + 1)

    def settings(self):
        result = [self.operation, self.exponent, self.after_factor, self.addend,
                  self.truncate_low, self.truncate_high, 
                  self.output_image_name]
        for (name, factor) in zip(self.image_names, self.factors):
            result += [name, factor]
        return result
    ...
    def visible_settings(self):
        ...
        for index, (name, factor) in enumerate(zip(self.image_names, self.factors)):
            if index >= 1 and single_image:
                # these operations use the first image only
                break 
            if index < 2:
                # the first two images can't be removed
                result += [name, factor]
            else:
                remover = cps.DoSomething("Remove the image above", "Remove", self.remove_image, name.text)
                result += [name, factor, remover]
        if not single_image:
    ...
    def run(self, workspace):
        image_names = [setting.value for setting in self.image_names]
        image_factors = [setting.value for setting in self.factors]
