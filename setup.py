...
jars = JARS + [os.path.join(imagej_path, jar) for jar in
               get_cellprofiler_jars()]
...
OPTIONS = {'argv_emulation': True,
           ...
           'includes': ['numpy', 'scipy', 'sklearn',
                        'sklearn.utils.sparsetools.*',
                        'wx', 'matplotlib', 'email.iterators', 'smtplib', 'zmq',
                        'javabridge', 'bioformats',
                        'sqlite3', 'libtiff', 'wx.lib.intctrl',
                        'libtiff.' + tiff_h_name,
                        'xml.dom.minidom', 'h5py', 'h5py.defs', 'h5py.utils',
                        'h5py._proxy', 'readline'],
...
if sys.argv[-1] == 'py2app':
    assert not os.path.exists(
        "build"), "Remove the build and dist directories before building app!"
    assert not os.path.exists(
        "dist"), "Remove the build and dist directories before building app!"
...
if sys.argv[-1] == 'py2app':
    ...
    call('find dist/CellProfiler.app -name tests -type d | xargs rm -rf',
         shell=True)
    ...
    call(
        'rm dist/CellProfiler.app/Contents/Resources/lib/python2.7/cellprofiler/artwork/*.png',
        shell=True)
