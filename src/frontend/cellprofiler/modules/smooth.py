...
from cellprofiler_library.modules._smooth import smooth
...
-  *{FIT_POLYNOMIAL}:* This method is fastest but does not allow
...
-  *{GAUSSIAN_FILTER}:* This method convolves the image with a
   ...
   specified diameter.
-  *{MEDIAN_FILTER}:* This method finds the median pixel value within
   the diameter you specify. It removes bright or dim features
   that are significantly smaller than the specified diameter.
-  *{SMOOTH_KEEPING_EDGES}:* This method uses a bilateral filter
   ...
   an image while smoothing other features. *{SMOOTH_KEEPING_EDGES}*
   ...
   lower these numbers.
-  *{CIRCULAR_AVERAGE_FILTER}:* This method convolves the image with
...
-  *{SM_TO_AVERAGE}:* Creates a flat, smooth image where every pixel
   of the image equals the average value of the original image.

*Note, when deciding between {MEDIAN_FILTER} and {GAUSSIAN_FILTER}
we typically recommend
{MEDIAN_FILTER} over {GAUSSIAN_FILTER} because the
...
""".format(
    **{
        "FIT_POLYNOMIAL": SmoothingMethod.FIT_POLYNOMIAL.value,
        "GAUSSIAN_FILTER": SmoothingMethod.GAUSSIAN_FILTER.value,
        "MEDIAN_FILTER": SmoothingMethod.MEDIAN_FILTER.value,
        "SMOOTH_KEEPING_EDGES": SmoothingMethod.SMOOTH_KEEPING_EDGES.value,
        "CIRCULAR_AVERAGE_FILTER": SmoothingMethod.CIRCULAR_AVERAGE_FILTER.value,
        "SM_TO_AVERAGE": SmoothingMethod.SM_TO_AVERAGE.value,
    }
),
        )
        ...
        self.wants_automatic_object_size = Binary(
            ...
            True,
            doc="""\
*(Used only if “{GAUSSIAN_FILTER}”, “{MEDIAN_FILTER}”, “{SMOOTH_KEEPING_EDGES}” or “{CIRCULAR_AVERAGE_FILTER}” is selected)*
...
and one. {pixel_help}
""".format(
                smooth_help=SmoothingMethod.SMOOTH_KEEPING_EDGES, pixel_help=HELP_ON_PIXEL_INTENSITIES
            ),
        )
...
the output image.
""".format(
                fit=SmoothingMethod.FIT_POLYNOMIAL
            ),
        )
    ...
    def visible_settings(self):
        ...
        if self.smoothing_method.value not in [SmoothingMethod.FIT_POLYNOMIAL, SmoothingMethod.SM_TO_AVERAGE]:
            ...
            if self.smoothing_method.value == SmoothingMethod.SMOOTH_KEEPING_EDGES:
                result.append(self.sigma_range)
        if self.smoothing_method.value == SmoothingMethod.FIT_POLYNOMIAL:
    ...
    def run(self, workspace):
        ...
        output_pixels = smooth(
            pixel_data = pixel_data, 
            mask = image.mask, 
            multichannel = image.multichannel, 
            object_size = None if self.wants_automatic_object_size else self.object_size.value,
            smoothing_method = self.smoothing_method.value, 
            sigma_range = self.sigma_range.value, 
