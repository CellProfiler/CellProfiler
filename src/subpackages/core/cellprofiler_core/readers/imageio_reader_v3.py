...
# bioformats returns 2 for these, imageiov3 reader returns 3
...
class ImageIOReaderV3(Reader):
    """
    Reads basic image formats using ImageIO v3.
    """

    reader_name = "ImageIOV3"
    ...
    def get_reader(self, volume=False):
        if self._reader is None or volume != self._volume:
            path = self.file.path
            if volume:
                self._reader = imageio.v3.imopen(path, 'rv')
            else:
                self._reader = imageio.v3.imopen(path, 'ri')
            self._volume = volume
        return self._reader
    ...
    def supports_format(cls, image_file, allow_open=False, volume=False):
        ...
        if image_file.full_extension in SEMI_SUPPORTED_EXTENSIONS:
            if config_read_typed(f"Reader.{ImageIOReaderV3.reader_name}.read_tif", bool):
                return 2
            return 3
