...
from typing import Tuple, Optional, Tuple, Dict, Callable, List, Union, Any, Sequence
...
from cellprofiler_library.functions.object_processing import relate_labels, size_similarly

from cellprofiler_library.types import Pixel, ObjectLabel, ImageGrayscale, ImageGrayscaleMask, ImageAny, ImageBinary, ImageBinaryMask, ObjectSegmentation, ObjectLabelsDense, ObjectLabelSet, ObjectSegmentationIJV, Image2DBinary, Image2DColor, Image2DGrayscale, Image2DGrayscaleMask
...
def get_intensity_measurement_values(
    ...
    pixel_count = numpy.float_(numpy.product(pixels.shape))
    pixel_sum = numpy.float_(0.0)
    pixel_mean = numpy.float_(0.0)
    pixel_std = numpy.float_(0.0)
    pixel_mad = numpy.float_(0.0)
    pixel_median = numpy.float_(0.0)
    pixel_min = numpy.float_(0.0)
    pixel_max = numpy.float_(0.0)
    if pixel_count != 0:
        pixel_sum = numpy.sum(pixels)
        pixel_mean = pixel_sum / float(pixel_count)
        pixel_std = numpy.std(pixels)
