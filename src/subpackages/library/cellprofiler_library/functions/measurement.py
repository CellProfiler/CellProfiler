...
def compute_center_distances(
    ...
    the point farthest from its edge. Otherwise, the supplied label matrix
    provides centering objects:
      - ``use_centers_of_other=True`` collapses each centering object to its
        centroid before propagating distances.
      - ``use_edges_of_other=True`` keeps the centering objects extended and
        excludes their pixels from the result.
...
def compute_minimum_enclosing_circles(
    ...
    tuples (matching the iteration shape of ``Objects.get_labels()``).
    Returns ``(ij, r)`` arrays sized ``n_objects + 1`` (slot 0 is unused
    so callers can index by 1-based label).
    """
...
def calculate_centroid_distances(parents, children, parents_of): 
    """Calculate the centroid-centroid distance between parent & child"""
