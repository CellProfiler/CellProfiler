...
isscalar = cast(Callable[[ImageAny], bool], _isscalar)
...
def apply_binary_operation(
    ...
    masks: List[ImageMaskAny], 
    output_pixel_data: ImageAny, 
    output_mask: ImageMaskAny,
    ...
    op: Callable[[ImageAny, ImageAny], ImageAny] = lambda im1, im2 : numpy.ones(im1.shape)
    ...
    if opval in (Operator.ADD, Operator.AVERAGE):
        ...
        op = numpy.logical_xor if use_logical else lambda x, y: numpy.abs(numpy.subtract(x, y))

    elif opval == Operator.MULTIPLY:
        op = numpy.logical_and if use_logical else numpy.multiply
    ...
    for pd, mask in zip(pixel_data[1:], masks[1:]):
        output_pixel_data = apply_on_image(output_pixel_data, pd, comparitor, op, opval, use_logical)
        if ignore_mask:
            continue
        else:
