...
class TestAlign(unittest.TestCase):
    ...
    def test_01_02_load_v1(self):
        ...
        data = (
            "eJztWl9P2lAUvyAuui2b7mV7vCZ7kE0IFJYoWRQmS8YmjgjRGeO2K1zgJre"
        ...
        fd = six.moves.StringIO(zlib.decompress(base64.b64decode(data)))
        pipeline = cellprofiler.pipeline.Pipeline()
        pipeline.load(fd)
        #
        ...
        for i, input, output in (
            ...
            (
                3,
                module.additional_images[0].input_image_name,
                module.additional_images[0].output_image_name,
            ),
            (
                4,
                module.additional_images[1].input_image_name,
        ...
        self.assertEqual(
            module.additional_images[0].align_choice.value,
            cellprofiler.modules.align.A_SIMILARLY,
        )
        self.assertEqual(
            module.additional_images[1].align_choice.value,
            cellprofiler.modules.align.A_SEPARATELY,
        )
        self.assertEqual(
            module.alignment_method.value,
            cellprofiler.modules.align.M_MUTUAL_INFORMATION,
        )
        self.assertEqual(module.crop_mode, cellprofiler.modules.align.C_SAME_SIZE)
            ...
            for mask1 in (None, numpy.random.uniform(size=shape) > 0.1):
                for mask2 in (None, numpy.random.uniform(size=shape) > 0.1):
                    for method in (
                        ...
                        if (
                            method == cellprofiler.modules.align.M_CROSS_CORRELATION
                            and ((mask1 is not None) or (mask2 is not None))
                        ):
                            continue
                        ...
                        if mask1 is not None:
                            image1[~mask1] = numpy.random.uniform(
                                size=numpy.sum(~mask1)
                            )
                        if mask2 is not None:
                            image2[~mask2] = numpy.random.uniform(
                                size=numpy.sum(~mask2)
                            )
            ...
            for mask1 in (None, numpy.random.uniform(size=shape[:2]) > 0.1):
                for mask2 in (None, numpy.random.uniform(size=shape[:2]) > 0.1):
                    for method in (
                        ...
                        image1 = numpy.dstack(
                            [
                                numpy.random.randint(0, 10, size=shape[:2]).astype(
                                    float
                                )
                                / 10.0
                            ]
