...
def test_01_01_combine():
    ...
    assert grayscale
    img = grayscale.image
    np.testing.assert_almost_equal(img[0, 0], 1.0 / 6.0)
    np.testing.assert_almost_equal(img[0, 25], 1.0 / 3.0)
    np.testing.assert_almost_equal(img[25, 0], 1.0 / 2.0)
    np.testing.assert_almost_equal(img[25, 25], 0)
...
def test_01_02_split_all():
    ...
    assert red
    img = red.image
    np.testing.assert_almost_equal(img[0, 0], 1)
    np.testing.assert_almost_equal(img[0, 25], 0)
    np.testing.assert_almost_equal(img[25, 0], 0)
    np.testing.assert_almost_equal(img[25, 25], 0)
    green = image_set.get_image("my_green")
    assert green
    img = green.image
    np.testing.assert_almost_equal(img[0, 0], 0)
    np.testing.assert_almost_equal(img[0, 25], 1)
    np.testing.assert_almost_equal(img[25, 0], 0)
    np.testing.assert_almost_equal(img[25, 25], 0)
    ...
    np.testing.assert_almost_equal(img[0, 0], 0)
    np.testing.assert_almost_equal(img[0, 25], 0)
    np.testing.assert_almost_equal(img[25, 0], 1)
    np.testing.assert_almost_equal(img[25, 25], 0)
...
def test_2_1_load_matlab_combine():
    ...
    pipeline = cpmt.load_pipeline(None, data)
...
def test_2_4_load_split():
    ...
    pipeline = cpmt.load_pipeline(None, data)
    module = pipeline.module(1)
    assert isinstance(module, cpm_ctg.ColorToGray)
    assert module.image_name.value == "TestInput"
    assert module.should_split
    assert module.use_red.value
    assert module.red_name.value == "TestRed"
    ...
    assert module.rgb_or_channels == cpm_ctg.CH_RGB
...
"""
    ...
    assert module.combine_or_split == cpm_ctg.COMBINE
    assert module.rgb_or_channels == cpm_ctg.CH_CHANNELS
