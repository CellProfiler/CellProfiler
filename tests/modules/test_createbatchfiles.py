...
"""
    ...
    def test_03_01_save_and_load(self):
        ...
        data = (
            "eJztWW1PGkEQXhC1WtPYTzb9tB+llROoGiWNgi9NSYUSIbZGbbvCApvu7ZJ7"
        ...
        for windows_mode in (
            (False, True) if sys.platform.startswith("win") else (False,)
        ):
            pipeline = cpp.Pipeline()
            ...
            try:
                ...
                self.assertTrue(
                    measurements.has_feature(cpmeas.EXPERIMENT, cpp.M_PIPELINE)
                )
                pipeline = cpp.Pipeline()
                pipeline.add_listener(callback)
                image_set_list = cpi.ImageSetList()
                ...
                workspace = cpw.Workspace(
                    pipeline, None, None, None, cpmeas.Measurements(), image_set_list
                )
