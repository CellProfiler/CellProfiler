...
EditObjectsManually:[module_num:1|svn_version:\'Unknown\'|variable_revision_number:4|show_window:True|notes:\x5B\x5D|batch_state:array(\x5B\x5D, dtype=uint8)|enabled:True|wants_pause:False]
    ...
    def test_02_02_measurements(self):
        ...
        for column in columns:
            ...
            self.assertEqual(
                len(
                    ["x" for c in columns if all([column[i] == c[i] for i in range(3)])]
                ),
                1,
            )
        for expected in expected_columns:
        ...
        for object_name, category_d in d.items():
            ...
            self.assertEqual(
                len(categories), len([k for k in list(category_d.keys()) if k != "Foo"])
            )
            ...
            for category, expected_features in category_d.items():
                ...
                for feature in features:
                    self.assertTrue(
                        feature in expected_features, "Unexpected feature: %s" % feature
                    )
                for feature in expected_features:
                    self.assertTrue(
                        feature in features, "Missing feature: %s" % feature
                    )
