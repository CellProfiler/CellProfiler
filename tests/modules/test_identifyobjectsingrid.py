...
class TestIdentifyObjectsInGrid(unittest.TestCase):
    ...
    def test_02_02_forced_location_auto(self):
        ...
        mask[
            np.abs(np.sqrt(idist ** 2 + jdist ** 2) - float(diameter + 1) / 2) <= 1
        ] = False
        #
    ...
    def test_03_01_natural_circle(self):
        ...
        #
        np.random.seed(0)
        x_locations += (np.random.uniform(size=x_locations.shape[0]) * 3 - 1).astype(
            int
        )
        y_locations += (np.random.uniform(size=y_locations.shape[0]) * 3 - 1).astype(
            int
        )
    ...
    def test_04_01_natural(self):
        ...
        guide_labels[guide_labels != 0] += (
            gridding.rows
            * gridding.columns
            * (np.random.uniform(size=np.sum(guide_labels != 0)) > 0.5)
        )
        #
