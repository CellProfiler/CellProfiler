...
import cellprofiler.modules.plugins.makeprojection
...
def test_load_v2():
    ...
    methods = (
        cellprofiler.modules.plugins.makeprojection.P_AVERAGE,
        cellprofiler.modules.plugins.makeprojection.P_MAXIMUM,
        cellprofiler.modules.plugins.makeprojection.P_MINIMUM,
        cellprofiler.modules.plugins.makeprojection.P_SUM,
        cellprofiler.modules.plugins.makeprojection.P_VARIANCE,
        cellprofiler.modules.plugins.makeprojection.P_POWER,
        cellprofiler.modules.plugins.makeprojection.P_BRIGHTFIELD,
    )
    assert len(pipeline.modules()) == len(methods)
    for method, module in zip(methods, pipeline.modules()):
        assert isinstance(module, cellprofiler.modules.plugins.makeprojection.MakeProjection)
...
def run_image_set(projection_type, images_and_masks, frequency=9, run_last=True):
    ...
    pipeline = cellprofiler_core.pipeline.Pipeline()
    module = cellprofiler.modules.plugins.makeprojection.MakeProjection()
...
def test_average():
    ...
    image = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_AVERAGE, images_and_masks
    )
...
def test_maximum():
    ...
    image = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_MAXIMUM, images_and_masks
    )
    assert not image.has_mask
    assert numpy.all(numpy.abs(image.pixel_data - expected) < numpy.finfo(float).eps)
...
def test_variance():
    ...
    image = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_VARIANCE, images_and_masks
    )
...
def test_power():
    ...
    image_out = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_POWER, images_and_masks, frequency=9
    )
...
def test_brightfield():
    ...
    for i, (img, _) in enumerate(images_and_masks):
        if i < 5:
            ...
            img[:5, 5:] = 0
    image_out = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_BRIGHTFIELD, images_and_masks
    )
    i, j = numpy.mgrid[: image.shape[0], : image.shape[1]]
    numpy.testing.assert_almost_equal(image_out.pixel_data[(i > 5) | (j < 5)], 0)
...
def test_minimum():
    ...
    image = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_MINIMUM, images_and_masks
    )
    assert not image.has_mask
    assert numpy.all(numpy.abs(image.pixel_data - expected) < numpy.finfo(float).eps)
...
def test_mask_unmasked():
    ...
    image = run_image_set(cellprofiler.modules.plugins.makeprojection.P_MASK, images_and_masks)
...
def test_filtered():
    ...
    images_and_masks = [(numpy.random.uniform(size=(10, 10)), None) for i in range(3)]
    image = run_image_set(
        cellprofiler.modules.plugins.makeprojection.P_AVERAGE, images_and_masks, run_last=False
    )
