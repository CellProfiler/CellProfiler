...
@pytest.mark.parametrize("grayscale_level", ["8", "256"])
def test_many_objects(pipeline, grayscale_level):
    ...
    assert all([oname in (INPUT_OBJECTS_NAME, cellprofiler.measurement.IMAGE)
                for oname, feature, coltype in all_columns])
    all_column_features = [
        feature for oname, feature, coltype in all_columns
        if oname == INPUT_OBJECTS_NAME]
    assert all([any([oname == cellprofiler.measurement.IMAGE and feature == afeature
                     for oname, feature, coltype in all_columns])
...
def test_measurements():
    ...
    for object_name in (cellprofiler.measurement.IMAGE, INPUT_OBJECTS_NAME):
        ...
        assert all([f in cellprofiler.modules.measuretexture.F_HARALICK for f in features])
        assert all([f in features for f in cellprofiler.modules.measuretexture.F_HARALICK])


@pytest.mark.parametrize("grayscale_level", ["8", "256"])
def test_zeros(grayscale_level):
...
@pytest.mark.parametrize("grayscale_level", ["8", "256"])
def test_wrong_size(grayscale_level):
