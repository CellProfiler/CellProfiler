...
import cellprofiler.modules.plugins.overlayoutlines
...
def make_workspace(image, labels=None, dimensions=2):
    ...
    module = cellprofiler.modules.plugins.overlayoutlines.OverlayOutlines()
...
def test_load_v2():
    ...
    assert isinstance(module, cellprofiler.modules.plugins.overlayoutlines.OverlayOutlines)
    ...
    assert module.max_type == cellprofiler.modules.plugins.overlayoutlines.MAX_IMAGE
...
def test_load_v3():
    ...
    for outline, name, color, choice, objects_name in (
        (
            ...
            "PrimaryOutlines",
            "Red",
                cellprofiler.modules.plugins.overlayoutlines.FROM_IMAGES,
            "Nuclei",
        ),
        (
                module.outlines[1],
            "SecondaryOutlines",
            "Green",
                cellprofiler.modules.plugins.overlayoutlines.FROM_OBJECTS,
            "Cells",
        ),
    ):
        assert outline.color == color
...
def test_gray_to_color_outlines():
    ...
    module.wants_color.value = cellprofiler.modules.plugins.overlayoutlines.WANTS_COLOR
...
def test_blank_to_gray():
    ...
    workspace, module = make_workspace(image, labels=[outline.astype(int)])
    module.blank_image.value = True
    module.wants_color.value = cellprofiler.modules.plugins.overlayoutlines.WANTS_GRAYSCALE
    module.line_mode.value = "Inner"
    module.run(workspace)
    output_image = workspace.image_set.get_image(OUTPUT_IMAGE_NAME)
    numpy.testing.assert_array_equal(output_image.pixel_data, expected)
...
def test_gray_max_image():
    ...
    module.blank_image.value = False
    module.wants_color.value = cellprofiler.modules.plugins.overlayoutlines.WANTS_GRAYSCALE
    module.max_type.value = cellprofiler.modules.plugins.overlayoutlines.MAX_IMAGE
    module.line_mode.value = "Inner"
    module.run(workspace)
    output_image = workspace.image_set.get_image(OUTPUT_IMAGE_NAME)
...
def test_gray_max_possible():
    ...
    module.blank_image.value = False
    module.wants_color.value = cellprofiler.modules.plugins.overlayoutlines.WANTS_GRAYSCALE
    module.max_type.value = cellprofiler.modules.plugins.overlayoutlines.MAX_POSSIBLE
    module.line_mode.value = "Inner"
    module.run(workspace)
    output_image = workspace.image_set.get_image(OUTPUT_IMAGE_NAME)
    numpy.testing.assert_almost_equal(output_image.pixel_data, expected)
