...
def test_load_v1():
    ...
    assert isinstance(module, cellprofiler.modules.rescaleintensity.RescaleIntensity)
    #
    # image_name = DNA
...
def make_workspace(
    ...
    ii = cellprofiler.modules.injectimage.InjectImage(
        INPUT_NAME, input_image, input_mask
    )
    ...
    if reference_image is not None:
        ...
        ii = cellprofiler.modules.injectimage.InjectImage(
            REFERENCE_NAME, reference_image, reference_mask
        )
        ii.module_num = module_number
        module_number += 1
        pipeline.add_module(ii)
...
def test_manual_input_range():
    ...
    module.source_scale.min = 0.1
    module.source_scale.max = 0.6
    module.run(workspace)
    pixels = workspace.image_set.get_image(OUTPUT_NAME).pixel_data
    numpy.testing.assert_almost_equal(pixels, expected)
...
def test_00_manual_input_range_auto_low():
    ...
    module.wants_automatic_low.value = (
        cellprofiler.modules.rescaleintensity.LOW_EACH_IMAGE
    )
    module.wants_automatic_high.value = (
        cellprofiler.modules.rescaleintensity.CUSTOM_VALUE
    )
...
def test_01_manual_input_range_auto_low_all():
    ...
    module.wants_automatic_low.value = (
        cellprofiler.modules.rescaleintensity.LOW_ALL_IMAGES
    )
    module.wants_automatic_high.value = (
        cellprofiler.modules.rescaleintensity.CUSTOM_VALUE
    )
...
def test_manual_input_range_truncate():
    ...
    expected[expected_low_mask] = -0.05
...
def test_manual_io_range():
    ...
    module.source_scale.min = 0.1
    module.source_scale.max = 0.6
    module.dest_scale.min = 0.05
    module.dest_scale.max = 0.80
    module.run(workspace)
    pixels = workspace.image_set.get_image(OUTPUT_NAME).pixel_data
    numpy.testing.assert_almost_equal(pixels, expected)
...
def test_scale_by_image_maximum_mask():
    ...
    mask[3:6, 4:8] = False
    image[~mask] = 0.9
    reference = numpy.random.uniform(size=(10, 10)) * 0.75
    reference[0, 0] = 0.75
