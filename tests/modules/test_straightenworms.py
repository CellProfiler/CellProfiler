...
"""
    ...
    assert module.training_set_directory.dir_choice == cps.DEFAULT_OUTPUT_FOLDER_NAME
...
"""
    ...
    for alignment, module in zip(
        ...
        assert module.straightened_objects_name == "StraightenedWorms"
        assert module.width == 20
        assert module.training_set_directory.dir_choice == cps.DEFAULT_INPUT_FOLDER_NAME
            ...
            for vscale in vscales:
                for hscale in hscales:
                    if vscale is None and hscale is None:
                        continue
                    meas = "_".join(
                        (S.C_WORM, ftr, image, module.get_scale_name(hscale, vscale))
                    )
                    expected_columns.append((OBJECTS_NAME, meas, cpmeas.COLTYPE_FLOAT))
    columns = module.get_measurement_columns(workspace.pipeline)
    columns = [column for column in columns if column[0] == OBJECTS_NAME]
    for expected_column in expected_columns:
        assert any(
...
def test_measure_no_worms(self):
    ...
    for i in range(5):
        for ftr, function in (
            ...
            mname = "_".join(
                (S.C_WORM, ftr, STRAIGHTENED_IMAGE_NAME, module.get_scale_name(None, i))
            )
...
def test_measure_checkerboarded_worm(self):
    ...
    expected_sds = np.sqrt(
        s2(
            i_w
            * (image[:, :, np.newaxis] - expected_means[np.newaxis, np.newaxis, :]) ** 2
        )
        / weights
        * counts
        / (counts - 1)
    ...
    for i in range(4):
        for ftr, expected in (
            (S.FTR_MEAN_INTENSITY, expected_means),
    ...
    expected_sds = np.sqrt(
        s2(
            ...
            * (
                ...
                - expected_means[np.newaxis, np.newaxis, :, :]
            )
            ** 2
        )
        / weights
