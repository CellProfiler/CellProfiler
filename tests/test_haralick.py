...
gray4 = numpy.array([[0, 0, 1, 1], [0, 0, 1, 1], [0, 2, 2, 2], [2, 2, 3, 3]])
gray = gray4 / (1.0 * gray4.max())
labels = numpy.ones((4, 4), dtype='int32')
...
class TestHaralick(unittest.TestCase):
    def test_quantize(self):
        q = centrosome.haralick.quantize(gray, 7)
        correct = numpy.array([[0, 0, 2, 2], [0, 0, 2, 2], [0, 4, 4, 4], [4, 4, 6, 6]],
                              dtype='i4')
        self.assertTrue((q == correct).all())

    def test_quantize_fixpoint(self):
        q = centrosome.haralick.quantize(gray, 4)
        self.assertTrue((q == gray4).all())

    def test_normalize_per_object(self):
        norm = centrosome.haralick.normalized_per_object(gray4, labels)
        self.assertTrue((norm == gray).all())

    def test_cooccurrence(self):
        P, levels = centrosome.haralick.cooccurrence(gray4, labels, 0, 1)
        correct = numpy.array([[[2, 2, 1, 0], [0, 2, 0, 0], [0, 0, 3, 1], [0, 0, 0, 1]]], float)
        correct = correct / numpy.sum(correct)
        self.assertEqual(levels, 4)
        self.assertTrue((P == correct).all())

    def test_H1(self):
        h = centrosome.haralick.Haralick(gray, labels, 0, 1, nlevels=4)
    ...
    def test_all_01(self):
        h = centrosome.haralick.Haralick(gray, labels, 0, 1, nlevels=4)
        fv = h.all()
        self.assertTrue((numpy.array(map(len, fv)) == 1).all())
    ...
    def test_01_01_regression_edge(self):
        ...
        numpy.random.seed(0)
        image = (numpy.random.uniform(size=(100, 100)) * 8).astype(int)
        c, nlevels = centrosome.haralick.cooccurrence(image, labels, 0, 30)
        self.assertTrue(numpy.all(c == 0))

    def test_01_02_mask(self):
        '''Test with a masked image'''
        labels = numpy.ones((10, 20), int)
        numpy.random.seed(12)
        image = numpy.random.uniform(size=(10, 20)).astype(numpy.float32)
        image[:, :10] *= .5
        mask = numpy.ones((10, 20), bool)
        mask[:, 10:] = False
        # Masked haralick
        hm = centrosome.haralick.Haralick(image, labels, 0, 1, mask=mask)
        # Expected haralick
        he = centrosome.haralick.Haralick(image[:, :10], labels[:, :10], 0, 1)
    ...
    def test_02_01_angles(self):
        ...
        numpy.random.seed(12)
        image = numpy.random.uniform(size=(10, 20)).astype(numpy.float32)
        hi = centrosome.haralick.Haralick(image, labels, 1, 0)
        hj = centrosome.haralick.Haralick(image.transpose(), labels.transpose(), 0, 1)
