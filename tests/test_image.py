...
class TestImageSet(unittest.TestCase):
    def test_01_01_add(self):
        ...
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.zeros((10, 20))))
    ...
    def test_02_01_must_be_binary(self):
        x = cellprofiler_core.image.ImageSet(0, {}, {})
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.zeros((10, 20), bool)))
        image = x.get_image(IMAGE_NAME, must_be_binary=True)
        self.assertEqual(tuple(image.pixel_data.shape), (10, 20))

    def test_02_02_must_be_binary_throws(self):
        x = cellprofiler_core.image.ImageSet(0, {}, {})
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.zeros((10, 20), float)))
    ...
    def test_03_02_must_be_gray_throws(self):
        ...
        x.add(
            IMAGE_NAME, cellprofiler_core.image.Image(numpy.random.uniform(size=(10, 20, 3)))
        )
        self.assertRaises(ValueError, x.get_image, IMAGE_NAME, must_be_grayscale=True)

    def test_03_03_must_be_gray_color(self):
        x = cellprofiler_core.image.ImageSet(0, {}, {})
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.zeros((10, 20, 3), float)))
        image = x.get_image(IMAGE_NAME, must_be_grayscale=True)
        self.assertEqual(tuple(image.pixel_data.shape), (10, 20))
    ...
    def test_04_02_must_be_color_throws(self):
        x = cellprofiler_core.image.ImageSet(0, {}, {})
        numpy.random.seed(22)
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.random.uniform(size=(10, 20))))
    ...
    def test_05_03_must_be_rgb_throws_5_channel(self):
        ...
        numpy.random.seed(22)
        x.add(
            IMAGE_NAME, cellprofiler_core.image.Image(numpy.random.uniform(size=(10, 20, 5)))
        )
        self.assertRaises(ValueError, x.get_image, IMAGE_NAME, must_be_rgb=True)

    def test_05_04_must_be_rgb_alpha(self):
        x = cellprofiler_core.image.ImageSet(0, {}, {})
        x.add(IMAGE_NAME, cellprofiler_core.image.Image(numpy.zeros((10, 20, 4), float)))
        image = x.get_image(IMAGE_NAME, must_be_rgb=True)
        self.assertEqual(tuple(image.pixel_data.shape), (10, 20, 3))
...
class TestImageSetList(unittest.TestCase):
    ...
    def test_02_01_add_image_provider(self):
        ...
        z = cellprofiler_core.image.CallbackImageProvider("TestImageProvider", fn)
    ...
    def test_02_02_add_two_image_providers(self):
        ...
        y.providers.append(cellprofiler_core.image.CallbackImageProvider("IP1", fn1))
        y.providers.append(cellprofiler_core.image.CallbackImageProvider("IP2", fn2))
    ...
    def test_03_02_serialize_key(self):
        ...
        for i in range(len(values)):
            ...
            self.assertTrue(isinstance(image_set, cellprofiler_core.image.ImageSet))
