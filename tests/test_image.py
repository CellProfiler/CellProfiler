...
class TestImage(unittest.TestCase):
    ...
    def test_07_02_cache(self):
        ...
        test_cases = (
            ...
            {"image": r.uniform(size=(10, 20)),
             "crop_mask": numpy.all(numpy.mgrid[0:10, 0:20] > 3, 0)})
        h, path = tempfile.mkstemp(suffix=".h5")
        hdf_file = h5py.File(path, "w")
        os.close(h)

        for test_case in test_cases:
            image = cellprofiler.image.Image(**test_case)
            image.cache("foo", hdf_file)
            expected = test_case["image"].astype(numpy.float32)
            numpy.testing.assert_array_equal(image.pixel_data, expected)
            if "mask" in test_case:
                numpy.testing.assert_array_equal(image.mask, test_case["mask"])
            else:
                numpy.testing.assert_equal(image.mask, True)
