...
class TestExample5a(unittest.TestCase):
    ...
    def run_tteesstt(self, objects, level):
        ...
        values = measurements.get_measurement(OBJECTS_NAME,
                                              "Example5_MeanDistance")
        ...
        for labels, indices in objects.get_labels():
            for l in np.unique(labels):
                ...
                if abs(value - values[l - 1]) > .0001:
                    if level == 1:
                        self.fail(
                            "You got the wrong answer (label = %d, mine = %f, yours = %f" %
                            l, value, values[l - 1])
                    else:
                        sys.stderr.write(
                            "Oh too bad, did not pass level %d\n" % level)
                        return
        if level > 1:
    ...
    def test_01_02_run_simple(self):
        ...
        labels, count = scipy.ndimage.label(fg_bg,
    ...
    def test_01_03_run_overlapped(self):
        ...
        ijv = np.vstack([
                            np.column_stack((i[plane != 0], j[plane != 0],
                                             plane[plane != 0]))
                            for plane in planes])
        objects = cpo.Objects()
